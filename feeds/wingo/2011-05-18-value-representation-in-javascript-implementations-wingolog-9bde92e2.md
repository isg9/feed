---
title: value representation in javascript implementations — wingolog
...
feed: wingo
guid: https://wingolog.org/archives/2011/05/18/value-representation-in-javascript-implementations
---
...
JavaScriptCore, the implementation used by WebKit, switched from tagged pointers to a "nan-boxed" format. The best explanation of this strategy is given by [Rob Sayre](http://blog.mozilla.com/rob-sayre/2010/08/02/mozillas-new-javascript-value-representation/) of Mozilla. I believe the technique was first made popular by LuaJIT, though I would appreciate additional references.
...
Well it turns out that on x64-64 systems, you only have a 48-bit address space, currently anyway. The shape of that space is [quite peculiar, too](http://en.wikipedia.org/wiki/X86-64#Virtual_address_space_details). I don't know whether Linux gives out addresses in the upper half, but Windows does not, though [Solaris does](https://bugzilla.mozilla.org/show_bug.cgi?id=577056).
...
JSC's implementation of nan-boxing is described [here](http://trac.webkit.org/browser/trunk/Source/JavaScriptCore/runtime/JSValue.h#L250), and you can see some of the neat tricks they do [here](http://trac.webkit.org/browser/trunk/Source/JavaScriptCore/runtime/JSValueInlineMethods.h). It actually makes me envious of C++, as a C programmer!
...
Amusingly, there is a third option as well. For 32-bit machines, you can address the second word in the double directly, so there is no need to mask off anything. This is the JSVALUE32\_64 case mentioned in the JSValue code I linked to above.

JSC chose to favor pointers, and as the first JS implementation to nan-box, got to call their choice "nan-boxing". Mozilla chose to favor doubles, and so made up the silly name ["nun-boxing"](https://bugzilla.mozilla.org/show_bug.cgi?id=549143).
...
1. John Cowan says:[18 May 2011 9:48 PM](#00264cdbe0bef18dacf79a24fa080c44b88c2c2d)
...
2. [Jeff Walden](http://whereswalden.com/) says:[19 May 2011 7:14 AM](#ccd400e3ded85e0b2857dc129181fd4890c6d7c3)
...
3. Mac says:[6 June 2011 1:47 PM](#9d3b41b7d159f31cdac084557f8785ef0c087366)
...
4. [Dave Mandelin](https://blog.mozilla.com/dmandelin/) says:[10 June 2011 6:52 PM](#3c135a64c50a009b2cd20f98d529599f5534146c)
...
5. Alex says:[26 November 2011 0:13 AM](#c2eb45b6d490b0a5175ff69d1f97b871ff2a0cb1)
...
6. Luke Wagner says:[23 December 2011 6:59 PM](#969f63bbe4eb912778c9da85feb0f5763e7a7862)

   Nice article! Just a few nits: the name "nunbox" refers to nan-boxing on 32-bit architectures and is roughly equivalent to JSC's JSVALUE32\_64 (the 'u' in 'nunbox' is for 'unboxed' since, as you pointed out, the low word is unboxed). The 64-bit nan-boxing encoding used by SpiderMonkey is named "punboxing" (the 'p' is for 'packed', since this scheme is logically like nunboxing, but with all the unused bits sucked out).

7. Sandro says:[18 March 2013 3:38 PM](#ac3f28844463fff409fd39fc00efbc547a2960ab)

   The IEEE 754 tagged pointer representation used by LuaJIT was [first invented in 1997 for an implementation of parallel Haskell](http://csg.csail.mit.edu/pubs/memos/Memo-396/memo-396.pdf).

8. gfour says:[12 May 2014 2:32 PM](#076ff2ecf12d7e4ecf5719c7e14085e815139d39)
...
9. Kane York says:[21 July 2015 1:05 AM](#2b409a478fb97ea7f22a4fcc4c996ff3ec751493)
