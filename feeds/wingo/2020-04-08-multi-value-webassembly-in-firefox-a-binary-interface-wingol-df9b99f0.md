---
title: 'multi-value webassembly in firefox: a binary interface — wingolog'
url: https://wingolog.org/archives/2020/04/08/multi-value-webassembly-in-firefox-a-binary-interface
published: "2020-04-08T00:00:00Z"
feed: wingo
guid: https://wingolog.org/archives/2020/04/08/multi-value-webassembly-in-firefox-a-binary-interface
---

# multi-value webassembly in firefox: a binary interface — wingolog
...
![](https://wingolog.org/pub/multi-value-abi-each.png)
...
![](https://wingolog.org/pub/multi-value-abi-area.png)
...
As it is, I did manage to hack it in, and I think in a way that doesn't regress. I added [a layer over an argument type vector that adds a synthetic stack results pointer argument, if the function returns stack results](https://searchfox.org/mozilla-central/rev/4d2a9d5dc8f0e65807ee66e2b04c64596c643b7a/js/src/wasm/WasmTypes.h#1179-1231); iterating over this type with [ABIArgIter](https://searchfox.org/mozilla-central/rev/4d2a9d5dc8f0e65807ee66e2b04c64596c643b7a/js/src/jit/MacroAssembler.h#3359-3398) will allocate a stack result area pointer, either as a register argument or a stack argument. In the optimizing compiler, I added [add a kind of value allocation coresponding to a variable-sized stack area](https://searchfox.org/mozilla-central/rev/4d2a9d5dc8f0e65807ee66e2b04c64596c643b7a/js/src/jit/LIR.h#379-413), (using [pointer tagging](https://searchfox.org/mozilla-central/rev/4d2a9d5dc8f0e65807ee66e2b04c64596c643b7a/js/src/jit/LIR.h#93-124) again!), and [extended the register allocator to allocate LStackArea](https://searchfox.org/mozilla-central/rev/4d2a9d5dc8f0e65807ee66e2b04c64596c643b7a/js/src/jit/BacktrackingAllocator.cpp#1280-1283), and the component stack results. Interestingly, I had to add a [kind of definition that starts life on the stack](https://searchfox.org/mozilla-central/rev/4d2a9d5dc8f0e65807ee66e2b04c64596c643b7a/js/src/jit/LIR.h#470-472); previously all Ion results started life in registers and were only spilled if needed.

In the end, a function will capture the incoming stack result area argument, either as a normal SSA value (for Ion) or stored to a stack slot (baseline), and when returning will write stack results to that pointer as appropriate. Passing in a pointer as an argument did make it relatively easy to implement calls from WebAssembly to and from C++, getting the [variable-shape result area to be known to the garbage collector for C++-to-WebAssembly calls](https://searchfox.org/mozilla-central/rev/4d2a9d5dc8f0e65807ee66e2b04c64596c643b7a/js/src/wasm/WasmInstance.cpp#1940-1957) was simple in the end but took me a while to figure out.
...
As far as code generation goes, things are progressing but it is still early days. Thomas Lively has implemented support in LLVM for emitting [return of C++ aggregates via multiple results](https://github.com/llvm/llvm-project/commit/8c3e6af71b8e827655c83997747a2042feddc845), which is enabled via the `-experimental-multivalue-abi` cc1 flag. Thomas has also been implementing [multi-value support in the binaryen WebAssembly toolchain component](https://github.com/WebAssembly/binaryen), used by the emscripten C++-to-WebAssembly toolchain. I think it will be a few months though before everything lands in a way that end users can take advantage of.

On the specification side, [the multi-value feature is now at phase 4](https://github.com/WebAssembly/proposals) since January, which basically means things are all done there.

Implementation-wise, V8 has had experimental support since 2017 or so, and the feature was [staged](https://chromium.googlesource.com/external/github.com/v8/v8.wiki/+/71511645ac05c5411b9dab09bc69f611cab5c0a5/Feature-launch-process.md#staging) [last fall](https://bugs.chromium.org/p/v8/issues/detail?id=6672), although V8 [doesn't yet support multi-value in their baseline compiler](https://chromium.googlesource.com/v8/v8/+/refs/heads/master/src/wasm/baseline/liftoff-compiler.cc#755). WebKit [also landed support last fall](https://trac.webkit.org/changeset/250559/webkit).

Unlike V8 and SpiderMonkey, JavaScriptCore (the JS and wasm engine in WebKit) actually implements a WebAssembly interpreter as their solution to the one-pass streaming compilation problem. Then on the compiler side, there are two tiers that both operate on basic block graphs ( [OMG and BBQ](https://webkit.org/blog/7691/webassembly/); I just puked a little in my mouth typing that). This strategy makes the compiler implementation quite straightforward. It's also an interesting design point because JavaScriptCore's garbage collector scans the stack conservatively; there's no need for the compiler to do bookkeeping on the GC's behalf, which I'm sure was a relief to the hacker. Anyway, multi-value in WebKit is done too.

The new thing of course is that finally, in Firefox, the feature is now [fully implemented](https://bugzilla.mozilla.org/showdependencytree.cgi?id=1401675&hide_resolved=0) (woo) and [enabled by default on Nightly builds](https://hg.mozilla.org/mozilla-central/rev/8e655f8db1fb8421f2f616de95b69e214fae106f) (woo!). I did that! It took me a while! Perhaps too long? Anyway it's done. Thanks again to [Bloomberg](https://techatbloomberg.com/) for supporting this work; large ups to y'all for helping the web move forward.
...
1. [Arne Babenhauserheide](http://draketo.de/index) says:[21 April 2020 5:43 PM](#3440dc9fba954521c640fb58179abb49d7f9e87b)

   very cool! Thank you for sharing!

2. [Charles](http://charles.stanho.pe) says:[6 May 2020 1:59 PM](#f6b7a4bfb3a919457a4606125adc767099a627d8)
