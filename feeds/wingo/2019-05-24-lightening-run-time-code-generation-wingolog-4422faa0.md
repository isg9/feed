---
title: lightening run-time code generation — wingolog
url: https://wingolog.org/archives/2019/05/24/lightening-run-time-code-generation
published: "2019-05-24T00:00:00Z"
feed: wingo
guid: https://wingolog.org/archives/2019/05/24/lightening-run-time-code-generation
---

# lightening run-time code generation — wingolog
...
The upcoming Guile 3 release will have just-in-time native code generation. Finally, amirite? There's lots that I'd like to share about that and I need to start somewhere, so this article is about one piece of it: [Lightening, a library to generate machine code](https://gitlab.com/wingo/lightening).

**on lightning**

Lightening is a fork of [GNU Lightning](https://www.gnu.org/software/lightning/), adapted to suit the needs of Guile. In fact at first we chose to use GNU Lightning directly, "vendored" into the Guile source respository via the [git subtree mechanism](https://git-scm.com/book/en/v1/Git-Tools-Subtree-Merging). (I see that in the meantime, `git` gained a kind of a [`subtree` command](https://github.com/git/git/blob/master/contrib/subtree/git-subtree.txt); one day I will have to figure out what it's for.)

[GNU Lightning](https://www.gnu.org/software/lightning/) has lots of things going for it. It has support for many architectures, even things like Itanium that I don't really care about but which a couple Guile users use. It abstracts the differences between e.g. x86 and ARMv7 behind a common API, so that in Guile I don't need to duplicate the JIT for each back-end. Such an abstraction can have a slight performance penalty, because maybe it missed the opportunity to generate optimal code, but this is acceptable to me: I was more concerned about the maintenance burden, and GNU Lightning seemed to solve that nicely.

GNU Lightning also has [fantastic documentation](https://www.gnu.org/software/lightning/manual/). It's written in C and not C++, which is the right thing for Guile at this time, and it's also released under the LGPL, which is Guile's license. As it's a GNU project there's a good chance that GNU Guile's needs might be taken into account if any changes need be made.

I mentally associated Paolo Bonzini with the project, who I knew was a good no-nonsense hacker, as he used Lightning for a [smalltalk implementation](http://smalltalk.gnu.org/); and I knew also that Matthew Flatt used Lightning in [Racket](https://racket-lang.org/). Then I looked in the source code to see architecture support and was pleasantly surprised to see MIPS, POWER, and so on, so I went with GNU Lightning for Guile in our [2.9.1 release last October](https://www.gnu.org/software/guile/news/gnu-guile-291-beta-released.html).
...
Incidentally, if you need a code generation library, consider your choices wisely. It is likely that Lightening is not right for you. If you can afford platform-specific code and you need C, Lua's [DynASM](https://luajit.org/dynasm.html) is probably the right thing for you. If you are in C++, copy the [assemblers](https://searchfox.org/mozilla-central/source/js/src/jit/arm/MacroAssembler-arm.cpp) from a [JavaScript](https://trac.webkit.org/browser/webkit/trunk/Source/JavaScriptCore/assembler) [engine](https://chromium.googlesource.com/v8/v8.git/+/refs/heads/master/src/x64/assembler-x64.h) \-\- C++ offers much more type safety, capabilities for optimization, and ergonomics.

But if you can only afford one emitter of JIT code for all architectures, you need simple C, you don't need register allocation, you want a simple library to just include in your source code, and you are good with the LGPL, then Lightening could be a thing for you. Check the [gitlab page](https://gitlab.com/wingo/lightening) for info on how to test Lightening and how to include it into your project.

**giving it a spin**

Yesterday's [Guile 2.9.2 release](https://lists.gnu.org/archive/html/guile-devel/2019-05/msg00030.html) includes Lightening, so you can give it a spin. The switch to Lightening allowed us to lower our JIT optimization threshold by a factor of 50, letting us generate fast code sooner. If you try it out, let `#guile` on freenode know how it went. In any case, happy hacking!
...
1. Nathan Myers says:[24 May 2019 10:16 PM](#684902643ec1b8e107afdf91c82c74dcabd843ac)
...
2. [Vítor De Araújo](https://elmord.org/blog/) says:[26 May 2019 4:30 PM](#d36a01529e74ce3ac4769a5839af5c758f819a1f)
...
3. Alexandr Karbivnichiy says:[27 May 2019 3:22 PM](#66e3bbc678877631170c060c3abd314e3a26a2ac)
...
4. Alexandr Karbivnichiy says:[27 May 2019 4:06 PM](#bb1cc5a758eb5c803c5e13b1a6bb720aed10e71c)
...
5. Reidar Rymdnalle says:[28 May 2019 6:47 AM](#7228ff10c8c2dc13c6edfd0a396f26309114509a)

   Alexandr: could you share the fasta code? I can't find any implementation of it, and it would be nice to play with it...

6. Alexandr Karbivnichiy says:[28 May 2019 1:17 PM](#1aa7577d11a757b3ab7cc40214fb7310529ba929)

   Reidar Rymdnalle:

   karbiv/guile\_benchmarksgame on github.

7. Dale P. Smith says:[28 May 2019 3:29 PM](#5dae7ea77da92c28459d7ec09f1b9aa179306484)

   Yey!

   Better and Better Andy!

8. [William ML Leslie](https://william-ml-leslie.id.au/) says:[6 June 2019 3:49 AM](#f112fbf8275591b5766d34a5c46fad6c716b6918)
