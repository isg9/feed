---
title: Slack's Secret STDERR Messages
url: https://www.brendangregg.com/blog/2021-08-27/slack-crashes-secret-stderr.html
published: "2021-08-27T00:00:00Z"
feed: gregg
guid: https://www.brendangregg.com/blog/2021-08-27/slack-crashes-secret-stderr.html
---
...
I'm guessing it's core dumping and Ubuntu's apport is eating them. Redirecting them to the file system so I can then do [core dump analysis using gdb(1)](/blog/2016-08-09/gdb-example-ncurses.html), as root:
...
Using an eBPF/ [bcc](https://github.com/iovisor/bcc) tool to look for exit reasons:
...
Nowadays this can be done as a [perf](/perf.html) one-liner:
...
Ok, so there's our slack SIGABRT, sent via tgkill(2). (And I filed an [issue](https://github.com/iovisor/bcc/issues/3592) to update bcc killsnoop(8) to use tracepoints.) This output doesn't really tell me much more about it though. I want to see a stack trace. I can use `perf record` or bpftrace...and that reminds me, didn't I write another signal tool using bpftrace?

## 5\. signals.bt

The [signals.bt](https://github.com/brendangregg/bpf-perf-tools-book/blob/master/originals/Ch13_Applications/signals.bt) bpftrace tool from my [BPF book](/bpf-performance-tools-book.html) traces the signal:signal\_generate tracepoint, which should catch every type of generated signal, including tgkill(2). Trying it out:
