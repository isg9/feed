...
url: https://www.brendangregg.com/blog/2017-09-05/solaris-to-linux-2017.html
published: "2017-09-05T00:00:00Z"
feed: gregg
guid: https://www.brendangregg.com/blog/2017-09-05/solaris-to-linux-2017.html
---
...
(Updated 12-Nov-2019) I recently met people who were still struggling with a Solaris to Linux migration. I recommended this post, and mentioned I needed to add a few technologies to bring it up to date. Now I have.
...
(Updated 12-Nov-2019) OpenZFS has now switched from the illumos repo to the [zfsonlinux repo](https://drive.google.com/file/d/197jS8_MWtfdW2LyvIFnH58uUasHuNszz/view) as the authorative source. This means the ZFS engineers have switched to Linux, greatly improving Linux support.
...
[![](http://www.brendangregg.com/Perf/linux_observability_tools.png)](http://www.brendangregg.com/linuxperf.html)
...
(Updated 12-Nov-2019) Linux has been adding tracing capabilities for years, and the final piece needed for DTrace functionality was added in Linux 4.9, released in December 2016. There are two recommended tracing front-ends: [bpftrace](https://github.com/iovisor/bpftrace) and [BCC](https://github.com/iovisor/bcc), which are installed by default on servers at Netflix and Facebook.
...
bpftrace great for one-liners and short scripts. I've ported over many of my DTrace tools to bpftrace, and cover them in my Addison Wesley book [BPF Performance Tools: Linux System and Application Observability](http://www.brendangregg.com/bpf-performance-tools-book.html), the tools from which I have made public and open source [here](https://github.com/brendangregg/bpf-perf-tools-book).

BCC, another front-end, is best for complex tools and deamons. I used it for the same reasons I wrapped some DTraceToolkit tools in the shell: to provide command line options and arguments (-p PID, etc). For example, here is my [zfsdist](https://github.com/iovisor/bcc/blob/master/tools/zfsdist_example.txt) BCC tool:

```
# zfsdist.py -h
usage: zfsdist.py [-h] [-T] [-m] [-p PID] [interval] [count]
[...]
...
It measures ZFS latency as a histogram on Linux:
...
Nowadays, the Linux kernel built-in tracers you should know about are:
...
- **BPF**: tracing features completed in 2016, this provides efficient programmatic tracing to existing kernel frameworks. I wrote about it in [DTrace for Linux 2016](/blog/2016-10-27/dtrace-for-linux-2016.html). Many new tools can now be written, and the recommended front-ends are [BCC](https://github.com/iovisor/bcc) and [bpftrace](https://github.com/iovisor/bpftrace).

You might also like my [Give me 15 minutes and I'll change your view of Linux tracing](https://www.youtube.com/watch?v=GsMs3n8CB6g) video as an introduction to the different built-in Linux tracers, although that was before bpftrace.
...
I have a page on [eBPF tracing](http://www.brendangregg.com/ebpf.html), and the current BCC tools are:

[![](http://www.brendangregg.com/Perf/bcc_tracing_tools.png)](http://www.brendangregg.com/ebpf.html)
...
On Linux, containers are a combination of namespaces (restricting what a process sees) and cgroups (similar to Solaris resource controls). People rarely create them manually. They use third-party software like Docker or Kubernetes to simplify their administration. I gave a talk about container performance recently at DockerCon, and included a quick summary of how they work: [YouTube](https://www.youtube.com/watch?v=bK9A5ODIgac), [slides](/Slides/DockerCon2017_performance_analysis):

[![](/blog/images/2017/dockercon2017_containers1.png)](/Slides/DockerCon2017_performance_analysis/#11)

If you search youtube, you'll find many other good talks on containers as well. Apart from Jessie, I also like talks by Jérôme Petazzoni, and Tejun Heo.
...
I spoke about the performance differences in my 2014 SCALE keynote "What Linux can learn from Solaris performance and vice-versa" ( [slides](https://www.brendangregg.com/Slides/SCaLE_Linux_vs_Solaris_Performance2014)) where the conclusion was that Linux may run faster out of the box, but I could typically make Solaris run much faster thanks to optimizations found using DTrace. DTrace didn't exist for Linux at the time, but now we have BPF (see previous section). There have been many other improvements to Linux since then, as well.
...
(Updated 12-Nov-2019) Instead of mdb you'll be using gdb, lldb, sdb, or drgn. gdb has been improving, and even has a TUI mode nowadays. I wrote [gdb Debugging Full Example (Tutorial)](http://www.brendangregg.com/blog/2016-08-09/gdb-example-ncurses.html), and I also recommend you watch Greg Law's talk [Give me 15 minutes and I'll change your view of GDB](http://undo.io/resources/presentations/cppcon-2015-greg-law-give-me-15-minutes-ill-change/).

Newer projects are using lldb. Here's an [lldb to gdb command map](http://lldb.llvm.org/lldb-gdb.html).

Omar Sandoval from Facebook has been developing a new debugger called [drgn](https://github.com/osandov/drgn), and Serapheim Dimitropoulos from Delphix has been developing a related tool, the Slick Debugger [sdb](https://github.com/delphix/sdb). Serapheim gave a talk about sdb at the OpenZFS developer summit ( [video](https://youtu.be/uDDJnzSb-2w), [slides](https://drive.google.com/open?id=1oho9X5bkW-I-yJ-pVD8VqkaloxhGepzT)), which included the following example:

[![](/blog/images/2017/sdb-screenshot1.png)](/blog/images/2017/sdb-screenshot1.png)

George Wilson (ex-Sun kernel engineer) also gave some great demos of sdb at [Linux Plumber's 2019](https://linuxplumbersconf.org/event/4/contributions/539/) in a session that was standing room only.

More Linux debugging tools are listed [here](https://github.com/MattPD/cpplinks/blob/master/debugging.md#linux).
...
[![](/blog/images/2017/rosetta-clip1.png)](http://bhami.com/rosetta.html)
...
I covered various kernel differences in my SCALE 2014 talk [What Linux can learn from Solaris performance and vice-versa](/Slides/SCaLE_Linux_vs_Solaris_Performance2014), and of course my book [Systems Performance: Enterprise and the Cloud](http://www.brendangregg.com/sysperfbook.html) where I cover both Linux and Solaris.
...
(Updated 12-Nov-2019) It's worth pointing out that most maintainers will only merge code of excellent quality. Patchsets are debated for weeks, months, and even years on lkml and other mailing lists. Many don't get merged. Many that do go through several versions, incorporating feedback each time. Some like to bash Linux by claiming that the Linux kernel "accepts any trash"; it doesn't, and you can see for yourself on the mailing lists. As a personal anecdote: my first Linux kernel patch was rejected. I was taking a shortcut and adding a tracepoint as a workaround instead of coding a proper solution. The maintainer demanded the proper solution, and was unwilling to merge a workaround.
...
[![](http://www.brendangregg.com/Images/suncobwebs_thumb.jpg)](http://www.brendangregg.com/Images/suncobwebs.jpg)
...
(Updated 12-Nov-2019) Kernel maintainer Steven Rostedt gave a great talk about Ftrace internals, and I posted about it: [Two kernel mysteries and the most technical talk I've ever seen](http://www.brendangregg.com/blog/2019-10-15/kernelrecipes-kernel-ftrace-internals.html). I recommend everyone from Solaris watch this talk, for the exposure to Linux kernel engineering by an actual maintainer (these talks are rare).
...
Update: there is a new effort to improve the kernel documentation (using RST and Sphinx), which you can see at [www.kernel.org/doc/html/latest](http://www.kernel.org/doc/html/latest).
...
The Linux job market has been much healthier for a while and growing. Solaris vs Linux jobs in the UK:

[![](/blog/images/2017/uk-solaris-market.png)](https://www.itjobswatch.co.uk/jobs/uk/solaris.do)[![](/blog/images/2017/uk-linux-market.png)](https://www.itjobswatch.co.uk/jobs/uk/linux.do)
...
[![](/blog/images/2017/indeed-solaris-linux-aws.png)](https://www.indeed.com/jobtrends/q-solaris-q-linux-q-AWS.html)
...
[![](/blog/images/2017/uk-aws-market.png)](https://www.itjobswatch.co.uk/jobs/uk/amazon%20aws.do)
...
I suspect at some point there'll be more jobs supporting the AWS cloud than there will be supporting Linux. If you choose this route, AWS makes it very easy to create servers (you just need a credit card) and learn how to use them. Which is also why it's been so successful: developers can create servers when they want them, without having to ask and wait for the system administration team. Adrian Cockcroft also wrote about moving from Solaris to AWS in his post [Open letter to my Sun friends at Oracle](https://medium.com/@adrianco/open-letter-to-my-sun-friends-at-oracle-updated-from-2010-post-1f8b2bcba693).
...
- [http://www.open-zfs.org/wiki/Main\_Page](http://www.open-zfs.org/wiki/Main_Page)
...
- [https://www.youtube.com/watch?v=GsMs3n8CB6g](https://www.youtube.com/watch?v=GsMs3n8CB6g)
...
- [https://github.com/iovisor/bcc/blob/master/tools/zfsdist\_example.txt](https://github.com/iovisor/bcc/blob/master/tools/zfsdist_example.txt)
- [https://github.com/iovisor/bcc](https://github.com/iovisor/bcc)
- [http://www.brendangregg.com/ebpf.html](http://www.brendangregg.com/ebpf.html)

Zones

- [https://blog.jessfraz.com/post/containers-zones-jails-vms/](https://blog.jessfraz.com/post/containers-zones-jails-vms/)
- [https://www.youtube.com/watch?v=bK9A5ODIgac](https://www.youtube.com/watch?v=bK9A5ODIgac)
- [https://www.brendangregg.com/Slides/DockerCon2017\_performance\_analysis](https://www.brendangregg.com/Slides/DockerCon2017_performance_analysis)
...
- [https://github.com/moby/moby/issues/32501](https://github.com/moby/moby/issues/32501)

Performance

- [https://www.brendangregg.com/Slides/SCaLE\_Linux\_vs\_Solaris\_Performance2014](https://www.brendangregg.com/Slides/SCaLE_Linux_vs_Solaris_Performance2014)
...
- [https://linux.die.net/man/5/auditd.conf](https://linux.die.net/man/5/auditd.conf)
...
- [http://www.brendangregg.com/blog/2016-08-09/gdb-example-ncurses.html](http://www.brendangregg.com/blog/2016-08-09/gdb-example-ncurses.html)
