...
url: https://www.brendangregg.com/blog/2016-10-27/dtrace-for-linux-2016.html
published: "2016-10-27T00:00:00Z"
feed: gregg
guid: https://www.brendangregg.com/blog/2016-10-27/dtrace-for-linux-2016.html
---
...
While BPF currently lacks a high-level language like DTrace, the front-ends available have been enough for me to create many BPF tools, some based on my older [DTraceToolkit](https://github.com/opendtrace/toolkit). In this post I'll describe how you can use these tools, the front-ends available, and discuss where the technology is going next.

## Screenshots

I've been adding BPF-based tracing tools to the open source [bcc](https://github.com/iovisor/bcc) project (thanks to Brenden Blanco, of PLUMgrid, for leading bcc development). See the [bcc install](https://github.com/iovisor/bcc/blob/master/INSTALL.md) instructions. It will add a collection of tools under /usr/share/bcc/tools, including the following.
...
Timed sampling was the final major piece, and it landed in Linux 4.9-rc1 ( [patchset](https://lkml.org/lkml/2016/9/1/831)). Many thanks to Alexei Starovoitov (now working on BPF at Facebook), the lead developer behind these BPF enhancements.
...
Now that BPF tracing has arrived, there's already tech companies on the lookout for BPF skills. I can still highly recommend [Netflix](http://www.brendangregg.com/blog/2016-03-30/working-at-netflix-2016.html). (If you're trying to hire *me* for BPF skills, then I'm still very happy at Netflix!.)
...
Wouldn't it be nice if we could have the SystemTap front-end with all its language integration and tapsets, with the high-performance kernel built in BPF back-end? Richard Henderson of Red Hat has already begun work on this, and has released an [initial version](https://lkml.org/lkml/2016/6/14/749)!

There's also [ply](https://wkz.github.io/ply/), an entirely new higher-level language for BPF:
...
- **Fix bcc issues**: See the [bcc issue list](https://github.com/iovisor/bcc/issues), which includes bugs and feature requests.
- **File bugs**: Use bcc/BPF, and file bugs as you find them.
- **New tools**: There are more observability tools to develop, but please don't be hasty: people are going to spend hours learning and using your tool, so make it as intuitive and excellent as possible (see my [docs](https://github.com/iovisor/bcc/blob/master/CONTRIBUTING-SCRIPTS.md)). As Mike Muuss has said about his [ping](http://ftp.arl.army.mil/%7Emike/ping.html) program: "If I'd known then that it would be my most famous accomplishment in life, I might have worked on it another day or two and added some more options."
...
I've written a [bcc/BPF Tool End-User Tutorial](https://github.com/iovisor/bcc/blob/master/docs/tutorial.md), a [bcc Python Developer's Tutorial](https://github.com/iovisor/bcc/blob/master/docs/tutorial_bcc_python_developer.md), a [bcc/BPF Reference Guide](https://github.com/iovisor/bcc/blob/master/docs/reference_guide.md), and contributed useful [/tools](https://github.com/iovisor/bcc/tree/master/tools), each with an [example.txt](https://github.com/iovisor/bcc/tree/master/tools) file and [man page](https://github.com/iovisor/bcc/tree/master/man/man8). My prior posts about bcc & BPF include:

- [eBPF: One Small Step](http://www.brendangregg.com/blog/2015-05-15/ebpf-one-small-step.html) (we later just called it BPF)
- [bcc: Taming Linux 4.3+ Tracing Superpowers](http://www.brendangregg.com/blog/2015-09-22/bcc-linux-4.3-tracing.html)
- [Linux eBPF Stack Trace Hack](http://www.brendangregg.com/blog/2016-01-18/ebpf-stack-trace-hack.html) (stack traces are now officially supported)
...
I've also giving a talk about bcc/BPF, at Facebook's Performance@Scale event: [Linux BPF Superpowers](http://www.brendangregg.com/blog/2016-03-05/linux-bpf-superpowers.html). In December, I'm giving a tutorial and talk on BPF/bcc at [USENIX LISA](https://www.usenix.org/conference/lisa16) in Boston.

(Update: I also have a new website, [Linux eBPF Tools](/ebpf.html).)
...
Finally, thanks to [Deirdré](/blog/2016-07-23/deirdre.html) for editing another post.
...
So when are you going to upgrade to Linux 4.9? Once it is officially released, new performance tools await: apt-get install bcc-tools. For updates on bcc/BPF tools, see [bcc (github)](https://github.com/iovisor/bcc) and my [eBPF Tools](/ebpf.html) page.
