...
feed: gregg
guid: https://www.brendangregg.com/blog/2021-09-26/the-speed-of-time.html
---
...
The Cassandra systems were EC2 virtual machine (Xen) instances. I logged into one and went through some basic CLI tools to get started (my [60s checklist](/Articles/Netflix_Linux_Perf_Analysis_60s.pdf)). Was there some other program consuming CPU, like a misbehaving Ubuntu service that wasn't in CentOS? top(1) showed that only the Cassandra database was consuming CPU.

What about short-lived processes, like a service restarting in a loop? These can be invisible to top(8). My execsnoop(8) tool (back then my [Ftrace version](https://github.com/brendangregg/perf-tools)) showed nothing.

It seemed that the extra CPU time really was in Cassandra, but how?

## 2\. CPU profile

Understanding a CPU time should be easy by comparing [CPU flame graphs](/FlameGraphs/cpuflamegraphs.html). Since instances of both CentOS and Ubuntu were running in parallel, I could collect flame graphs at the same time (same time-of-day traffic mix) and compare them side by side.

The CentOS flame graph:

[![](/blog/images/2021/flamegraph_time_centos.png)](/blog/images/2021/flamegraph_time_centos.svg)

The Ubuntu flame graph:

[![](/blog/images/2021/flamegraph_time_old.png)](/blog/images/2021/flamegraph_time_old.svg)
...
I love using [Linux performance tools](/linuxperf.html). But I also love solving issues quickly, and sometimes that means just asking colleagues or searching the Internet. I'm including this step as a reminder for anyone following this kind of analysis.
...
But as I summarized in my [What is Observability](/blog/2021-05-23/what-is-observability.html) post, the term observability can be a reminder not to get stuck on that one type of analysis. Here's some experimental approaches I could also explore:

3. Disable tracesys/syscall\_trace.
4. Microbenchmark os::javaTimeMillis() on both systems.
5. Try changing the kernel clocksource.
...
Ok, so it's defaulted to xen, which we saw in the flame graph (the tower ends with pvclock\_clocksource\_read()). Let's try tsc, which should be the fastest:
...
The change is immediate, and my Java microbenchmark is now running over 20x faster than before! (And nearly 4x faster than on CentOS.) Now that it's reaching 33 ns, the loop instructions are likely inflating this result. If I wanted more accuracy, I'd partially [unroll the loop](/blog/2014-04-26/the-noploop-cpu-benchmark.html) so that the loop instructions become negligible.
...
[![](/blog/images/2021/clocksource_tsc_annotated-red.png)](/blog/images/2021/clocksource_tsc-red.png)
...
[![](/blog/images/2021/flamegraph_time_new.png)](/blog/images/2021/flamegraph_time_new.svg)
...
Later that year (2014), Anthony Liguori from AWS gave a [re:Invent talk](https://youtu.be/ujGx0tiI1L4?t=2160) recommending users switch the clocksource to tsc to improve performance. I also shared setting the clocksource in my talks and in my 2015 [Linux tunables](/blog/2015-03-03/performance-tuning-linux-instances-on-ec2.html) post. Over the years, more and more articles have been published about clocksource in virtual machines, and it's now a well-known issue. Amazon even provides an official [recommendation](https://aws.amazon.com/premiumsupport/knowledge-center/manage-ec2-linux-clock-source/) (2021):
