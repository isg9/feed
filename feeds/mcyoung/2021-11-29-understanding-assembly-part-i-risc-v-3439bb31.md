---
title: 'Understanding Assembly Part I: RISC-V'
url: /2021/11/29/assembly-1
published: "2021-11-29T00:00:00Z"
feed: mcyoung
guid: /2021/11/29/assembly-1
---
...
> [tip](#tip:1)
>
> If you’re here to just learn assembly and don’t really care for motivation, you can just [skip ahead](#diving-in).
>
...
`addi a1, a0, 10` is a single line of assembly: it describes a *single* instruction in text form. Assembly language is “just” a textual representation of the program’s machine code. Your assembler can convert from text into machine instructions, and a *dis* assembler reverses the process.
...
### [On Architectures](\#on-architectures)
...
At this point, it may be worth looking at my [article on linkers](/2021/06/01/linker-script#seriously-whats-a-linker) as a refresher on the C compilation model.
...
[godbolt](https://godbolt.) [square.c](#code:square.c)
...
[godbolt](https://godbolt.) [Go](#code:3)
...
There’s a lot of terms below that I haven’t defined yet. I’ll break down what this code does gradually, so feel free to refer back to it as necessary, using [this handy-dandy link.](#big-example)
...
### [The Core Syntax](\#the-core-syntax)
...
>
> The usefulness of these names will be much more apparent when we discuss [the calling convention](#the-calling-convention), so feel free to come back to this later.
>
...
>
> The remaining registers belong to one of three categories: *argument* registers, *saved* registers, and *temporary* registers, named so for their role in calling a function (as described [below](#the-calling-convention)).
>
...
Multiplication and division are somewhat rarer, because they are expensive to implement in silicon; smaller devices don’t have them[3](#fn:3). Division in particular is very complex to implement in silicon. Instruction sets usually have different behavior around division by zero: some architectures will fault, similar to a memory error, while some, like RISC-V, produce a well-defined trap value.
...
Load/store instructions frequently take an offset for indexing into memory. `lw a1, 4(a0)` [4](#fn:4) is effectively `a1 = a0[4]`, treating `a0` like a pointer.
...
### [A Function Call in Slow-Mo](\#a-function-call-in-slow-mo)
...
Arguments in the usual[5](#fn:5) RISC-V calling convention, word-sized arguments are passed in the `a0` through `a7` registers, falling back to passing on the stack if they run out of space. If the argument is too big to fit in a register, it gets passed by reference instead. Arguments that fit into two registers can be split across registers.

We can see this in action [above](#big-example). The first argument, a string, is passed by pointer in `a0`; `lui` and `addi` do the work of actually putting that pointer into `a0`. The second argument `x` is passed in `a1`, copied from `s0` where it landed from the earlier `mul` instruction.

Complex function signatures require much more[6](#fn:6) work to set up.
...
[godbolt](https://godbolt.org/clientstate/eyJzZXNzaW9ucyI6W3siY29tcGlsZXJzIjpbeyJpZCI6InJ2MzItY2NsYW5nIiwib3B0aW9ucyI6Ii1PeiJ9XSwiaWQiOjEsImxhbmd1YWdlIjoiYyIsInNvdXJjZSI6ImludCBzcXVhcmUoaW50IHgpIHtcbiAgcmV0dXJuIHggKiB4O1xufVxuIn1dfQ==) [C](#code:9)
...
1. This is a hilarious lie that is beyond the scope of this post. See, for example, [https://en.wikipedia.org/wiki/Superscalar\_processor](https://en.wikipedia.org/wiki/Superscalar_processor). [↩︎](#fnref:1)
...
This is not true of all architectures, so beware. [↩︎](#fnref:2)

3. Thankfully, these can be polyfilled using the previous ubiquitous instructions. Hacker’s Delight contains all of the relevant algorithms, so I won’t reproduce them here. The division polyfills are particularly interesting. [↩︎](#fnref:3)

4. It’s a bit interesting that we don’t write `lw a1, a0[4]` in imitation of array syntax. This specific corner of the notation is shockingly diverse across assemblers: in ARM, we write `ldr r0, [r1, #offset]`; in x86, `mov rax, [rdx + offset]`, or `movq offset(%rdx), %rax` for AT&T-flavored assemblers (which is surprisingly similar to the RISC-V syntax!); in 6502, `lda ($1234, X)`. [↩︎](#fnref:4)
...
RISC-V defines a recommended calling convention for ELF-based targets: [https://github.com/riscv-non-isa/riscv-elf-psabi-doc](https://github.com/riscv-non-isa/riscv-elf-psabi-doc). [↩︎](#fnref:5)
...
[godbolt](https://godbolt.) [C](#code:11)
...
[godbolt](https://godbolt.) [C](#code:13)
