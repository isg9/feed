...
feed: nullprogram
guid: https://nullprogram.com/blog/2020/11/17/
---
...
*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=25120083).*

[Pixelmusement](https://www.pixelships.com/) produces videos about [MS-DOS games](/blog/2020/10/19/) and software. Each video ends with a short, randomly-selected listing of financial backers. In [ADG Filler #57](https://www.youtube.com/watch?v=YVV9bkbpaPY), Kris revealed the selection process, and it absolutely fits the channel’s core theme: a [QBasic](https://en.wikipedia.org/wiki/QBasic) program. His program relies on QBasic’s built-in pseudo random number generator (PRNG). Even accounting for the platform’s limitations, the PRNG is much poorer quality than it could be. Let’s discuss these weaknesses and figure out how to make the selection more fair.
...
QBasic’s PRNG is this 24-bit [Linear Congruential Generator](https://en.wikipedia.org/wiki/Linear_congruential_generator) (LCG):
...
*reasonable*](/blog/2019/11/19/), but the choice to limit the state to 24 bits is strange. According to the [QBasic 16-bit assembly](https://www.qb64.org/forum/index.php?topic=1414.0) (note: the LCG constants listed here [are wrong](http://www.qb64.net/forum/index_topic_10727-0/)), the implementation is a full 32-bit multiply using 16-bit limbs, and it allocates and writes a full 32 bits when storing the state. As expected for the 8086, there was nothing gained by using only the lower 24 bits.

To illustrate how poor it is, here’s a [randogram](https://www.pcg-random.org/posts/visualizing-the-heart-of-some-prngs.html) for this PRNG, which shows obvious structure. (This is a small slice of a 4096x4096 randogram where each of the 2^23 24-bit samples is plotted as two 12-bit coordinates.)

![](/img/qbasic/rnd-thumb.png)

Admittedly this far [*overtaxes*](https://www.pcg-random.org/paper.html) the PRNG. With a 24-bit state, it’s only good for 4,096 (2^12) outputs, after which it no longer follows the [birthday paradox](/blog/2019/07/22/): No outputs are repeated even though we should start seeing some. However, as I’ll soon show, this doesn’t actually matter.
...
The [`TIMER` function](https://www.qb64.org/wiki/TIMER) returns the single precision number of seconds since midnight with ~55ms precision (i.e. the 18.2Hz timer interrupt counter). This is strictly time of day, and the current date is not part of the result, unlike, say, the unix epoch.
...
Since iterations fly by quickly, the exact number of iterations becomes another [source of entropy](/blog/2019/04/30/). The results will be quite different if it runs 214,600 iterations versus 273,500 iterations.
...
Since the built-in PRNG is so poor, we could improve the situation by implementing a [new one](/blog/2017/09/21/) in QBasic itself. The challenge is that QBasic has no unsigned integers, not even unsigned integer operators (i.e. Java and JavaScript’s `>>>`), and signed overflow is a run-time error. We can’t even re-implement QBasic’s own LCG without doing long multiplication in software, since the intermediate result overflows its 32-bit `LONG`.

Popular choices in these constraints are [Park–Miller generator](https://en.wikipedia.org/wiki/Lehmer_random_number_generator) (as we saw [in Bash](/blog/2018/12/25/)) or a [lagged Fibonacci generator](https://en.wikipedia.org/wiki/Lagged_Fibonacci_generator) (as used by Emacs, which was for a long time constrained to 29-bit integers).

However, I have a better idea: a PRNG based on [RC4](https://en.wikipedia.org/wiki/RC4). Specifically, my own design called [**Sponge4**](https://github.com/skeeto/scratch/tree/master/sp4), a [sponge construction](https://en.wikipedia.org/wiki/Sponge_function) built atop RC4. In short: Mixing in more input is just a matter of running the key schedule again. Implementing this PRNG requires just two simple operations: modular addition over 2^8, and array swap. QBasic has a `SWAP` statement, so it’s a natural fit!
...
If you want to try it out for yourself in, say, DOSBox, here’s the full source: [**`sponge4.bas`**](https://github.com/skeeto/scratch/blob/master/sp4/sponge4.bas)

- [c](/tags/c/)
- [optimization](/tags/optimization/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Improving%20on%20QBasic's%20Random%20Number%20Generator) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Improving+on+QBasic%27s+Random+Number+Generator).
