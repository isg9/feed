...
url: https://nullprogram.com/blog/2016/04/30/
published: "2016-04-30T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2016/04/30/
---

# Makefile Assignments are Turing-Complete

## [Makefile Assignments are Turing-Complete](/blog/2016/04/30/)

April 30, 2016

nullprogram.com/blog/2016/04/30/

For over a decade now, GNU Make has almost exclusively been my build system of choice, either directly or indirectly. Unfortunately this means I unnecessarily depend on some GNU extensions — an annoyance when porting to the BSDs. In an effort to increase the portability of my Makefiles, I recently read [the POSIX make specification](http://pubs.opengroup.org/onlinepubs/9699919799/utilities/make.html). I learned two important things: 1) POSIX make is so barren it’s not really worth striving for ( *update*: I’ve [changed my mind](/blog/2017/08/20/)), and 2) **make’s macro assignment mechanism is Turing-complete**.
...
- [**life.mak**](/download/life.mak) (174kB) \[ [or generate your own](https://github.com/skeeto/makefile-game-of-life)\]
...
This last case had me thinking: This is very similar to the (ab)use of the x86 `mov` instruction in [mov is Turing-complete](https://www.cl.cam.ac.uk/~sd601/papers/mov.pdf). These macro assignments alone should be enough to compute *any* algorithm.
...
creativity](/blog/2008/03/15/). For example, integers could be represented as a series of x characters. The number 4 is `xxxx`, the number 6 is `xxxxxx`, etc. Addition is concatenation (note: macros can have `+` in their names):
...
Looping is a tricky problem. However, one of the most common build ( [anti](http://aegis.sourceforge.net/auug97.pdf)?)patterns is the recursive Makefile. Borrowing from the `mov` paper, which used an unconditional jump to restart the program from the beginning, for a Makefile Turing-completeness I can invoke the Makefile recursively, restarting the program with a new set of inputs.
...
The last line will be used to halt by clearing the name on the right side. This is [three star](http://c2.com/cgi/wiki?ThreeStarProgrammer) territory.
...
And that’s all there is to it: Conway’s Game of Life running in a Makefile. [Life, uh, finds a way](https://www.youtube.com/watch?v=dMjQ3hA9mEA).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Makefile%20Assignments%20are%20Turing-Complete) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Makefile+Assignments+are+Turing-Complete).

« [Mapping Multiple Memory Views in User Space](/blog/2016/04/10/)
