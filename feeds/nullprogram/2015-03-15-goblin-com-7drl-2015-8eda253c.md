...
url: https://nullprogram.com/blog/2015/03/15/
published: "2015-03-15T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2015/03/15/
---
...
nullprogram.com/blog/2015/03/15/

Yesterday I completed my third entry to the annual Seven Day Roguelike (7DRL) challenge (previously: [2013](/blog/2013/03/17/) and [2014](/blog/2014/03/31/)). This year’s entry is called **Goblin-COM**.
...
- [Video review](https://www.youtube.com/watch?v=QW3Uul7-Iss) by Akhier Dragonheart

As with previous years, the ideas behind the game are not all that original. The goal was to be a fantasy version of [classic
X-COM](http://en.wikipedia.org/wiki/UFO:_Enemy_Unknown) with an ANSI terminal interface. You are the ruler of a fledgling human nation that is under attack by invading goblins. You hire heroes, operate squads, construct buildings, and manage resource income.

The inspiration this year came from watching [BattleBunny](https://www.youtube.com/playlist?list=PL2xITSnTC0YkB2-B8fs-02YVT81AE0WtP) play [OpenXCOM](http://openxcom.org/), an open source clone of the original X-COM. It had its major 1.0 release last year. Like the early days of [OpenTTD](https://www.openttd.org/en/), it currently depends on the original game assets. But also like OpenTTD, it surpasses the original game in every way, so there’s no reason to bother running the original anymore. I’ve also recently been watching [One F Jef play Silent Storm](https://youtu.be/bwPLKud0rP4), which is another turn-based squad game with a similar combat simulation.
...
I **didn’t make use of ncurses**, instead opting to write my own terminal graphics engine. That’s because I wanted a [single, small
binary](/blog/2014/12/09/) that was easy to build, and I didn’t want to mess around with [PDCurses](http://pdcurses.sourceforge.net/). I’ve also been studying the Win32 API lately, so writing my own terminal platform layer would rather easy to do anyway.
...
To save time, and because it doesn’t really matter, saves are just memory dumps. I took another page from [Handmade Hero](http://handmadehero.org/) and allocate everything in a single, contiguous block of memory. With one exception, there are no pointers, so the entire block is relocatable. When references are needed, it’s done via integers into the embedded arrays. This allows it to be cleanly reloaded in another process later. As a side effect, it also means there are no dynamic allocations ( `malloc()`) while the game is running. Here’s roughly what it looks like.
...
The data isn’t important enough to bother with [rename+fsync](http://lwn.net/Articles/322823/) durability. I’ll risk the data if it makes savescumming that much harder!
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Goblin-COM%207DRL%202015) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Goblin-COM+7DRL+2015).
