...
url: https://nullprogram.com/blog/2014/10/12/
published: "2014-10-12T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2014/10/12/
---
...
nullprogram.com/blog/2014/10/12/
...
The [BSD regular expression API](http://man7.org/linux/man-pages/man3/re_comp.3.html) dates back to 4.3BSD, released in 1986. It’s just a pair of functions: one compiles the regex, the other executes it on a string.
...
This API has been deprecated for some time now, so hopefully no one’s using it anymore. 15 years after the BSD regex API came out, POSIX standardized [a much better API](http://man7.org/linux/man-pages/man3/regcomp.3.html). It operates on an opaque `regex_t` object, on which all state is stored. There’s no global state.
...
This API eliminates the global state and, as a result, solves *all* of the problems listed above. It’s essentially the same API defined by [Popt](http://linux.die.net/man/3/popt) and my own embeddable [Optparse](https://github.com/skeeto/optparse). They’re much better options if the limitations of POSIX-style Getopt are an issue.

- [c](/tags/c/)
- [posix](/tags/posix/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Global%20State:%20a%20Tale%20of%20Two%20Bad%20C%20APIs) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Global+State%3A+a+Tale+of+Two+Bad+C+APIs).
