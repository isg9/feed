...
url: https://nullprogram.com/blog/2021/06/29/
published: "2021-06-29T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2021/06/29/
---
...
nullprogram.com/blog/2021/06/29/

My previous article explained [how to work with dynamic-link libraries
(DLLs) using w64devkit](/blog/2021/05/31/). These techniques also apply to other circumstances, including with languages and ecosystems outside of C and C++. In particular, [w64devkit](/blog/2020/05/15/) is a great complement to Go and reliably fullfills all the needs of [cgo](https://golang.org/cmd/cgo/) — Go’s C interop — and can even bootstrap Go itself. As before, this article is in large part an exercise in capturing practical information I’ve picked up over time.

### Go: bootstrap and cgo

The primary Go implementation, confusingly [named “gc”](https://golang.org/doc/faq#What_compiler_technology_is_used_to_build_the_compilers), is an [incredible piece of software engineering](/blog/2020/01/21/). This is apparent when building the Go toolchain itself, a process that is fast, reliable, easy, and simple. It was originally written in C, but was re-written in Go starting with Go 1.5. The C compiler in w64devkit can build the original C implementation which then can be used to bootstrap any more recent version. It’s so easy that I personally never use official binary releases and always bootstrap from source.

You will need the Go 1.4 source, [go1.4-bootstrap-20171003.tar.gz](https://dl.google.com/go/go1.4-bootstrap-20171003.tar.gz). This “bootstrap” tarball is the last Go 1.4 release plus a few additional bugfixes. You will also need the source of the actual version of Go you want to use, such as Go 1.16.5 (latest version as of this writing).

Start by building Go 1.4 using w64devkit. On Windows, Go is built using a batch script and no special build system is needed. Since it shouldn’t be invoked with the BusyBox ash shell, I use [`cmd.exe`](/blog/2021/02/08/) explicitly.
...
costs](https://dave.cheney.net/2016/01/18/cgo-is-not-go) if used.)
...
w64devkit includes two assemblers: GAS (Binutils) which is used by GCC, and NASM which has [friendlier syntax](https://elronnd.net/writ/2021-02-13_att-asm.html). I prefer the latter whenever possible — exactly why I included NASM in the distribution. So here’s how I implemented “version 3” in NASM assembly.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20More%20DLL%20fun%20with%20w64devkit:%20Go,%20assembly,%20and%20Python) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=More+DLL+fun+with+w64devkit%3A+Go%2C+assembly%2C+and+Python).
