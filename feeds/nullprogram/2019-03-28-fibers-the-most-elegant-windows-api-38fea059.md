...
url: https://nullprogram.com/blog/2019/03/28/
published: "2019-03-28T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2019/03/28/
---
...
*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=19520078).*
...
That’s why, when I recently revisited the [Fibers API](https://docs.microsoft.com/en-us/windows/desktop/procthread/fibers), I was pleasantly surprised. It’s one of the exceptions — much cleaner than the optional, deprecated, and now obsolete [POSIX equivalent](/blog/2017/06/21/#coroutines). It’s not quite an apples-to-apples comparison since the POSIX version is slightly more powerful, and more complicated as a result. I’ll cover the difference in this article.
...
[**Fiber Async/await Demo**](https://github.com/skeeto/fiber-await)

### Fibers

Windows fibers are really just [stackful](https://blog.varunramesh.net/posts/stackless-vs-stackful-coroutines/), symmetric coroutines. From a different point of view, they’re cooperatively scheduled threads, which is the source of the analogous name, *fibers*. They’re symmetric because all fibers are equal, and no fiber is the “main” fiber. If *any* fiber returns from its start routine, the program exits. (Older versions of Wine will crash when this happens, but it was recently fixed.) It’s equivalent to the process’ main thread returning from `main()`. The initial fiber is free to create a second fiber, yield to it, then the second fiber destroys the first.
...
- [CreateFiber](https://docs.microsoft.com/en-us/windows/desktop/api/WinBase/nf-winbase-createfiber)
- [SwitchToFiber](https://docs.microsoft.com/en-us/windows/desktop/api/WinBase/nf-winbase-switchtofiber)
- [ConvertFiberToThread](https://docs.microsoft.com/en-us/windows/desktop/api/winbase/nf-winbase-convertfibertothread)
- [ConvertThreadToFiber](https://docs.microsoft.com/en-us/windows/desktop/api/WinBase/nf-winbase-convertthreadtofiber)
- [DeleteFiber](https://docs.microsoft.com/en-us/windows/desktop/api/WinBase/nf-winbase-deletefiber)
...
The equivalent POSIX systems was context switching. It’s also stackful and symmetric, but it has just three important functions: [`getcontext(3)`](http://man7.org/linux/man-pages/man3/setcontext.3.html), [`makecontext(3)`](http://man7.org/linux/man-pages/man3/makecontext.3.html), and [`swapcontext`](http://man7.org/linux/man-pages/man3/makecontext.3.html).
...
These are roughly equivalent to [`GetCurrentFiber()`](https://docs.microsoft.com/en-us/windows/desktop/api/winnt/nf-winnt-getcurrentfiber), `CreateFiber()`, and `SwitchToFiber()`. There is no need for `ConvertFiberToThread()` since threads can context switch without preparation. There’s also no `DeleteFiber()` because the resources are managed by the program itself. That’s where POSIX contexts are a little bit more powerful.
...
Why was I looking at the Fiber API? I’ve known about coroutines for years but I didn’t understand how they could be useful. Sure, the function can yield, but what other coroutine should it yield to? It wasn’t until I was [recently bit by the async/await bug](/blog/2019/03/10/) that I finally saw a “killer feature” that justified their use. Generators come pretty close, though.

Windows fibers are a coroutine primitive suitable for async/await in C programs, where [it can also be useful](/blog/2019/03/22/). To prove that it’s possible, I built async/await on top of fibers in [95 lines of code](https://github.com/skeeto/fiber-await/blob/master/async.c).

The alternatives are to use a [third-party coroutine library](https://www.gnu.org/software/pth/) or to do it myself [with some assembly programming](/blog/2015/05/15/). However, having it built into the operating system is quite convenient! It’s unfortunate that it’s limited to Windows. Ironically, though, everything I wrote for this article, including the async/await demonstration, was originally written on Linux using Mingw-w64 and tested using [Wine](https://www.winehq.org/). Only after I was done did I even try it on Windows.

Before diving into how it works, there’s a general concept about the Windows API that must be understood: **All kernel objects can be in** **either a signaled or unsignaled state.** The API provides functions that block on a kernel object until it is signaled. The two important ones are [`WaitForSingleObject()`](https://docs.microsoft.com/en-us/windows/desktop/api/synchapi/nf-synchapi-waitforsingleobject) and [`WaitForMultipleObjects()`](https://docs.microsoft.com/en-us/windows/desktop/api/synchapi/nf-synchapi-waitformultipleobjects). The latter behaves very much like `poll(2)` in POSIX.
...
files](https://blog.libtorrent.org/2012/10/asynchronous-disk-io/), and many operations can’t be done asynchronously, like opening files. When it comes to files, you’re [better off using
dedicated threads](https://blog.libtorrent.org/2012/10/asynchronous-disk-io/) as [libuv does](http://docs.libuv.org/en/v1.x/design.html#file-i-o) instead of overlapped I/O. You can still await on these operations. You’d just await on the signal from the thread doing synchronous I/O, not from overlapped I/O.
...
particular problem well](https://news.ycombinator.com/item?id=11866562) — awaiting on kernel objects — so I couldn’t use it.
...
That’s all there is to it. Have a look at [`demo.c`](https://github.com/skeeto/fiber-await/blob/master/demo.c) to see how the API looks in some trivial examples. On Linux you can see it in action with `make check`. On Windows, you just [need to compile
it](/blog/2016/06/13/), then run it like a normal program. If there was a better function than `WaitForMultipleObjects()` in the Windows API, I would have considered turning this demonstration into a real library.

- [win32](/tags/win32/)
- [c](/tags/c/)
- [posix](/tags/posix/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Fibers:%20the%20Most%20Elegant%20Windows%20API) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Fibers%3A+the+Most+Elegant+Windows+API).

« [Endlessh: an SSH Tarpit](/blog/2019/03/22/)
