...
url: https://nullprogram.com/blog/2022/12/18/
published: "2022-12-18T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2022/12/18/
---
...
nullprogram.com/blog/2022/12/18/

*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=34035024).*

The [Quite OK Image (QOI) format](https://qoiformat.org/) was announced late last year and finalized into a specification a month later. Initially dismissive, a revisit has shifted my opinion to impressed. The format hits a sweet spot in the trade-off space between complexity, speed, and compression ratio. Also considering its alpha channel support, QOI has become my default choice for embedded image assets. It’s not perfect, but at the very least it’s a solid foundation.

Since I’m now working with QOI images, I need a good QOI viewer, and so I added support to my ill-named [pbmview](https://github.com/skeeto/scratch/tree/master/pbmview) tool, which I wrote to serve the same purpose for [Netpbm](https://netpbm.sourceforge.net/doc/ppm.html). I will [continue to use Netpbm
as an output format](/blog/2020/06/29/), especially for raw video output, but no longer will I use it for an embedded asset (nor re-invent yet another [RLE](https://en.wikipedia.org/wiki/Run-length_encoding) over Netpbm).
...
However, as I said, QOI’s strength is its trade-off sweet spot. The [specification is one page](https://qoiformat.org/qoi-specification.pdf), and an experienced developer can write a complete implementation from scratch in a single sitting. [My own
implementation is about 100 lines of libc-free C](https://github.com/skeeto/scratch/blob/master/parsers/qoi.c) for each of the encoder and decoder. With error checking removed, my decoder is ~600 bytes of x86 object code — a great story for embedding alongside assets. It’s more complex than Netpbm or [farbfeld](https://tools.suckless.org/farbfeld/), but it’s far simpler than BMP. I’ve already begun [experimenting with converting assets to QOI](https://github.com/skeeto/chess/commit/5c123b3), and the results have so far exceeded my expectations.
...
- The QOI hash function operates on channels individually, with individual overflow, making it slower and larger than necessary. The hash function should have been [over a packed 32-bit sample](/blog/2018/07/31/). I would have used [a multiplication](/blog/2022/08/08/#hash-functions) by a carefully-chosen 32-bit integer, then a right shift using the highest 6 bits of the result for the index.

More subjective criticisms that might count as having trade-offs:

- Given a “flag byte” (mentioned above) it would have been free to assign another flag bit indicating pre-multiplied alpha, also still advisory. [You want to use pre-multiplied alpha](https://www.adriancourreges.com/blog/2017/05/09/beware-of-transparent-pixels/) for your assets, and the option store them this way would help.
...
I’m really happy with my QOI implementation, particularly since it’s another example of [a minimalist C API](/blog/2018/06/10/): no allocating, no input or output, and no standard library use. As usual, the expectation is that it’s in the same translation unit where it’s used, so it’s likely inlined into callers.
...
To make it easier to use, my decoder range checks to guarantee that width and height [can be multiplied without overflow](/blog/2017/07/19/). Unlike encoding, there may be errors due to invalid input, including that failed range check. The decoder error flag is “sticky” and the decoder returns zero samples when in an error state, so callers can wait to check for errors until the end. (Though if you’re only decoding embedded assets, then there are no practical errors, and checks can be removed/ignored.)
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20QOI%20is%20now%20my%20favorite%20asset%20format) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=QOI+is+now+my+favorite+asset+format).

« [I solved the Dandelions paper-and-pencil game](/blog/2022/10/12/)
