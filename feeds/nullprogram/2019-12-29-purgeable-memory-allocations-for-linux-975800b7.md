...
url: https://nullprogram.com/blog/2019/12/29/
published: "2019-12-29T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2019/12/29/
---
...
nullprogram.com/blog/2019/12/29/

I saw (part of) a video, [OS hacking: Purgeable memory](https://www.youtube.com/watch?v=9l0nWEUpg7s), by Andreas Kling who’s writing an operating system called [Serenity](https://github.com/SerenityOS/serenity) and recording videos his progress. In the video he implements *purgeable memory* as [found on some Apple platforms](https://developer.apple.com/library/archive/documentation/Performance/Conceptual/ManagingMemory/Articles/CachingandPurgeableMemory.html) by adding special support in the kernel. A process tells the kernel that a particular range of memory isn’t important, and so the kernel can reclaim if it the system is under memory pressure — the memory is purgeable.

Linux has a mechanism like this, [`madvise(2)`](http://man7.org/linux/man-pages/man2/madvise.2.html), that allows processes to provide hints to the kernel on how memory is expected to be used. The flag of interest is `MADV_FREE`:
...
It [allocates anonymous pages](/blog/2018/11/15/) using `mmap(2)`. When the allocation is “unlocked” — i.e. the process isn’t actively using it — its pages are marked with `MADV_FREE` so that the kernel can reclaim them at any time. To lock the allocation so that the process can safely make use of them, the `MADV_FREE` is canceled. This is all a little trickier than it sounds, and that’s the subject of this article.

Note: There’s also `MADV_DONTNEED` which seems like it would fit the bill, but [it’s implemented incorrectly in Linux](https://www.youtube.com/watch?v=bg6-LVCHmGM#t=58m23s). It *immediately* frees the pages, and so it’s useless for implementing purgeable memory.

### Purgeable API

Before diving into the implementation, here’s the API. It’s [just four
functions](/blog/2018/06/10/) with no structure definitions. The pointer used by the API is the memory allocation itself. All the bookkeeping [associated
with that pointer](/blog/2017/01/08/) is hidden away, out of sight from the API’s consumer. The full documentation is in [`purgeable.h`](https://github.com/skeeto/purgeable/blob/master/purgeable.h).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Purgeable%20Memory%20Allocations%20for%20Linux) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Purgeable+Memory+Allocations+for+Linux).

« [Unintuitive JSON Parsing](/blog/2019/12/28/)
