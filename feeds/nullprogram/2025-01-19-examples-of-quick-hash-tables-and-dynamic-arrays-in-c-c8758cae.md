...
url: https://nullprogram.com/blog/2025/01/19/
published: "2025-01-19T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2025/01/19/
---
...
This article durably captures [my reddit comment](https://old.reddit.com/r/C_Programming/comments/1hrvhfl/_/m51saq2/) showing techniques for `std::unordered_map` and `std::vector` equivalents in C programs. The core, important features of these data structures require only a dozen or so lines of code apiece. They compile quickly, and tend to run faster in debug builds than *release builds* of their C++ equivalents. What they lack in genericity they compensate in simplicity. Nothing here will be new. Everything has been covered in greater detail previously, which I will reference when appropriate.
...
The foundation is our allocator, a simple [bump allocator](/blog/2023/09/27/), so we’ll start there:
...
Allocating through the `new` macro eliminates several classes of common defects in C programs. If we get our types mixed up we get errors, or at least warnings. Our [size calculations cannot overflow](/blog/2024/05/24/). We cannot accidentally use uninitialized memory. We cannot leak memory; deallocating is implicit. The main downside is that it doesn’t fit some less common allocator requirements.

### Strings

Next, a string representation. Classic [null-terminated strings are an
error-prone paradigm](https://www.symas.com/post/the-sad-state-of-c-strings), so we’ll use [counted strings](/blog/2024/04/14/) instead:
...
forbids null pointers](/blog/2023/02/11/#strings), and we may be passed a zero-initialized `Str`. Though [this is scheduled to be corrected](https://developers.redhat.com/articles/2024/12/11/making-memcpynull-null-0-well-defined).
...
We have a couple string-to-string map options. The more restrictive, but more efficient — in terms of memory use and speed — is a [Mask-Step-Index
(MSI) hash table](/blog/2022/08/08/). I don’t think it fits our problem as well as the next option, particularly because it puts a hard limit on unique keys, but it’s worth evaluating. Let’s call it `FlatEnv`:
...
If the number of keys is unbounded, [hash tries](/blog/2023/09/30/) work better. Trees scale well, and we can allocate nodes out of the arena as it grows. We’ll use a 4-ary trie, a good default that balances size and performance:
...
Next I’d like a function that takes an `Env` and produces an `envp` data structure as expected by [`execve(2)`](https://man7.org/linux/man-pages/man2/execve.2.html). Then we can use this map as the environment in a child process. We’ll need some string manipulation, particularly [string concatenation](/blog/2024/05/25/). The core is a copy function:
...
To implement the `envp` conversion for the hash trie `Env`, let’s add one more tool to our toolbox: dynamic arrays. Our `std::vector` equivalent. We’ll start with [a familiar slice header](/blog/2023/10/05/):
...
( **Update**: Aleh pointed out an inefficiency in the original code: [applying alignment in the second `alloc` may introduce unnecessary
fragmentation](https://lists.sr.ht/~skeeto/public-inbox/%3CCAB2_dQWNOKCSCa8L8khH2W0eunsKK-_CkJZaDUpRAA4AFMG8Jg@mail.gmail.com%3E). This has been corrected above.)
...
**Update**: [NRK points out](https://old.reddit.com/r/C_Programming/comments/1i74hii/_/m8l40fo/) we can use `__typeof__` (extension) or `typeof` (C23), to work around this syntactical limitation of `_Alignof`. Convert the `align` local variable into a parameter:
...
The `init` array is a form of [small-size optimization](/blog/2016/10/07/). It’s used at first, and sufficient for nearly all inputs. So no stack litter in the arena. If it’s not enough, then `push` will *automatically move the stack* *into the arena*. I think that’s a super duper neato trick!
...
Source from this article in runnable form, which I used to test my samples: [`example.c`](https://gist.github.com/skeeto/42d8a23871642696b6b8de30d9222328)

- [c](/tags/c/)
- [tutorial](/tags/tutorial/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Examples%20of%20quick%20hash%20tables%20and%20dynamic%20arrays%20in%20C) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Examples+of+quick+hash+tables+and+dynamic+arrays+in+C).

« [Rules to avoid common extended inline assembly mistakes](/blog/2024/12/20/)
