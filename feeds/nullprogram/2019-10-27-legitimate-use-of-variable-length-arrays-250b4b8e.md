...
url: https://nullprogram.com/blog/2019/10/27/
published: "2019-10-27T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2019/10/27/
---
...
nullprogram.com/blog/2019/10/27/

*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=21375580) and [on reddit](https://old.reddit.com/r/programming/comments/dz1fau/variable_length_arrays_in_c_are_nearly_always_the/)*.
...
Then small inputs are fast, but large inputs still work correctly. This is called [**small size optimization**](/blog/2016/10/07/).

If the correct solution ultimately didn’t use a VLA, then what good are they? In general, VLAs not useful. They’re [time bombs](https://www.phoronix.com/scan.php?page=news_item&px=Linux-Kills-The-VLA). **VLAs** **are nearly always the wrong choice.** You must be careul to check that they don’t exceed some safe maximum, and there’s no reason not to always use the maximum. This problem was realized for the C11 standard (ISO/IEC 9899:2011) where VLAs were made optional. A program containing a VLA will not necessarily compile on a C11 compiler.
...
**Update**: [bru del pointed out](https://lists.sr.ht/~skeeto/public-inbox/%3CCAP-ht1CQKVByZt1EXOb3J7TF%3DMcCKi%3DEtzjEH+CaEsPtvY5%3Djg%40mail.gmail.com%3E) that these multi-dimensional VLAs can be simplified such that the parenthesis may be omitted when indexing. The trick is to omit the first dimension from the VLA expression:
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Legitimate%20Use%20of%20Variable%20Length%20Arrays) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Legitimate+Use+of+Variable+Length+Arrays).
