...
feed: nullprogram
guid: https://nullprogram.com/blog/2016/08/03/
---

# Appending to a File from Multiple Processes

## [Appending to a File from Multiple Processes](/blog/2016/08/03/)

August 03, 2016

nullprogram.com/blog/2016/08/03/
...
The first concern is solved by the operating system, with one caveat. On POSIX systems, opening a file with the `O_APPEND` flag will guarantee that [writes always safely append](http://pubs.opengroup.org/onlinepubs/9699919799/functions/write.html).
...
`O_APPEND`](http://pubs.opengroup.org/onlinepubs/9699919799/functions/fopen.html), so you don’t necessarily need to use open(2). On Linux this can be verified for any language’s standard library with strace.
...
For Win32, the equivalent is the `FILE_APPEND_DATA` access right, and similarly [only applies to “local files.”](https://msdn.microsoft.com/en-us/library/windows/desktop/gg258116(v=vs.85).aspx)
...
The minimum value for `PIPE_BUF` for POSIX systems is 512 bytes. On Linux it’s 4kB, and on other systems [it’s as high as 32kB](http://ar.to/notes/posix). As long as each record is less than 512 bytes, a simple write(2) will due. None of this depends on a filesystem since no files are involved.

If more than `PIPE_BUF` bytes isn’t enough, the POSIX writev(2) can be used to [atomically write up to `IOV_MAX` buffers](http://pubs.opengroup.org/onlinepubs/9699919799/functions/writev.html) of `PIPE_BUF` bytes. The minimum value for `IOV_MAX` is 16, but is typically 1024. This means the maximum safe atomic write size for pipes — and therefore the largest record size — for a perfectly portable program is 8kB (16✕512). On Linux it’s 4MB.
...
records may contain new line characters](https://tools.ietf.org/html/rfc4180), there are two approaches. In each, the record must still be no larger than `PIPE_BUF` bytes.
...
On Win32, [anonymous pipes](https://msdn.microsoft.com/en-us/library/windows/desktop/aa365152(v=vs.85).aspx) have no semantics regarding interleaving. [Named pipes](https://msdn.microsoft.com/en-us/library/windows/desktop/aa365150(v=vs.85).aspx) have per-client buffers that prevent interleaving. However, the pipe buffer size is unspecified, and requesting a particular size is only advisory, so it comes down to trial and error, though the unstated limits should be comparatively generous.

### Interleaving and Files

Suppose instead of a pipe we have an `O_APPEND` file on POSIX. Common wisdom states that the same `PIPE_BUF` atomic write rule applies. While this often works, especially on Linux, this is not correct. The POSIX specification doesn’t require it and [there are systems where it
doesn’t work](http://www.notthewizard.com/2014/06/17/are-files-appends-really-atomic/).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Appending%20to%20a%20File%20from%20Multiple%20Processes) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Appending+to+a+File+from+Multiple+Processes).

« [Const and Optimization in C](/blog/2016/07/25/)
