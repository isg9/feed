...
url: https://nullprogram.com/blog/2019/06/30/
published: "2019-06-30T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2019/06/30/
---
...
nullprogram.com/blog/2019/06/30/

*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=20321116).*
...
The necessary information describing the buffer is split across two arguments. That can become tedious, and it’s also a source of serious bugs if the two parameters aren’t in agreement (buffer overflow, [information disclosure](/blog/2017/07/19/), etc.). Wouldn’t it be nice if this information was packed into the pointer itself? That’s essentially the definition of a *fat pointer*.

### Fat pointers via bit hacks

If we assume some things about the target platform, we can encode fat pointers inside a plain pointer with [some dirty pointer
tricks](/blog/2016/05/30/), exploiting unused bits in the pointer value. For example, currently on x86-64, only the lower 48 bits of a pointer are actually used. The other 16 bits could carefully be used for other information, like communicating the number of elements or bytes:
...
The other side can unpack this to get the components back out. Obviously 16 bits for the count will often be insufficient, so this would more likely be used for [baggy bounds checks](https://www.usenix.org/legacy/event/sec09/tech/full_papers/akritidis.pdf).
...
In typical C implementations, the structure fields would be passed practically, if not exactly, same way as the individual parameters would have been passed, so it’s really no less efficient. ( **Update June 2024**: Pengji Zhang pointed out that this [applies only to the 2-element `struct\ fatptr`](https://lists.sr.ht/~skeeto/public-inbox/%3CCANOCUiz9ZjRi06pvSDmKsXcHcTiWfAJCeKQUn3EYCh7Tv0poVA@mail.gmail.com%3E), and not to 3-element slice headers discussed below.)
...
Go famously has pointers, including *internal pointers*, but not pointer arithmetic. You can take the address of ( [nearly](https://utcc.utoronto.ca/~cks/space/blog/programming/GoAddressableValues)) anything, but you can’t make that pointer point at anything else, even if you took the address of an array element. Pointer arithmetic would undermine Go’s type safety, so it can only be done through special mechanisms in the `unsafe` package.
...
`unsafe`](https://utcc.utoronto.ca/~cks/space/blog/programming/GoVariableToArrayConversion).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Go%20Slices%20are%20Fat%20Pointers) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Go+Slices+are+Fat+Pointers).
