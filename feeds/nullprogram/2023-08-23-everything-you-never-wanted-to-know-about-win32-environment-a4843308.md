...
url: https://nullprogram.com/blog/2023/08/23/
published: "2023-08-23T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2023/08/23/
---
...
nullprogram.com/blog/2023/08/23/

In an effort to avoid [programming by superstition](https://utcc.utoronto.ca/~cks/space/blog/programming/ProgrammingViaSuperstition), I did a deep dive into the Win32 “environment block,” the data structure holding a process’s environment variables, in order to better understand it. Along the way I discovered implied and undocumented behaviors. (The *environment block* must not to be confused with the [Process Environment Block](https://www.geoffchappell.com/studies/windows/km/ntoskrnl/inc/api/pebteb/peb/index.htm) (PEB) which is different.) Because I cannot possibly retain all the quirky details in my head for long, I’m writing them down for future reference. I ran my tests on different Windows versions as far back as Windows XP SP3 in order to fill in gaps where documentation is ambiguous, incomplete, or wrong. Overall conclusion: Correct, direct manipulation of an environment block is impossible *in the general case* due to under-specified and incorrect documentation. This has important consequences mainly for programming language runtimes.

Win32 has two interfaces for interacting with environment variables:

1. [GetEnvironmentVariable](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-getenvironmentvariable) and [SetEnvironmentVariable](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-setenvironmentvariable)
2. [GetEnvironmentStrings](https://learn.microsoft.com/en-us/windows/win32/api/processenv/nf-processenv-getenvironmentstringsw) and [FreeEnvironmentStrings](https://learn.microsoft.com/en-us/windows/win32/api/processenv/nf-processenv-freeenvironmentstringsw)
...
As usual for Win32, there are [no rules against ill-formed UTF-16](https://simonsapin.github.io/wtf-8/), and I could [always pass](/blog/2022/02/18/) such “UTF-16” through into the child environment block. Keep that in mind even when using the get/set interface.
...
At least on more recent versions of Windows, my experiments proved exactly the opposite. There is no limit on a user-defined environment variables, but environment blocks are limited to 2GiB, for both 32-bit and 64-bit processes. I could even create such huge environments in [large address
aware](https://learn.microsoft.com/en-us/cpp/build/reference/largeaddressaware-handle-large-addresses) 32-bit processes, though the interfaces are prone to error due to allocations problems.
...
Suppose you’re meticulous and want to sort your environment block before spawning a process. How do you go about it? There’s the rub: The official documentation is incomplete! The [Changing Environment Variables](https://learn.microsoft.com/en-us/windows/win32/procthread/changing-environment-variables) page says:
...
2023](https://web.archive.org/web/20180110151515/http://msdn.microsoft.com/en-us/library/ms682425(VS.85).aspx) about the environment block parameter:
...
variables beginning with `=`](https://lists.sr.ht/~skeeto/public-inbox/%3C098b0421-af0e-46fb-8921-2a4e76f5a361@app.fastmail.com%3E). I misunderstood how it was parsed and came to the wrong conclusion.

- [c](/tags/c/)
- [win32](/tags/win32/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Everything%20you%20never%20wanted%20to%20know%20about%20Win32%20environment%20blocks) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Everything+you+never+wanted+to+know+about+Win32+environment+blocks).

« ["Once" one-time concurrent initialization with an integer](/blog/2023/07/31/)

» [How to link identical function names from different DLLs](/blog/2023/08/27/)
