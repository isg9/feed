...
feed: nullprogram
guid: https://nullprogram.com/blog/2015/02/17/
---
...
nullprogram.com/blog/2015/02/17/

As a result of making regular use of [object-oriented programming in
C](/blog/2014/10/21/), I’ve discovered a useful reference counting technique for the occasional dynamically allocated structs that need it. The situation arises when the same struct instance is shared between an arbitrary number of other data structures and I need to keep track of it all.
...
This is basically all there is to a C++ [shared\_ptr](http://en.cppreference.com/w/cpp/memory/shared_ptr), leveraging C++’s destructors and performing all increment/decrement work automatically.

### Thread Safety

Those increments and decrements aren’t thread safe, so this won’t work as-is when data structures are shared between threads. If you’re sure that you’re using GCC on a capable platform, you can make use of its [atomic builtins](http://gcc.gnu.org/onlinedocs/gcc-4.1.2/gcc/Atomic-Builtins.html), making the reference counter completely thread safe.
...
Or if you’re using C11, [make use of the new stdatomic.h](/blog/2014/09/02/).
...
On a related note, the `stdlib.h` `free()` function doesn’t take a `const` pointer, so the `free()` method taking a `const` pointer is a slight departure from the norm. Taking a non- `const` pointer [was a
mistake in the C standard library](http://yarchive.net/comp/const.html). The `free()` function mutates the pointer itself — including all other pointers to that object — making it invalid. Semantically, it doesn’t mutate the memory *behind* the pointer, so it’s not actually violating the `const`. To compare, the [Linux kernel `kfree()`](http://lxr.free-electrons.com/source/include/linux/slab.h#L144) takes a `const void *`.
...
broken](https://randomascii.wordpress.com/2013/04/03/stop-using-strncpy-already/) and `strlcpy()` is non-standard, so it’s the most straightforward way to do this in standard C.);
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Generic%20C%20Reference%20Counting) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Generic+C+Reference+Counting).
