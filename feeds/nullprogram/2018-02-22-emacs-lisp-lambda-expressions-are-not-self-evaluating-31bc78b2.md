...
url: https://nullprogram.com/blog/2018/02/22/
published: "2018-02-22T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2018/02/22/
---
...
One of the long-standing issues with Emacs is that loading Emacs Lisp files (.el and .elc) is a slow process, even when those files have been byte compiled. There are a number of dirty hacks in place to deal with this issue, and the biggest and nastiest of them all is the [*dumper*](https://lwn.net/Articles/707615/), also known as *unexec*.

The Emacs you routinely use throughout the day is actually a previous instance of Emacs that’s been resurrected from the dead. Your undead Emacs was probably created months, if not years, earlier, back when it was originally compiled. The first stage of compiling Emacs is to compile a minimal C core called `temacs`. The second stage is loading a bunch of Emacs Lisp files, then dumping a memory image in an unportable, platform-dependent way. On Linux, this actually [requires
special hooks in glibc](https://lwn.net/Articles/707615/). The Emacs you know and love is this dumped image loaded back into memory, continuing from where it left off just after it was compiled. Regardless of your own feelings on the matter, you have to admit [this *is* a very lispy thing to do](/blog/2011/01/30/).

There are two notable costs to Emacs’ dumper:

1. The dumped image contains hard-coded memory addresses. This means Emacs can’t be a *Position Independent Executable* (PIE). It can’t take advantage of a security feature called *Address Space Layout* *Randomization* (ASLR), which would increase the difficulty of [exploiting](/blog/2017/07/19/) some [classes of bugs](/blog/2012/09/28/). This might be important to you if Emacs processes untrusted data, such as when it’s used as [a mail client](/blog/2013/09/03/), [a web server](https://github.com/skeeto/emacs-web-server) or generally [parses data downloaded across the network](https://github.com/skeeto/elfeed).

2. It’s not possible to cross-compile Emacs since it can only be dumped by running `temacs` on its target platform. As an experiment I’ve attempted to dump the Windows version of Emacs on Linux using [Wine](https://www.winehq.org/), but was unsuccessful.

The good news is that there’s [a portable dumper](https://lists.gnu.org/archive/html/emacs-devel/2018-02/msg00347.html) in the works that makes this a lot less nasty. If you’re adventurous, you can already disable dumping and run `temacs` directly by setting [`CANNOT_DUMP=yes` at compile time](https://lists.gnu.org/archive/html/bug-gnu-emacs/2016-11/msg00729.html). Be warned, though, that a non-dumped Emacs takes several seconds, or worse, to initialize *before* it even begins loading your own configuration. It’s also somewhat buggy since it seems nobody ever runs it this way productively.
...
The big problem with passing an s-expression is that it will be left uncompiled due to being quoted. [I’ve talked before about the
importance of evaluating your lambdas](/blog/2017/12/14/). `eval-after-load` not only encourages badly written Emacs Lisp, it demands it.
...
sparked this article](https://github.com/skeeto/elfeed/pull/268)? What can we do with the bad version of `eval-after-load`? We could situate a lambda such that it’s evaluated, but then smuggle the resulting function object into the form passed to `eval-after-load`, all using a backquote.
...
Where the second value ( `#[...]`) is a [byte-code object](/blog/2014/01/04/). However, as the comment notes, this is subtly broken. A cleaner and correct way to solve all this is with a named function. The damage caused by `eval-after-load` will have been (mostly) minimized.
...
Instead, **dynamic scope lambda expression evaluation is** **[idempotent](https://labs.spotify.com/2013/06/18/creative-usernames/).** Applying `eval` to the result will return an `equal`, but not identical ( `eq`), expression. In contrast, a self-evaluating value is also idempotent under evaluation, but with `eq` results.
...
I can [imagine an implementation](/blog/2017/05/03/) of Emacs Lisp where dynamic scope lambda expressions are in the same boat, where they’re not even idempotent. For example:
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Emacs%20Lisp%20Lambda%20Expressions%20Are%20Not%20Self-Evaluating) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Emacs+Lisp+Lambda+Expressions+Are+Not+Self-Evaluating).
