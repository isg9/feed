...
feed: nullprogram
guid: https://nullprogram.com/blog/2025/09/30/
---
...
*Update October 2025: [further enhancements](/blog/2025/10/16/)*.

Patrice Roy’s new book, [*C++ Memory Management*](https://www.packtpub.com/en-us/product/c-memory-management-9781805129806), has made me more conscious of object lifetimes. C++ is stricter than C about lifetimes, and common, textbook memory management that’s sound in C is less so in C++ — *more than I realized*. The book also presents a form of arena allocation so watered down as to enjoy none of the benefits. (Despite its precision otherwise, the second half is also littered with [integer overflows](https://github.com/PacktPublishing/C-Plus-Plus-Memory-Management/blob/9e4c4ea7/chapter12/Vector-better.cpp#L45) lacking [the appropriate checks](/blog/2024/05/24/), and near the end has some [pointer
overflows](https://github.com/PacktPublishing/C-Plus-Plus-Memory-Management/blob/9e4c4ea7/chapter14/Vector_with_allocator_cpp23.cpp#L118-L119) invalidating the check.) However, I’m grateful for the new insights, and it’s made me revisit [my own C++ arena allocation](/blog/2024/04/14/). In this new light I see I got it subtly wrong myself!

Surprising to most C++ programmers, but not language lawyers, [idiomatic C
memory allocation was ill-formed in C++ until recently](https://wg21.link/P0593#idiomatic-c-code-as-c):
...
Since I’m thinking about lifetimes, what about the other end? My arena does not call destructors, by design, and starts new lifetimes on top of objects that are technically still alive. Is that undefined behavior? As far as I can tell [this is allowed](https://en.cppreference.com/w/cpp/language/lifetime.html#Storage_reuse), even for non-trivial destructors, with the caveat that it might leak resources. In this case the resource is memory managed by the arena, so that’s fine of course.

So addressing pointer provenance also produced a nicer definition. What a great result from reading that book! While researching, I noticed Jonathan Müller, who personally gave me great advice and feedback on my previous article, [talked about lifetimes](https://www.youtube.com/watch?v=oZyhq4D-QL4) just a couple weeks later. I recommend both.

- [cpp](/tags/cpp/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20More%20speculations%20on%20arenas%20in%20C++) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=More+speculations+on+arenas+in+C%2B%2B).
