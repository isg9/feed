...
url: https://nullprogram.com/blog/2017/01/21/
published: "2017-01-21T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2017/01/21/
---

# Manual Control Flow Guard in C

## [Manual Control Flow Guard in C](/blog/2017/01/21/)

January 21, 2017

nullprogram.com/blog/2017/01/21/

Recent versions of Windows have a new exploit mitigation feature called [*Control Flow Guard*](http://sjc1-te-ftp.trendmicro.com/assets/wp/exploring-control-flow-guard-in-windows10.pdf) (CFG). Before an indirect function call — e.g. function pointers and virtual functions — the target address checked against a table of valid call addresses. If the address isn’t the entry point of a known function, then the program is aborted.

If an application has a buffer overflow vulnerability, an attacker may use it to overwrite a function pointer and, by the call through that pointer, control the execution flow of the program. This is one way to initiate a [*Return Oriented Programming*](https://skeeto.s3.amazonaws.com/share/p15-coffman.pdf) (ROP) attack, where the attacker constructs [a chain of *gadget* addresses](https://github.com/JonathanSalwan/ROPgadget) — a gadget being a couple of instructions followed by a return instruction, all in the original program — using the indirect call as the starting point. The execution then flows from gadget to gadget so that the program does what the attacker wants it to do, all without the attacker supplying any code.

The two most widely practiced ROP attack mitigation techniques today are *Address Space Layout Randomization* (ASLR) and *stack* *protectors*. The former randomizes the base address of executable images (programs, shared libraries) so that process memory layout is unpredictable to the attacker. The addresses in the ROP attack chain depend on the run-time memory layout, so the attacker must also find and exploit an [information leak](https://github.com/torvalds/linux/blob/4c9eff7af69c61749b9eb09141f18f35edbf2210/Documentation/sysctl/kernel.txt#L373) to bypass ASLR.
...
pointer for polymorphism](/blog/2014/10/21/).
...
unspecified](/blog/2016/05/30/) and isn’t portable, but this implementation will work fine anywhere I would care to use it.
...
In a real application I would have a [global `cfg` bitmap](/blog/2016/12/23/) for the whole program, and define `cfg_check()` in a header as an `inline` function.
...
check](http://www.tedunangst.com/inks/l/849) in its mbuf library. Instead of a bitmap, the function pointer is replaced with an index into an array of registered function pointers. That approach is cleaner, more efficient, completely portable, and has no false positives.

- [c](/tags/c/)
- [linux](/tags/linux/)
- [netsec](/tags/netsec/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Manual%20Control%20Flow%20Guard%20in%20C) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Manual+Control+Flow+Guard+in+C).

« [C Closures as a Library](/blog/2017/01/08/)

» [How to Write Fast(er) Emacs Lisp](/blog/2017/01/30/)
