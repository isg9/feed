...
url: https://nullprogram.com/blog/2016/08/07/
published: "2016-08-07T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2016/08/07/
---
...
nullprogram.com/blog/2016/08/07/
...
There are exceptions of course, such as programs that download files over a network. The partial result has value, especially if the transfer can be [continued from where it left off](https://tools.ietf.org/html/rfc7233). The convention is to append another extension, such as “.part”, to indicate a partial output.
...
A common, more complex approach is to name the file differently from its final name while being written. If written successfully, the completed file is renamed into place. This is already [required for
durable replacement](http://blog.httrack.com/blog/2013/11/15/everything-you-always-wanted-to-know-about-fsync/), so it’s basically free for many applications. In the worst case, where the program is unable to clean up, the obviously incomplete file is left behind only wasting space.
...
The primary Win32 function for opening and creating files is [CreateFile](https://msdn.microsoft.com/en-us/library/windows/desktop/aa363858(v=vs.85).aspx). There are many options, but the key is `FILE_FLAG_DELETE_ON_CLOSE`. Here’s how an application might typically open a file for output.
...
This special flag asks Windows to delete the file as soon as the last handle to to *file object* is closed. Notice I said file object, not file, since [these are different things](https://web.archive.org/web/0/https://blogs.msdn.microsoft.com/oldnewthing/20160108-00/?p=92821). The catch: This flag is a property of the file object, not the file, and cannot be removed.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Automatic%20Deletion%20of%20Incomplete%20Output%20Files) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Automatic+Deletion+of+Incomplete+Output+Files).
