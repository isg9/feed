...
url: https://nullprogram.com/blog/2015/04/19/
published: "2015-04-19T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2015/04/19/
---
...
nullprogram.com/blog/2015/04/19/

Last weekend I created a new Emacs mode, [**nasm-mode**](https://github.com/skeeto/nasm-mode), for editing [Netwide Assembler](http://www.nasm.us/) (NASM) x86 assembly programs. Over the past week I tweaked it until it felt comfortable enough to share on [MELPA](http://melpa.org/). It’s got what you’d expect from a standard Emacs programming language mode: syntax highlighting, automatic indentation, and imenu support. It’s not a full parser, but it knows all of NASM’s instructions and directives.

Until recently I didn’t really have preferences about x86 assemblers ( [GAS](https://www.gnu.org/software/binutils/), NASM, [YASM](http://yasm.tortall.net/), [FASM](http://flatassembler.net/), MASM, etc.) or syntax (Intel, AT&T). I stuck to the GNU Assembler (GAS) since it’s already there with all the other GNU development tools I know and love, and it’s required for inline assembly in GCC. However, nasm-mode now marks my commitment to NASM as my primary x86 assembler.

### Why NASM?

I need an assembler that can assemble 16-bit code (8086, 8088, 80186, 80286), because [real mode is fun](/blog/2014/12/09/). Despite its `.code16gcc` directive, GAS is not suitable for this purpose. It’s *just* enough to get the CPU into protected mode — as needed when writing an operating system with GCC — and that’s it. A different assembler is required for serious 16-bit programming.

[GAS syntax has problems](http://x86asm.net/articles/what-i-dislike-about-gas/). I’m not talking about the argument order (source first or destination first), since there’s no right answer to that one. The linked article covers a number of problems, with these being the big ones for me:
...
- GAS cannot produce pure binaries — raw machine code without any headers or container (ELF, COFF, PE). Pure binaries are useful for developing [shellcode](http://www.vividmachines.com/shellcode/shellcode.html), bootloaders, 16-bit COM programs, and [just-in-time compilers](/blog/2015/03/19/).
...
This is Microsoft’s assembler that comes with Visual Studio. Windows only and not open source, this is in no way a serious consideration. But since NASM’s syntax was originally derived from MASM, it’s worth mentioning. NASM takes the good parts of MASM and [fixes the
mistakes](https://courses.engr.illinois.edu/ece390/archive/mp/f99/mp5/masm_nasm.html) (such as the `offset` operator). It’s different enough that nasm-mode would not work well with MASM.

#### NASM

It’s not perfect, but it’s got an [excellent manual](http://www.nasm.us/doc/), it’s a solid program that does exactly what it says it will do, has a powerful macro system, great 16-bit support, highly portable, easy to build, and its semantics and syntax has been carefully considered. It also comes with a simple, pure binary disassembler ( `ndisasm`). In retrospect it seems like an obvious choice!
...
There are a couple of [other nasm-modes](http://matthieuhauglustaine.blogspot.com/2011/08/nasm-mode-for-emacs.html) floating around with different levels of completeness. Mine should supersede these, and will be much easier to maintain into the future as NASM evolves.

- [emacs](/tags/emacs/)
- [x86](/tags/x86/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20NASM%20x86%20Assembly%20Major%20Mode%20for%20Emacs) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=NASM+x86+Assembly+Major+Mode+for+Emacs).
