...
url: https://nullprogram.com/blog/2018/11/15/
published: "2018-11-15T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2018/11/15/
---

# A JIT Compiler Skirmish with SELinux

## [A JIT Compiler Skirmish with SELinux](/blog/2018/11/15/)
...
compiler](/blog/2015/03/19/) to speed it up. The converter generated a fast, native binary parser in memory according to the data dictionary specification. Processing data now took much less time and everyone rejoiced.
...
Unfortunately, `MAP_ANONYMOUS` not part of POSIX. If you’re being super strict with your includes — [as I tend to be](/blog/2017/03/30/) — this flag won’t be defined, even on systems where it’s supported.
...
So we’ve allocated our memory, but it’s not going to start in an executable state. Why? Because a [W^X](https://en.wikipedia.org/wiki/W%5EX) (“write xor execute”) policy is becoming increasingly common. Attempting to set both write and execute protections at the same time may be denied. (In fact, there’s an SELinux policy for this.)

As a JIT compiler, we need to write to a page *and* execute it. Again, there are two strategies. The complicated strategy is to [map the same
memory at two different places](/blog/2016/04/10/), one with the execute protection, one with the write protection. This allows the page to be modified as it’s being executed without violating W^X.
...
At a high level, That’s pretty close to what I was actually doing. That includes neglecting to check the result of `mprotect(2)`. This worked fine and dandy for several years, when suddenly (shown here in the style [of strace](/blog/2018/06/23/)):
...
myself](/blog/2018/03/27/). This Wine build definitely has no SELinux exceptions. Not only did the Wine loader work correctly, it can change page protections in ways my own Linux programs could not. What’s different?
...
With a little bit of research, I found the [two SELinux policies](https://akkadia.org/drepper/selinux-mem.html) at play here. In my minimal example, I was blocked by `allow_execheap`.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20A%20JIT%20Compiler%20Skirmish%20with%20SELinux) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=A+JIT+Compiler+Skirmish+with+SELinux).
