...
feed: nullprogram
guid: https://nullprogram.com/blog/2019/03/22/
---
...
nullprogram.com/blog/2019/03/22/

*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=19465967) ( [later](https://news.ycombinator.com/item?id=24491453)), [on\_ _reddit](https://old.reddit.com/r/programming/comments/b4iq00/endlessh_an_ssh_tarpit/) ( [also](https://old.reddit.com/r/netsec/comments/b4dwjl/endlessh_an_ssh_tarpit/)), featured in [BSD Now 294](https://www.youtube.com/watch?v=bM65iyRRW0A&t=3m52s).* *Also check out [this Endlessh analysis](https://github.com/bediger4000/ssh-tarpit-behavior).*
...
The Internet is a very hostile place, and anyone who’s ever stood up an Internet-facing IPv4 host has witnessed the immediate and continuous attacks against their server. I’ve maintained [such a
server](/blog/2017/06/15/) for nearly six years now, and more than 99% of my incoming traffic has ill intent. One part of my defenses has been tarpits in various forms. The latest addition is an SSH tarpit I wrote a couple of months ago:
...
4253](https://tools.ietf.org/html/rfc4253#section-4.2), the SSH protocol specification. Immediately after the TCP connection is established, and before negotiating the cryptography, both ends send an identification string:
...
A better option is, instead of processes, to create a thread per connection. On Linux [this is practically the same thing](/blog/2015/05/15/), but it’s still better. However, you still have to allocate a stack for the thread and the kernel will have to spend some resources managing the thread.
...
itself](https://nyman.re/super-simple-ssh-tarpit/). It could introduce artificial “noise” to the connection that requires packet retransmissions, delay ACKs, etc. It would look a lot more like network problems than a tarpit.
...
Since writing Endless [I’ve learned about Python’s `asyncio`](/blog/2019/03/10/), and it’s actually a near perfect fit for this problem. I should have just used it in the first place. The hard part is already implemented within `asyncio`, and the problem isn’t CPU-bound, so being written in Python [doesn’t matter](/blog/2019/02/24/).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Endlessh:%20an%20SSH%20Tarpit) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Endlessh%3A+an+SSH+Tarpit).
