...
feed: nullprogram
guid: https://nullprogram.com/blog/2017/08/20/
---
...
specification](http://pubs.opengroup.org/onlinepubs/9699919799/utilities/make.html), and I’ve spend the last year writing strictly portable Makefiles. Not only has are my builds now portable across all unix-like systems, my Makefiles are cleaner and more robust. Many of the common make extensions — conditionals in particular — lead to fragile, complicated Makefiles and are best avoided anyway. It’s important to be able to trust your build system to do its job correctly.
...
Macro values can contain other macros, which will be expanded recursively each time the macro is expanded. Some make implementations allow the name of the macro being expanded to itself be a macro, which [is turing complete](/blog/2016/04/30/), but this behavior is non-standard.
...
The `PREFIX` macro should default to `/usr/local`, and since it’s a macro the user can override it to install elsewhere, [such as in their
home directory](/blog/2017/06/19/). The user should override it for both building and installing, since the prefix may need to be built into the binary (e.g. `-DPREFIX=$(PREFIX)`).
...
It’s nice to have reliable, portable Makefiles that just work anywhere. [Code to the standards](/blog/2017/03/30/) and you don’t need feature tests or other sorts of special treatment.

- [tutorial](/tags/tutorial/)
- [c](/tags/c/)
- [posix](/tags/posix/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20A%20Tutorial%20on%20Portable%20Makefiles) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=A+Tutorial+on+Portable+Makefiles).
