...
url: https://nullprogram.com/blog/2014/11/22/
published: "2014-11-22T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2014/11/22/
---
...
nullprogram.com/blog/2014/11/22/

When I was a kid I spent some time playing a top-down, 2D, puzzle/action, 1993, MS-DOS game called [*God of Thunder*](http://en.wikipedia.org/wiki/God_of_Thunder_(video_game)). It came on a shareware CD, now long lost, called *Games People Play*. A couple decades later I was recently reminded of the game and decided [to dig it up](http://www.adeptsoftware.com/got/) and play it again. It’s not quite as exciting as I remember it — nostalgia really warps perception — but it’s still an interesting game nonetheless.
...
You see, back in those old days, compression wasn’t really a “solved” problem like it is today. They had to [design and implement their own
algorithms](http://www.gamecrafters.com/gamecrafters/maddog/docs/history.html), with varying degrees of success. Today if you need compression for a project, you just grab [zlib](http://www.zlib.net/). Released in 1995, it implements the most widely used compression algorithm today, DEFLATE, with a tidy, in-memory API. zlib is well-tested, thoroughly optimized, and sits in a nearly-perfect sweet spot between compression ratio and performance. There’s even an [embeddable
version](https://code.google.com/p/miniz/). Since spinning platters are so slow compared to CPUs, compression is likely to speed up an application simply because fewer bytes need to go to and from the disk. Today it’s less about saving storage space and more about reducing input/output demands.

Fortunately for me, someone has [already reversed engineered](http://www.shikadi.net/moddingwiki/DAT_Format_%28God_of_Thunder%29) most of the *God of Thunder* assets. It uses its own flavor of [Lempel-Ziv-Storer-Szymanski](http://en.wikipedia.org/wiki/Lempel%E2%80%93Ziv%E2%80%93Storer%E2%80%93Szymanski) (LZSS), which itself is derived from LZ77, one of the algorithms used in DEFLATE. The original LZSS paper focuses purely on the math, describing the algorithm in terms of symbols with no concern for how it’s actually serialized into bits. Those specific details were decided by the game’s developers, and that’s what I’ll be describing below.

As an adult I’m finding the *God of Thunder* asset formats to be more interesting than the game itself. It’s a better puzzle! I [really
enjoy](https://github.com/skeeto/binitools) studying the file formats of various applications, especially older ones that didn’t have modern standards to build on. Usually lots of thought and engineering goes into the design these formats — and, too often, not enough thought goes into it. The format’s specifics reveal insights into the internal workings of the application, sometimes exposing unanticipated failure modes. Prying apart odd, proprietary formats (i.e. “data reduction”) is probably my favorite kind of work at my day job, and it comes up fairly often.
...
It’s important to note that the length is allowed to extend beyond the output buffer (offset < length). The bytes are, in effect, copied one at a time into the output and may potentially be reused within the same operation (like the opposite of [memmove](http://man7.org/linux/man-pages/man3/memmove.3.html)). An offset of 1 and a length of 10 means “repeat the last output byte 10 times.”
...
After spending some time playing around with this format, I thought about what it would take to make an LZSS quine. That is, **find an** **LZSS compressed stream that decompresses to itself.** It’s been done for DEFLATE, which I imagine is a much harder problem. There are [zip
files containing exact copies of themselves](http://steike.com/code/useless/zip-file-quine/), recursively. I’m pretty confident it’s never been done for this exact compression format, simply because it’s so specific to this old MS-DOS game.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20LZSS%20Quine%20Puzzle) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=LZSS+Quine+Puzzle).

« [C Object Oriented Programming](/blog/2014/10/21/)
