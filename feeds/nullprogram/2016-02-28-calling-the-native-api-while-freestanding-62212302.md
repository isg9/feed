...
url: https://nullprogram.com/blog/2016/02/28/
published: "2016-02-28T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2016/02/28/
---
...
On x86\_64 (GCC 4.9.3, -Os), this `memmove()` call is replaced with two instructions. This isn’t possible when calling an opaque function in a non-standard dynamic library. The side effects could be anything.
...
These Native API macro aliases are what have allowed certain Wine issues [to slip by unnoticed for years](https://bugs.winehq.org/show_bug.cgi?id=38783). Very few user space applications actually call Native API functions, even when addressed directly by name in the source. The development suite is pulling a bait and switch.

Like [last time I danced at the edge of the compiler](/blog/2014/12/09/), this has caused headaches in my recent experimentation with freestanding executables. The MinGW headers assume that the programs including them will link against a C runtime. Dirty hack warning: To work around it, I have to undo the definition in the MinGW headers and make my own. For example, to use the real `RtlMoveMemory()`:
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Calling%20the%20Native%20API%20While%20Freestanding) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Calling+the+Native+API+While+Freestanding).
