...
url: https://nullprogram.com/blog/2017/07/19/
published: "2017-07-19T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2017/07/19/
---
...
nullprogram.com/blog/2017/07/19/

Last week I was discussing [CVE-2017-7529](https://security-tracker.debian.org/tracker/CVE-2017-7529) with [my intern](/blog/2016/09/02/). Specially crafted input to Nginx causes an integer overflow which has the potential to leak sensitive information. But how could an integer overflow be abused to trick a program into leaking information? To answer this question, I put together the simplest practical example I could imagine.

- [https://github.com/skeeto/integer-overflow-demo](https://github.com/skeeto/integer-overflow-demo)

This small C program converts a vector image from a custom format (described below) into a [Netpbm image](https://en.wikipedia.org/wiki/Netpbm_format), a [conveniently simple
format](/blog/2017/07/02/). The program defensively and carefully parses its input, but still makes a subtle, fatal mistake. This mistake not only leads to sensitive information disclosure, but, with a more sophisticated attack, could be used to execute arbitrary code.
...
Without spoiling anything yet, let’s look at how this program works. The first thing to notice is that I’m using a custom “ [obstack](http://www.gnu.org/software/libc/manual/html_node/Obstacks.html)” allocator instead of `malloc()` and `free()`. Real-world allocators have some defenses against this particular vulnerability. Plus a specific exploit would have to target a specific libc. By using my own allocator, the exploit will mostly be portable, making for a better and easier demonstration.
...
In practice, this sort of program wouldn’t have sensitive data resident in memory. Instead an attacker would target the program’s stack with those `s` commands — specifically the [return pointers](/blog/2017/01/21/) — and perform a ROP attack against the application. With the exploit header above and a platform where `long` the same size as a `size_t`, the program will behave as if all available memory has been allocated to the image, so the `s` command could be used to poke custom values *anywhere* in memory. This is a much more complicated exploit, and it has to contend with ASLR and random stack gap, but it’s feasible.

- [netsec](/tags/netsec/)
- [c](/tags/c/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Integer%20Overflow%20into%20Information%20Disclosure) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Integer+Overflow+into+Information+Disclosure).
