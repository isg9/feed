...
url: https://nullprogram.com/blog/2016/09/05/
published: "2016-09-05T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2016/09/05/
---
...
nullprogram.com/blog/2016/09/05/

The C standard library includes a qsort() function for sorting arbitrary buffers given a comparator function. The name comes from its [original Unix implementation, “quicker sort,”](https://gallium.inria.fr/~maranget/X/421/09/bentley93engineering.pdf) a variation of the well-known quicksort algorithm. The C standard doesn’t specify an algorithm, except to say that it may be unstable (C99 §7.20.5.2¶4) — equal elements have an unspecified order. As such, different C libraries use different algorithms, and even when using the same algorithm they make different implementation trade-offs.

I added a drawing routine to a comparison function to see what the sort function was doing for different C libraries. Every time it’s called for a comparison, it writes out a snapshot of the array as a Netpbm PPM image. It’s [easy to turn concatenated PPMs into a GIF or
video](/blog/2011/11/28/). Here’s my code if you want to try it yourself:

- [qsort-animate.c](/download/qsort-animate.c)
...
To get an optimized GIF on unix-like systems, run it like so. (Microsoft’s [UCRT currently has serious bugs](https://web.archive.org/web/20161126142829/http://radiance-online.org:82/pipermail/radiance-dev/2016-March/001578.html) with pipes, so it was run differently in that case.)
...
The number of animation frames reflects the efficiency of the sort, but this isn’t really a benchmark. The input array is fully shuffled, and real data often not. For a benchmark, have a look at [a libc
qsort() shootout of sorts](http://calmerthanyouare.org/2013/05/31/qsort-shootout.html) instead.
...
Sorted in **503 frames**. [dietlibc](https://www.fefe.de/dietlibc/) is an alternative C standard library for Linux. It’s optimized for size, which shows through its slower performance. It looks like a quicksort that always chooses the last element as the pivot.
...
Sort in **637 frames**. [musl libc](https://www.musl-libc.org/) is another alternative C standard library for Linux. It’s my personal preference when I statically link Linux binaries. Its qsort() looks a lot like a heapsort, and with some research I see it’s actually [smoothsort](http://www.keithschwarz.com/smoothsort/), a heapsort variant.
...
![](/img/qsort/cygwin.gif)

### MSVCRT.DLL (MinGW) and UCRT (Visual Studio)

MinGW builds against MSVCRT.DLL, found on every Windows system despite its [unofficial status](https://web.archive.org/web/0/https://blogs.msdn.microsoft.com/oldnewthing/20140411-00/?p=1273). Until recently Microsoft didn’t include a C standard library as part of the OS, but that changed with their [Universal CRT (UCRT) announcement](https://web.archive.org/web/0/https://blogs.msdn.microsoft.com/vcblog/2015/03/03/introducing-the-universal-crt/). I thought I’d try them both.
...
Finally I ran it against [Pelles C](http://www.smorgasbordet.com/pellesc/), a C compiler for Windows. It sorted in **463 frames**. I can’t find any information about it, but it looks like some sort of hybrid between quicksort and insertion sort. Like BSD qsort(), it finds a good median for the pivot, partitions the elements, and if a partition is small enough, it switches to insertion sort. This should behave well on mostly-sorted arrays, but poorly on well-shuffled arrays (like this one).

![](/img/qsort/pellesc.gif)
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Inspecting%20C's%20qsort%20Through%20Animation) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Inspecting+C%27s+qsort+Through+Animation).
