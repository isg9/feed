...
feed: nullprogram
guid: https://nullprogram.com/blog/2023/10/08/
---
...
*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=37815674) and [on reddit](https://old.reddit.com/r/C_Programming/comments/173e0vn/).*
...
Some people prefer an `s` prefix for signed types. I prefer `i`, plus as you’ll see, I have other designs for `s`. For sizes, `isize` would be more consistent, and wouldn’t hog the identifier, but [signed sizes are the
way](/blog/2023/09/27/) and so I want them in a place of privilege. `usize` is niche, mainly for interacting with external interfaces where it might matter.
...
It has useful properties beyond [the usual benefits](/blog/2022/06/26/):

- It does not require separate definitions for debug and release builds. Instead it’s controlled by the presence of Undefined Behavior Sanitizer (UBSan), which is already present/absent in these circumstances. That includes [fuzz testing](/blog/2019/01/25/).
...
One of my most productive changes this year has been the total rejection of null terminated strings — another of those terrible mistakes — and the embrace of [this basic string type](/blog/2023/01/18/#implementation-highlights):
...
Aside from static data, I’ve also moved away from initializers except the conventional zero initializer. (Notable exception: `s8` and `s16` macros.) This includes designated initializers. Instead I’ve been initializing with assignments. For example, this [buffered output](/blog/2023/02/13/) “constructor”:
...
There’s surely a lot more to my style than this, but unlike the above, those details haven’t changed this year. To see most of the mentioned items in action in a small program, see [`wordhist.c`](https://github.com/skeeto/scratch/blob/master/misc/wordhist.c), one of my testing grounds for [hash-tries](/blog/2023/09/30/), or for a slightly larger program, [`asmint.c`](https://github.com/skeeto/scratch/blob/master/misc/asmint.c), a mini programming language implementation.

- [c](/tags/c/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20My%20personal%20C%20coding%20style%20as%20of%20late%202023) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=My+personal+C+coding+style+as+of+late+2023).
