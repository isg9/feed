...
url: https://nullprogram.com/blog/2019/03/10/
published: "2019-03-10T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2019/03/10/
---
...
nullprogram.com/blog/2019/03/10/

As part of [building my Python proficiency](/blog/2019/02/24/), I’ve learned how to use [asyncio](https://docs.python.org/3/library/asyncio.html). This new language feature [first appeared in
Python 3.5](https://docs.python.org/3/whatsnew/3.5.html#whatsnew-pep-492) ( [PEP 492](https://www.python.org/dev/peps/pep-0492/), September 2015). JavaScript grew [a
nearly identical feature](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) in ES2017 (June 2017). An async function can pause to await on an asynchronously computed result, much like a generator pausing when it yields a value.

In fact, both Python and JavaScript async functions are essentially just fancy generator functions with some specialized syntax and semantics. That is, they’re [stackless coroutines](https://blog.varunramesh.net/posts/stackless-vs-stackful-coroutines/). Both languages already had generators, so their generator-like async functions are a natural extension that — unlike [*stackful* coroutines](/blog/2017/06/21/) — do not require significant, new runtime plumbing.

Emacs [officially got generators in 25.1](/blog/2018/05/31/) (September 2016), though, unlike Python and JavaScript, it didn’t require any additional support from the compiler or runtime. It’s implemented entirely using Lisp macros. In other words, it’s just another library, not a core language feature. In theory, the generator library could be easily backported to the first Emacs release to [properly support lexical
closures](/blog/2016/12/22/), Emacs 24.1 (June 2012).
...
- **[https://github.com/skeeto/emacs-aio](https://github.com/skeeto/emacs-aio)**
...
As I said, a promise represents a future result. In practical terms, a promise is just an object to which one can subscribe with a callback. When the result is ready, the callbacks are invoked. Another way to put it is that *promises [reify](https://en.wikipedia.org/wiki/Reification_(computer_science)) the concept of callbacks*. A callback is no longer just the idea of extra argument on a function. It’s a first-class *thing* that itself can be passed around as a value.

Promises have two slots: the final promise *result* and a list of *subscribers*. A `nil` result means the result hasn’t been computed yet. It’s so simple I’m not even [bothering with `cl-struct`](/blog/2018/02/14/).
...
Despite `aio` being entirely about managing concurrent, asynchronous operations, it has nothing at all to do with threads — as in Emacs 26’s support for kernel threads. All async functions and promise callbacks are expected to run *only* on the main thread. That’s not to say an async function can’t await on a result from another thread. It just must be [done very carefully](/blog/2017/02/14/).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20An%20Async%20/%20Await%20Library%20for%20Emacs%20Lisp) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=An+Async+%2F+Await+Library+for+Emacs+Lisp).
