...
feed: nullprogram
guid: https://nullprogram.com/blog/2014/06/10/
---

# A GPU Approach to Conway's Game of Life

## [A GPU Approach to Conway's Game of Life](/blog/2014/06/10/)

June 10, 2014

nullprogram.com/blog/2014/06/10/

Update: In the next article, I [extend this
program to solving mazes](/blog/2014/06/22/). The demo has also been [ported to the Skew programming language](http://rykap.com/graphics/skew/2016/01/23/game-of-life.html).

[Conway’s Game of Life](http://en.wikipedia.org/wiki/Conway%27s_Game_of_Life) is [another well-matched
workload](/blog/2014/06/01/) for GPUs. Here’s the actual WebGL demo if you want to check it out before continuing.

- [https://skeeto.github.io/webgl-game-of-life/](http://skeeto.github.io/webgl-game-of-life/) ( [source](http://github.com/skeeto/webgl-game-of-life/))
...
The entire simulation state will be stored in a single, 2D texture in GPU memory. Each pixel of the texture represents one Life cell. The texture will have the internal format GL\_RGBA. That is, each pixel will have a red, green, blue, and alpha channel. This texture is not drawn directly to the screen, so how exactly these channels are used is mostly unimportant. It’s merely a simulation data structure. This is because I’m using [the OpenGL programmable pipeline for general
computation](/blog/2013/06/10/). I’m calling this the “front” texture.

Four multi-bit (actual width is up to the GPU) channels seems excessive considering that all I *really* need is a single bit of state for each cell. However, due to [framebuffer completion
rules](http://www.opengl.org/wiki/Framebuffer_Object#Framebuffer_Completeness), in order to draw onto this texture it *must* be GL\_RGBA. I could pack more than one cell into one texture pixel, but this would reduce parallelism: the shader will run once per pixel, not once per cell.
...
[Using Igloo again](https://github.com/skeeto/igloojs) to keep the call concise, a fullscreen quad is rendered so that the fragment shader runs *exactly* once for each cell. That `state` uniform is the front texture, bound as `GL_TEXTURE0`.
...
This project turned out to be far simpler than I anticipated, so much so that I was able to get the simulation running within an evening’s effort. I learned a whole lot more about WebGL in the process, enough for me to revisit [my WebGL liquid simulation](/blog/2013/06/26/). It uses a similar texture-drawing technique, which I really fumbled through that first time. I dramatically cleaned it up, making it fast enough to run smoothly on my mobile devices.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20A%20GPU%20Approach%20to%20Conway's%20Game%20of%20Life) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=A+GPU+Approach+to+Conway%27s+Game+of+Life).

« [Tag Feeds for null program](/blog/2014/06/08/)
