...
feed: nullprogram
guid: https://nullprogram.com/blog/2021/12/30/
---

# Some sanity for C and C++ development on Windows

## [Some sanity for C and C++ development on Windows](/blog/2021/12/30/)

December 30, 2021

nullprogram.com/blog/2021/12/30/
...
There are [multiple C implementations](/blog/2016/06/13/), so how could they all be bad, even the [early ones](/blog/2018/04/13/)? Microsoft’s C runtime has defined how the standard library should work on the platform, and everyone else followed along for the sake of compatibility. I’m excluding [Cygwin](https://www.cygwin.com/) and its major fork, [MSYS2](https://www.msys2.org/), despite not inheriting any of these flaws. They change so much that they’re effectively whole new platforms, not truly “native” to Windows.
...
If you’re just working on one large project, bypassing the C runtime isn’t such a big deal, and you’re likely already doing so to access important platform functionality. You don’t really even need a C runtime. However, if you write many small programs, [as I do](https://github.com/skeeto/scratch), writing the same special Windows support for each one ends up being most of the work, and honestly makes properly supporting Windows not worth the trouble. I end up just accepting the broken defaults most of the time.

Before diving into the details, if you’re looking for a quick-and-easy solution for the Mingw-w64 toolchain, [including w64devkit](/blog/2020/05/15/), which magically makes your C and C++ console programs behave well on Windows, I’ve put together a “library” named **[libwinsane](https://github.com/skeeto/scratch/tree/master/libwinsane)**. It solves all problems discussed in this article, except for one. No source changes required, simply link it into your program.

### What exactly is broken?

The Windows API comes in two flavors: narrow with an “A” (“ANSI”) suffix, and wide (Unicode, UTF-16) with a “W” suffix. The former is the legacy API, where an active *code page* maps 256 bytes onto (up to) 256 specific characters. On typical machines configured for European languages, this means [code page 1252](https://en.wikipedia.org/wiki/Windows-1252). [Roughly speaking](http://simonsapin.github.io/wtf-8/), Windows internally uses UTF-16, and calls through the narrow interface use the active code page to translate the narrow strings to wide strings. The result is that calls through the narrow API have limited access to the system.

The UTF-8 encoding was invented in 1992 and standardized by January 1993. UTF-8 was adopted by the unix world over the following years due to [its
backwards-compatibility](/blog/2017/10/06/#what-is-utf-8) with its existing interfaces. Programs could read and write Unicode data, access Unicode paths, pass Unicode arguments, and get and set Unicode environment variables without needing to change anything. Today UTF-8 has become the dominant text encoding format in the world, in large part due to the world wide web.

In July 1993, Microsoft introduced the wide Windows API with the release of Windows NT 3.1, placing all their bets on UCS-2 (later UTF-16) rather than UTF-8. This turned out to be a mistake, since [UTF-16 is inferior to
UTF-8 in practically every way](http://utf8everywhere.org/), though admittedly some problems weren’t so obvious at the time.
...
UTF-8 as their active code page on start](https://docs.microsoft.com/en-us/windows/apps/design/globalizing/use-utf8-code-page), along with supporting UTF-8 on more narrow API functions. This is like the magic switch I wanted, except that it involves embedding some ugly XML into your binary in a particular way. At least it’s now an option.
...
Annoying, but at least it’s not *that* painful. This only covers output, though, meaning programs can only print UTF-8. Unfortunately [UTF-8 input
still doesn’t work](https://github.com/microsoft/terminal/issues/4551#issuecomment-585487802), and setting the input code page doesn’t do anything despite reporting success:
...
bypassing the C runtime](/blog/2020/05/04/) since it’s still broken.
...
The standard also specifies that standard input, output, and error are all open as text streams, and there’s no portable method to change the stream mode to binary — a serious deficiency with the standard. On unix-likes this doesn’t matter, but on Windows it means programs can’t read or write binary data on standard streams without calling a non-standard function. It also means reading and writing standard streams is slow, [frequently a
bottleneck](/blog/2021/12/04/) unless I route around it.

Personally, I like [writing binary data to standard output](/blog/2020/06/29/), [including video](/blog/2020/11/24/), and sometimes [binary filters](/blog/2017/07/02/) that also read binary input. I do it so often that in probably half my C programs I have this snippet in `main` just so they work correctly on Windows:
...
This built-in newline translation, along with the Windows standard text editor, Notepad, [lagging decades behind](https://devblogs.microsoft.com/commandline/extended-eol-in-notepad/), meant that many other programs, including Git, grew their own, annoying, newline conversion [misfeatures](https://github.com/skeeto/w64devkit/issues/10) that cause [other problems](https://github.com/skeeto/binitools/commit/2efd690c3983856c9633b0be66d57483491d1e10).
...
If you’re maintaining a substantial program, you probably want to copy and integrate the necessary parts of libwinsane into your project and build, rather than always link against this loose object file. This is more for convenience and for succinctly capturing the concept. You may even want to [enable ANSI escape processing](https://github.com/skeeto/hastyhex/blob/f03b6e0f/hastyhex.c#L298-L309) in your version.

**Update December 2024**: Pavel Galkin [demonstrates how `libwinsane.o`
changes the console state](https://lists.sr.ht/~skeeto/public-inbox/%3Cdf749edc-0413-4735-9cf2-c77db202cc6e@app.fastmail.com%3E), which affects all processes associated with the terminal. This is mostly unavoidable, and it’s one reason I’ve since concluded that UTF-8 manifests are a poor solution. Better to [solve
the problem using a platform layer](/blog/2023/01/18/).

- [c](/tags/c/)
- [cpp](/tags/cpp/)
- [win32](/tags/win32/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Some%20sanity%20for%20C%20and%20C++%20development%20on%20Windows) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Some+sanity+for+C+and+C%2B%2B+development+on+Windows).

« [Fast CSV processing with SIMD](/blog/2021/12/04/)
