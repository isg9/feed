...
url: https://nullprogram.com/blog/2014/05/27/
published: "2014-05-27T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2014/05/27/
---
...
In these languages, performing many string operations at a time, especially with the `+=` operator, allocates many temporary strings. It’s also awkward. For these situations, Java provides a class, [StringBuilder](http://docs.oracle.com/javase/7/docs/api/java/lang/StringBuffer.html), so that these operations can be done with a temporary, efficient, mutable data structure that will emit the final string when complete.
...
There’s an important caveat to using buffers as mutable strings: they’re [not managed by the garbage collector](/blog/2014/01/27/). Each buffer goes into the global buffer list, implemented internally as an intrusive linked list. If a buffer is not on this list, it’s a dead buffer.
...
package](https://github.com/skeeto/elisp-finalize) to associate the buffer with an object that is subject to garbage collection. The buffer will be killed immediately when the object is garbage collected.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Emacs%20Lisp%20Buffer%20Passing%20Style) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Emacs+Lisp+Buffer+Passing+Style).
