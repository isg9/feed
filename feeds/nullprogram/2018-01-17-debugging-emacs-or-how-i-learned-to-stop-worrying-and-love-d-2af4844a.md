...
url: https://nullprogram.com/blog/2018/01/17/
published: "2018-01-17T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2018/01/17/
---

# Debugging Emacs or: How I Learned to Stop Worrying and Love DTrace

## [Debugging Emacs or: How I Learned to Stop Worrying and Love DTrace](/blog/2018/01/17/)

January 17, 2018

nullprogram.com/blog/2018/01/17/

*Update: This article was featured on [BSD Now 233](https://www.youtube.com/watch?v=Xi_pX2QIzho) (starting* *at 21:38).*

For some time [Elfeed](https://github.com/skeeto/elfeed) was experiencing a strange, spurious failure. Every so often users were [seeing an error](https://github.com/skeeto/elfeed/issues/248) (spoiler warning) when updating feeds: “error in process sentinel: Search failed.” If you use Elfeed, you might have even seen this yourself. From the surface it appeared that curl, tasked with the [responsibility for downloading feed data](/blog/2016/06/16/), was producing incomplete output despite reporting a successful run. Since the run was successful, Elfeed assumed certain data was in curl’s output buffer, but, since it wasn’t, it failed hard.
...
For now I just had to wait to [hunt that zebra](https://www.youtube.com/watch?v=fE2KDzZaxvE).

### Bryan Cantrill, DTrace, and FreeBSD

Over the holidays I re-discovered [Bryan Cantrill](https://en.wikipedia.org/wiki/Bryan_Cantrill), a systems software engineer who worked for Sun between 1996 and 2010, and is most well known for [DTrace](http://dtrace.org/blogs/about/). My first exposure to him was in a [BSD
Now interview](https://www.youtube.com/watch?v=l6XQUciI-Sc) in 2015. I had re-watched that interview and decided there was a lot more I had to learn from him. He’s become a personal hero to me. So I scoured the internet for [more of his writing and
talks](http://dtrace.org/blogs/bmc/2018/02/03/talks/). Besides what I’ve already linked in this article, here are a couple more great presentations:
...
Some interesting operating system technology came out of Sun during its final 15 or so years — most notably DTrace and ZFS — and Bryan speaks about it passionately. Almost as a matter of luck, most of it survived the Oracle acquisition thanks to Sun releasing it as open source in just the nick of time. Otherwise it would have been lost forever. The scattered ex-Sun employees, still passionate about their prior work at Sun, along with some of their old customers have since picked up the pieces and kept going as a community under the name [illumos](https://illumos.org/). It’s like an open source flotilla.

Naturally I wanted to get my hands on this stuff to try it out for myself. Is it really as good as they say? Normally I stick to Linux, but it (generally) doesn’t have these Sun technologies. The main reason is license incompatibility. Sun released its code under the [CDDL](https://opensource.org/licenses/CDDL-1.0), which is incompatible with the GPL. Ubuntu *does* [infamously include ZFS](https://insights.ubuntu.com/2016/02/18/zfs-licensing-and-linux/), but other distributions are unwilling to take that risk. Porting DTrace is a serious undertaking since it’s got its fingers throughout the kernel, which also makes the licensing issues even more complicated.

( *Update Feburary 2018*: [DTrace has been released under the
GPLv2](https://gnu.wildebeest.org/blog/mjw/2018/02/14/dtrace-for-linux-oracle-does-the-right-thing/), allowing it to be legally integrated with Linux.)

Linux has a reputation for Not Invented Here (NIH) syndrome, and these licensing issues certainly contribute to that. Rather than adopt ZFS and DTrace, they’ve been reinvented from scratch: btrfs instead of ZFS, and [a slew of partial options](http://www.brendangregg.com/blog/2015-07-08/choosing-a-linux-tracer.html) instead of DTrace. Normally I’m most interested in system call tracing, and my go to is [strace](https://en.wikipedia.org/wiki/Strace), though it certainly has its limitations — including this situation of debugging curl under Emacs. Another famous example of NIH is Linux’s [`epoll(2)`](http://man7.org/linux/man-pages/man7/epoll.7.html), which is a [broken](https://idea.popcount.org/2017-02-20-epoll-is-fundamentally-broken-12/) [version](https://idea.popcount.org/2017-03-20-epoll-is-fundamentally-broken-22/) of BSD [`kqueue(2)`](https://www.freebsd.org/cgi/man.cgi?query=kqueue&sektion=2).

So, if I want to try these for myself, I’ll need to install a different operating system. I’ve dabbled with [OmniOS](https://omnios.omniti.com/), an OS built on illumos, in virtual machines, using it as an alien environment to test some of my software (e.g. [enchive](/blog/2017/03/12/)). OmniOS has a philosophy called [Keep Your Software To Yourself](https://omnios.omniti.com/wiki.php/KYSTY) (KYSTY), which is really just code for “we don’t do packaging.” Honestly, you can’t blame them since [they’re a tiny community](https://utcc.utoronto.ca/~cks/space/blog/solaris/IllumosSupportLimits). The best solution to this is probably [pkgsrc](https://www.pkgsrc.org/), which is essentially a universal packaging system. Otherwise [you’re on your
own](/blog/2017/06/19/).

There’s also [openindiana](https://www.openindiana.org/), which is a more friendly desktop-oriented illumos distribution. Still, the short of it is that you’re very much on your own when things don’t work. The situation is like running Linux a couple decades ago, when it was still difficult to do.

If you’re interested in trying DTrace, the easiest option these days is probably [FreeBSD](https://www.freebsd.org/). It’s got a big, active community, thorough documentation, and a huge selection of packages. Its license (the *BSD* *license*, duh) is compatible with the CDDL, so both ZFS and DTrace have been ported to FreeBSD.

### What is DTrace?

I’ve done all this talking but haven’t yet described what [DTrace
really is](https://wiki.freebsd.org/DTrace/Tutorial). I won’t pretend to write my own tutorial, but I’ll provide enough information to follow along. DTrace is a tracing framework for debugging production systems *in real time*, both for the kernel and for applications. The “production systems” part means it’s stable and safe — using DTrace won’t put your system at risk of crashing or damaging data. The “real time” part means it has little impact on performance. You can use DTrace on live, active systems with little impact. Both of these core design principles are vital for troubleshooting those really tricky bugs that only show up in production.
...
So I’ve settled in FreeBSD as the playground for these technologies, I just have to decide where. I could always run it in a virtual machine, but it’s always more interesting to try things out on real hardware. [FreeBSD supports the Raspberry Pi 2](https://wiki.freebsd.org/FreeBSD/arm/Raspberry%20Pi) as a Tier 2 system, and I had a Raspberry Pi 2 sitting around collecting dust, so I put it to use.
...
I [mostly program in Vim these days](/blog/2017/04/01/), so it’s some days before I even set up Emacs. Eventually I do build Emacs, clone my configuration, fire it up, and give Elfeed a spin.
...
I also want to compare this to exactly what is delivered to Elfeed when curl exits, so I modify the [process sentinel](http://www.gnu.org/software/emacs/manual/html_node/elisp/Sentinels.html) — the callback that handles a subprocess exiting — to call `write-file` before any action is taken. I can compare these buffer dumps to the logs produced by DTrace.
...
*Oh.* That’s right. *This is the [same problem I ran into four years\_ _ago when writing EmacSQL](/blog/2014/02/06/).* By default Emacs connects to subprocesses through a pseudo-terminal (pty). I called this a mistake in Emacs back then, and I still stand by that claim. The pty causes weird, annoying problems for little benefit:
...
Just from eyeballing the DTrace log I knew what to do: dump the pty and switch to a pipe. This is controlled with the `process-connection-type` variable, and fixing it [is a
one-liner](https://github.com/skeeto/elfeed/commit/945765a57d2f27996b6a43bc62e803dc167d1547).
...
The fix reminds me of that [old boilermaker story](https://www.buzzmaven.com/old-engineer-hammer-2/) about charging a lot of money just to swing a hammer. Once the problem arose, **DTrace quickly helped to identify the place to hit Emacs with** **the hammer**.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Debugging%20Emacs%20or:%20How%20I%20Learned%20to%20Stop%20Worrying%20and%20Love%20DTrace) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Debugging+Emacs+or%3A+How+I+Learned+to+Stop+Worrying+and+Love+DTrace).

« [What's in an Emacs Lambda](/blog/2017/12/14/)

» [Inspiration from Data-dependent Rotations](/blog/2018/02/07/)
