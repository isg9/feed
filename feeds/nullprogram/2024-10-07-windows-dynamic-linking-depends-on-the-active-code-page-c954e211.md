...
url: https://nullprogram.com/blog/2024/10/07/
published: "2024-10-07T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2024/10/07/
---
...
Windows paths have been [WTF-16](https://simonsapin.github.io/wtf-8/#ill-formed-utf-16)-encoded for decades, but module names in the [import tables](/blog/2024/06/30/) of [Portable Executable](https://learn.microsoft.com/en-us/windows/win32/debug/pe-format) are octets. If a name contains values beyond ASCII — technically out of spec — then the dynamic linker must somehow decode those octets into Unicode in order to construct a lookup path. There are multiple ways this could be done, and the most obvious is the process’s active code page (ACP), which is exactly what happens. As a consequence, the specific DLL loaded by the linker may depend on the system code page. In this article I’ll contrive such a situation.

[LoadLibraryA](https://learn.microsoft.com/en-us/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibrarya) is a similar situation, and potentially applies the code page to a longer portion of the module path. [LoadLibraryW](https://learn.microsoft.com/en-us/windows/win32/api/libloaderapi/nf-libloaderapi-loadlibraryw) is unaffected, at least for the directly-named module, because it’s Unicode all the way through.
...
Both end with `ral.dll`. I’ve included the [CP-1252](https://en.wikipedia.org/wiki/Windows-1252) encoding for the differing prefixes, and the UTF-8 encoding for the second. I’m using CP-1252 because it’s the most common system code page in the world, especially the Western hemisphere. Due to case insensitivity, the actual DLL may be named `ãµral.dll` — i.e. to match the second library case — but the module name *must* be encoded as uppercase when [building the import
library](/blog/2021/05/31/). Alternatively the second could be `Õral.dll`, particularly because I won’t use it when constructing an import library.

The plan is to store the octets `C3 B5 …` in the import table. A process using CP-1252 decodes it to `Ãµral.dll`. In the UTF-8 code page it decodes to `õral.dll`. For testing we can use an [application manifest](/blog/2021/12/30/) to control the code page for a particular PE image — a lot easier than changing the system code page. Otherwise, this trick could dynamically change the behavior of a program in response to the system code page without actually inspecting the active code page.
...
I designed [`peports`](/blog/2024/06/30/) to print non-ASCII octets unambiguously ( `\xXX`), and it’s the only tool I know that does so:
...
If I [add a UTF-8 manifest](/blog/2021/12/30/), even just a “side-by-side” manifest, it loads the other library despite an identical import table:
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Windows%20dynamic%20linking%20depends%20on%20the%20active%20code%20page) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Windows+dynamic+linking+depends+on+the+active+code+page).

« [Slim Reader/Writer Locks are neato](/blog/2024/10/03/)

» [Everything I've learned so far about running local LLMs](/blog/2024/11/10/)
