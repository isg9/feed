...
url: https://nullprogram.com/blog/2017/01/30/
published: "2017-01-30T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2017/01/30/
---
...
nullprogram.com/blog/2017/01/30/

Not everything written in Emacs Lisp needs to be fast. Most of Emacs itself — around 82% — is written in Emacs Lisp *because* those parts are generally not performance-critical. Otherwise these functions would be built-ins written in C. Extensions to Emacs don’t have a choice and — outside of a few exceptions like [dynamic modules](/blog/2016/11/05/) and inferior processes — must be written in Emacs Lisp, including their performance-critical bits. Common performance hot spots are automatic indentation, [AST parsing](https://github.com/mooz/js2-mode), and [interactive
completion](/blog/2016/12/11/).
...
There’s one caveat: These guidelines are focused on Emacs 25.1 and “nearby” versions. Emacs is constantly evolving. Changes to the [virtual machine](/blog/2014/01/04/) and byte-code compiler may transform currently-slow expressions into fast code, obsoleting some of these guidelines. In the future I’ll add notes to this article for anything that changes.
...
Listen, I know you love [dash.el](https://github.com/magnars/dash.el) and higher order functions, but *this habit ain’t cheap*. The byte-code compiler does not know how to inline these lambdas, so there’s an additional per-element function call overhead.
...
How can you tell when a function has an assigned opcode? Take a peek at the `byte-defop` listings in [bytecomp.el](https://github.com/emacs-mirror/emacs/blob/master/lisp/emacs-lisp/bytecomp.el). Optimization often involves getting into the weeds, so don’t be shy.
...
I’ve had the most success with this approach by not by unrolling these loops myself, but by [using a macro](/blog/2016/12/27/), or [similar](/blog/2016/12/11/), to generate the unrolled form.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20How%20to%20Write%20Fast(er)%20Emacs%20Lisp) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=How+to+Write+Fast%28er%29+Emacs+Lisp).
