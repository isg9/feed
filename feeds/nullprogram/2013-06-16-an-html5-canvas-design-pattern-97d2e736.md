...
url: https://nullprogram.com/blog/2013/06/16/
published: "2013-06-16T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2013/06/16/
---
...
nullprogram.com/blog/2013/06/16/

I’ve been falling into a particular design pattern when using the HTML5 Canvas element. By “design pattern” I don’t mean some pretty arrangement but rather a [software design pattern](http://en.wikipedia.org/wiki/Software_design_pattern). This one’s a very Lisp-like pattern, and I wonder if I would have come up with it if I hadn’t first seen it in Lisp. It can also be applied to the Java 2D API, though less elegantly.
...
I think Canvas’s built-in stack is more elegant than managing an extraneous variable and object. However, what’s significant about Java 2D is that **we actually have access to the transform matrix**. It’s that [AffineTransform](http://docs.oracle.com/javase/7/docs/api/java/awt/geom/AffineTransform.html) object. The Canvas transform matrix is an internal, inaccessible data structure. It has an established external representation, [SVGMatrix](http://www.w3.org/TR/SVGTiny12/svgudom.html#svg__SVGMatrix), but it won’t provide a copy. If one of these is needed, a separate matrix must to be maintained in parallel. What a pain!
...
Since JavaScript has proper closures, that `step` variable is completely available to the callback. This function-as-body pattern comes up a lot (e.g. [AMD](http://requirejs.org/docs/whyamd.html)), and seeing it work so well makes me think of JavaScript as a “suitable Lisp.”
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20An%20HTML5%20Canvas%20Design%20Pattern) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=An+HTML5+Canvas+Design+Pattern).
