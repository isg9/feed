...
url: https://nullprogram.com/blog/2017/09/07/
published: "2017-09-07T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2017/09/07/
---
...
- Due to having contiguous content, saving a gap buffer is basically just two `write(2)` system calls. (Plus [`fsync(2)`, etc.](https://www.youtube.com/watch?v=LMe7hf2G1po))
...
This is close to [how Emacs represents it](http://git.savannah.gnu.org/cgit/emacs.git/tree/src/buffer.h?h=emacs-25.2#n425). In the structure above, the size of the content after the cursor isn’t tracked directly, but can be computed on the fly from the other three quantities. That is to say, this data structure is *normalized*.
...
A gap buffer certainly isn’t the only way to do it. For example, the original [vi used an array of lines](https://ecc-comp.blogspot.com/2015/05/a-brief-glance-at-how-5-text-editors.html), which sort of explains some of its quirky [line-oriented idioms](http://vimhelp.appspot.com/options.txt.html#'backspace'). The BSD clone of vi, nvi, [uses an entire database](https://en.wikipedia.org/wiki/Nvi) to represent buffers. Vim uses a fairly complex [rope](https://en.wikipedia.org/wiki/Rope_(data_structure))-like [data structure](https://github.com/vim/vim/blob/e723c42836d971180d1bf9f98916966c5543fff1/src/memline.c) with [page-oriented
blocks](http://www.free-soft.org/FSM/english/issue01/vim.html), which may be stored out-of-order in its swap file.

### Multiple cursors

[*Multiple cursors*](http://emacsrocks.com/e13.html) is fairly recent text editor invention that has gained a lot of popularity recent years. It seems every major editor either has the feature built in or a readily-available extension. I myself used Magnar Sveen’s [well-polished package](https://github.com/magnars/multiple-cursors.el) for several years. Though obviously the concept didn’t originate in Emacs or else it would have been called *multiple points*, which doesn’t quite roll off the tongue quite the same way.
...
However, as a result of [improving my typing skills](/blog/2017/04/01/), I’ve come to the conclusion that [multiple cursors is all hat and no
cattle](https://medium.com/@schtoeffel/you-don-t-need-more-than-one-cursor-in-vim-2c44117d51db). It doesn’t compose well with other editing commands, it doesn’t scale up to large operations, and it’s got all sorts of flaky edge cases (off-screen cursors). Nearly anything you can do with multiple cursors, you can do better with old, well-established editing paradigms.
...
- [https://github.com/skeeto/gap-buffer-animator](https://github.com/skeeto/gap-buffer-animator)

- [emacs](/tags/emacs/)
- [c](/tags/c/)
- [vim](/tags/vim/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Gap%20Buffers%20Are%20Not%20Optimized%20for%20Multiple%20Cursors) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Gap+Buffers+Are+Not+Optimized+for+Multiple+Cursors).
