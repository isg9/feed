...
url: https://nullprogram.com/blog/2015/10/30/
published: "2015-10-30T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2015/10/30/
---
...
Emacs comes with a wonderful arbitrary-precision computer algebra system called [calc](http://www.gnu.org/software/emacs/manual/html_mono/calc.html). I’ve [discussed it previously](/blog/2009/06/23/) and continue to use it on a daily basis. That’s right, people, *Emacs can* *do calculus*. Like everything Emacs, it’s programmable and extensible from Emacs Lisp. In this article, I’m going to implement the [RSA
public-key cryptosystem](https://en.wikipedia.org/wiki/RSA_(cryptosystem)) in Emacs Lisp using calc.
...
`/dev/random` for generating keys](http://www.2uo.de/myths-about-urandom/).)
...
The function `rsa--inverse` is just a straight Emacs Lisp + calc implementation of the extended Euclidean algorithm from [the Wikipedia
article pseudocode](https://en.wikipedia.org/wiki/Extended_Euclidean_algorithm), computing `d ≡ e^-1 (mod i)`. It’s not much use sharing it here, so take a look at the repository if you’re curious.
...
To compute signature `s` of an integer `m` (where `m < n`), compute `s ≡ m^d (mod n)`. I chose the right-to-left binary method, again straight from [the Wikipedia pseudocode](https://en.wikipedia.org/wiki/Modular_exponentiation#Right-to-left_binary_method) (lazy!). I’ll share this one since it’s short. The backslash denotes integer division.
...
Verifying the signature is the same process, but with the public key’s `e`: `m ≡ s^e (mod n)`. If the signature is valid, `m` will be recovered. In theory, only someone who knows `d` can feasibly compute `s` from `m`. If `n` is [small enough to factor](http://crypto.stackexchange.com/a/5942), revealing `p` and `q`, then `d` can be feasibly recomputed from the public key. So mind your Ps and Qs.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20RSA%20Signatures%20in%20Emacs%20Lisp) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=RSA+Signatures+in+Emacs+Lisp).

« [Counting Processor Cores in Emacs](/blog/2015/10/14/)
