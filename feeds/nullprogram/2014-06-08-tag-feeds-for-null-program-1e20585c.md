...
url: https://nullprogram.com/blog/2014/06/08/
published: "2014-06-08T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2014/06/08/
---
...
nullprogram.com/blog/2014/06/08/

I just added a [formal tags page](/tags/) along with individual feeds for each tag. I’ve had tags for a couple of years now, but they were really only useful for traveling sideways to similar articles. So now, if you’re only interested in a subset of my content, you can subscribe to one or more tags rather than the main Atom feed.

What prompted this? In [my *Emacs Chat*](/blog/2014/06/04/), Sacha asked me if this blog was part of [Planet Emacsen](http://planet.emacsen.org/) (currently, it’s not). If my tags are accurate, only about 25% of my articles are about Emacs, so most of my blog isn’t relevant there. Tag feeds will go a long way to help support these “planet” aggregators, should they want to include my articles. For example, Planet Emacsen would use [my Emacs
feed](/tags/emacs/feed/).

### Static Site Generation

I couldn’t practically support these extra feeds until recently. Remember, this blog [is statically generated](/blog/2011/08/05/). More feeds means more content to generate, because articles are duplicated in whole for each feed. In past years, Jekyll would probably take on the order of an hour to do all this for a single build. Fortunately, Jekyll has improved dramatically, especially in the past year or so, and these feeds have little impact on the total build time. It’s currently around 10 seconds or so. Not bad at all!
...
Unfortunately, [Elfeed](/blog/2013/09/04/) isn’t smart enough for this. Sorry! In order to better support all the broken RSS feeds out there, [I had to
compromise on entry keying](/blog/2013/09/09/). I couldn’t trust RSS feeds to provide me a reasonably unique key, so, transitively, Elfeed doesn’t fully trust Atom’s UUIDs either. These RSS feeds are broken largely because [RSS itself is a broken mess](/blog/2013/09/23/). When making new feeds in the future, please use Atom!
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Tag%20Feeds%20for%20null%20program) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Tag+Feeds+for+null+program).

« [Per Loop vs. Per Iteration Bindings](/blog/2014/06/06/)
