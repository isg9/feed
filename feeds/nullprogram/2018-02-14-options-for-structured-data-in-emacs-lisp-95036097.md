...
url: https://nullprogram.com/blog/2018/02/14/
published: "2018-02-14T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2018/02/14/
---
...
The other pitfall is related to printing. In Emacs 25 and earlier, types defined by `cl-defstruct` are still only types by convention. They’re really just vectors as far as Emacs Lisp is concerned. One benefit from this is that [printing and reading](/blog/2013/12/30/) these structures is “free” because vectors are printable. It’s trivial to serialize `cl-defstruct` structures out to a file. This is [exactly
how the Elfeed database works](/blog/2013/09/09/).
...
Emacs 26 throws a wrench in all this, though it’s worth it in the long run. There’s a new primitive type in Emacs 26 with its own reader syntax: records. This is similar to hash tables [becoming first class
in the reader in Emacs 23.2](/blog/2010/06/07/). In Emacs 26, `cl-defstruct` uses records instead of vectors.
...
Without either of these packages, the most direct way to build single dispatch on top of `cl-defstruct` would be to [shove a function in one
of the slots](/blog/2014/10/21/). Then the “method” is just a wrapper that call this function.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Options%20for%20Structured%20Data%20in%20Emacs%20Lisp) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Options+for+Structured+Data+in+Emacs+Lisp).
