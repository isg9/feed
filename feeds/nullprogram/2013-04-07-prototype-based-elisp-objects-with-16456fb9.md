...
url: https://nullprogram.com/blog/2013/04/07/
published: "2013-04-07T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2013/04/07/
---
...
nullprogram.com/blog/2013/04/07/
...
Last weekend I had the itch to play around with a multiple-inheritance prototype-based object system in lisp. It would [look a lot like JavaScript’s object system](http://steve-yegge.blogspot.com/2008/10/universal-design-pattern.html) but wanted to try experimenting some different ideas. My favorite lisp to hack in is Emacs Lisp, so that’s what I built it on. What I ended up with is actually pretty neat. Despite the lack of reader macros in Elisp, I still managed to introduce new syntax by manipulating symbols at compile time.

- [https://github.com/skeeto/at-el](https://github.com/skeeto/at-el)

See the README for a quick demonstration. What follows is the long explanation.

It’s called [@](https://github.com/skeeto/at-el), due to the syntax that it adds to Elisp as a domain-specific language. It’s a mini-language, really. The name is also a challenge to the code that supports Elisp, because so much of it — including emacs-lisp-mode and Paredit — doesn’t properly handle @ in identifiers. Even [Maruku](https://github.com/bhollis/maruku), the Markdown to HTML translator I use for this blog, has bugs that won’t allow it to handle the @ characters in my code, so I had to forgo most syntax highlighting for this post. (Update: I now use Kramdown so this is no longer an issue.)
...
I use this technique in the `@vector` class under `lib/` to expose the elements of the internal vector as if they were properties. [Brian](http://50ply.com/) used this trick to make a @buffer prototype that wraps Emacs’ buffers, with methods provided virtually by `:get`. For example, the `:string` property would return a lambda that calls `buffer-string`.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Prototype-based%20Elisp%20Objects%20with%20@) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Prototype-based+Elisp+Objects+with+%40).

« [Precise JavaScript Serialization with ResurrectJS](/blog/2013/03/28/)

» [Tracking Mobile Device Orientation with Emacs](/blog/2013/04/27/)
