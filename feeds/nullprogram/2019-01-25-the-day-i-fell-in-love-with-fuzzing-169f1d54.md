...
url: https://nullprogram.com/blog/2019/01/25/
published: "2019-01-25T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2019/01/25/
---

# The Day I Fell in Love with Fuzzing

## [The Day I Fell in Love with Fuzzing](/blog/2019/01/25/)

January 25, 2019

nullprogram.com/blog/2019/01/25/

*Follow-up: [Tips for more effective fuzz testing with AFL++](/blog/2025/02/05/)*

*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=19019048) and [on reddit](https://old.reddit.com/r/programming/comments/akrcyp/the_day_i_fell_in_love_with_fuzzing/).*

In 2007 I wrote a pair of modding tools, [binitools](https://github.com/skeeto/binitools), for a space trading and combat simulation game named [*Freelancer*](https://en.wikipedia.org/wiki/Freelancer_(video_game)). The game stores its non-art assets in the format of “binary INI” files, or “BINI” files. The motivation for the binary format over traditional INI files was probably performance: it’s faster to load and read these files than it is to parse arbitrary text in INI format.
...
I didn’t reverse engineer the BINI format, nor was I the first person the create tools to edit them. The existing tools weren’t to my tastes, and I had my own vision for how they should work — an interface more closely following [the Unix tradition](http://www.catb.org/esr/writings/taoup/html/) despite the target being a Windows game.

When I got started, I had just learned how to use [yacc](http://pubs.opengroup.org/onlinepubs/9699919799/utilities/yacc.html) (really [Bison](https://www.gnu.org/software/bison/)) and [lex](http://pubs.opengroup.org/onlinepubs/9699919799/utilities/lex.html) (really [flex](https://github.com/westes/flex)), as well as Autoconf, so I went all-in with these newly-discovered tools. It was exciting to try them out in a real-world situation, though I slavishly aped the practices of other open source projects without really understanding why things were they way they were. Due to the use of yacc/lex and the configure script build, compiling the project required a full, Unix-like environment. This is all visible in [the original
version of the source](https://github.com/skeeto/binitools/tree/original).
...
[I like to keep things simple](/blog/2017/03/30/) which meant no more Autoconf, and instead [a simple, portable Makefile](/blog/2017/08/20/). No more yacc or lex, and instead a hand-coded parser. Using only conforming, portable C. The result was so simple that I can [build using Visual Studio](/blog/2016/06/13/) in a single, short command, so the Makefile isn’t all that necessary. With one small tweak (replace `stdint.h` with a `typedef`), I can even [build
and run binitools in DOS](/blog/2018/04/13/).
...
I’ve been interested in [fuzzing](https://labs.mwrinfosecurity.com/blog/what-the-fuzz/) for years, especially [american fuzzy lop](http://lcamtuf.coredump.cx/afl/), or *afl*. However, I wasn’t having success with it. I’d fuzz some of the tools I use regularly, and it wouldn’t find anything of note, at least not before I gave up. I fuzzed [my
JSON library](https://github.com/skeeto/pdjson), and somehow it turned up nothing. Surely my JSON parser couldn’t be *that* robust already, could it? Fuzzing just wasn’t accomplishing anything for me. (As it turns out, my JSON library *is* quite robust, thanks in large part to various contributors!)
...
In my example above, I swapped out the default compiler for afl’s GCC wrapper ( `CC=afl-gcc`). It calls GCC in the background, but in doing so adds its own instrumentation to the binary. When fuzzing, `afl-fuzz` uses that instrumentation to monitor the program’s execution path. The [afl whitepaper](http://lcamtuf.coredump.cx/afl/technical_details.txt) explains the technical details.
...
The most exciting and dreaded result is a crash. The first time I ran it against binitools, `bini` had *many* such crashes. Within minutes, afl was finding a number of subtle and interesting bugs in my program, which was *incredibly* useful. It even discovered an unlikely [stale pointer
bug](https://github.com/skeeto/binitools/commit/b695aec7d0021299cbd83c8c6983055f16d11507) by exercising different orderings for various memory allocations. This particular bug was the turning point that made me realize the value of fuzzing.
...
I sorted the valid from invalid inputs and checked them into the repository. Have a look at all the wacky inputs [invented by](https://lcamtuf.blogspot.com/2014/11/pulling-jpegs-out-of-thin-air.html) the fuzzer starting from my single, minimal input:

- [valid inputs](https://github.com/skeeto/binitools/tree/master/tests/valid)
- [invalid inputs](https://github.com/skeeto/binitools/tree/master/tests/invalid)
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20The%20Day%20I%20Fell%20in%20Love%20with%20Fuzzing) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=The+Day+I+Fell+in+Love+with+Fuzzing).

« [A JavaScript Typed Array Gotcha](/blog/2019/01/23/)
