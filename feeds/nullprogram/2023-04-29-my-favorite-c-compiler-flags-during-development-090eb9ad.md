...
feed: nullprogram
guid: https://nullprogram.com/blog/2023/04/29/
---
...
On 64-bit targets, it will warn about truncating the 64-bit `len` for the 32-bit parameter. To dismiss the warning, you must either address it by using a loop to [call `WriteFile` multiple times](/blog/2023/02/13/), or acknowledge the truncation with an explicit cast and accept the consequences. In this case I may know from context it’s impossible for the program to even construct such a large message, so I’d use an assertion and truncate.
...
The catch is that the above warning only happens for 64-bit targets. So you might miss it. The inverse is true in other cases. This is one area where [cross-architecture testing](/blog/2021/08/21/) can pay off.
...
For me, the most useful UBSan check is signed overflow — e.g. computing the wrong result — and it’s instrumentation I miss when not working in C. In programs where this might be an issue, combine it [with a fuzzer](/blog/2019/01/25/) to search for inputs that cause overflows. This is yet another argument in favor of [signed sizes](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2019/p1428r0.pdf), as UBSan can detect such overflows. (Yes, UBSan optionally instruments unsigned overflow, too, but then you must somehow distinguish [intentional](/blog/2019/11/19/) from [unintentional](/blog/2017/07/19/) overflow.)

On Linux, ASan and UBSan strangely do not have [debugger-oriented
defaults](/blog/2022/06/26/). Fortunately that’s easy to address with a couple of environment variables, which cause them to break on error instead of uselessly exiting:
...
As of this writing, MSVC does not have UBSan, but it does have a similar feature, [run-time error checks](https://learn.microsoft.com/en-us/cpp/build/reference/rtc-run-time-error-checks). Three sub-flags ( `c`, `s`, `u`) enable different checks, and **`/RTCcsu`** turns them all on. The `c` flag generates the assertion I had manually written with `-﻿Wconversion`, and traps any truncation at run time. There’s nothing quite like this in UBSan! It’s so extreme that it’s compatible with neither standard runtime libraries (fortunately [not a big deal](/blog/2023/02/11/)) nor with ASan.
...
Unfortunately it only works in a narrow context. The target must use pthreads, not C11 threads, OpenMP, nor [direct cloning](/blog/2023/03/23/). It must only synchronize through code that was compiled with TSan. That means no synchronization [through system calls](/blog/2022/10/03/), especially no futexes. Most non-trivial programs do not meet the criteria.
...
(Short term note: GCC 11, released in March 2021, switched to DWARF5 by default. However, GDB could not access the extra `-﻿g3` debug information in DWARF5 until GDB 13, released February 2023. If you have a toolchain from that two year window — except [mine](https://github.com/skeeto/w64devkit) because I patched it — then you may also need `-﻿gdwarf-4` to switch back to DWARF4.)
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20My%20favorite%20C%20compiler%20flags%20during%20development) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=My+favorite+C+compiler+flags+during+development).
