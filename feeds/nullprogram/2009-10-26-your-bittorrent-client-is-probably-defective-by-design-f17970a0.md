...
url: https://nullprogram.com/blog/2009/10/26/
published: "2009-10-26T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2009/10/26/
---
...
nullprogram.com/blog/2009/10/26/
...
The private flag wasn't placed in front with the rest of the metadata where it belonged. It's intentionally placed at the end of the torrent file inside of the info section. This means that the flag is part of the info\_hash property of the torrent, which is the global identifier for the torrent. Unset or remove the private flag and the hash changes, creating a whole new torrent without any seeds.
...
I went on the Debian IRC channel and brought up the issue only to find out that everyone thought a little DRM was reasonable. So then I [filed
a bug report on it](http://bugs.debian.org/cgi-bin/bugreport.cgi?bug=549607), which was simply closed [citing
that the DRM is a beneficial "feature"](http://bugs.debian.org/cgi-bin/bugreport.cgi?bug=549607#10) and that removing the intentional defect would make the clients "poorer". They also insisted that it's part of the spec [when it's
not](http://wiki.theory.org/BitTorrentSpecification). I'm really disappointed in Debian now.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Your%20BitTorrent%20Client%20is%20Probably%20Defective%20by%20Design) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Your+BitTorrent+Client+is+Probably+Defective+by+Design).
