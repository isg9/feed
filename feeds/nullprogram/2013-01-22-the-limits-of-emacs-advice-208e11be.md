...
url: https://nullprogram.com/blog/2013/01/22/
published: "2013-01-22T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2013/01/22/
---

# The Limits of Emacs Advice

## [The Limits of Emacs Advice](/blog/2013/01/22/)

January 22, 2013

nullprogram.com/blog/2013/01/22/

Today at work I was using [impatient-mode](http://www.50ply.com/blog/2012/08/13/introducing-impatient-mode/) to share some code with [Brian](http://www.50ply.com/). It makes for a really handy live pastebin. To limit the buffer to the relevant code, I narrowed it down with `narrow-to-region`. However, the browser wouldn’t update to show only the narrowed region until I made an edit. This makes sense because impatient-mode hooks `after-change-functions`. Narrowing the buffer doesn’t *change* anything in the buffer, so, as expected, this hook is not called.

The solution would be to also join whatever hook is called when the buffer restriction changes. Unfortunately, [no such hook exists](http://www.gnu.org/software/emacs/manual/html_node/elisp/Standard-Hooks.html). I thought I could create this hook with some [advice](http://www.gnu.org/software/emacs/manual/html_node/elisp/Advising-Functions.html), but this turns out to be currently impossible.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20The%20Limits%20of%20Emacs%20Advice) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=The+Limits+of+Emacs+Advice).
