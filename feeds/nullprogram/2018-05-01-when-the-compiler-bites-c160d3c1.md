...
url: https://nullprogram.com/blog/2018/05/01/
published: "2018-05-01T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2018/05/01/
---
...
*Update: There are discussions [on Reddit](https://old.reddit.com/r/cpp/comments/8gfhq3/when_the_compiler_bites/) and [on Hacker\_ _News](https://news.ycombinator.com/item?id=16974770).*
...
The `-ffast-math` option relaxes the C standard floating point rules, permitting an optimization at the cost of conformance and [consistency](https://possiblywrong.wordpress.com/2017/09/12/floating-point-agreement-between-matlab-and-c/):
...
Side note: `-ffast-math` doesn’t necessarily mean “less precise.” Sometimes it will actually [improve precision](https://en.wikipedia.org/wiki/Multiply–accumulate_operation#Fused_multiply–add).
...
It turns out that the [x86-64 ABI](https://www.uclibc.org/docs/psABI-x86_64.pdf) didn’t specify what happens with the upper bits in argument registers. Are they garbage? Are they zeroed? GCC takes the conservative position of assuming the upper bits are arbitrary garbage. Clang takes the boldest position of assuming arguments smaller than 32 bits have been promoted to 32 bits by the caller. This is what the ABI specification *should* have said, but currently it does not.

Fortunately GCC also conservative when passing arguments. It promotes arguments to 32 bits as necessary, so there are no conflicts when linking against Clang-compiled code. However, this is not true for Intel’s ICC compiler: [**Clang and ICC are not ABI-compatible on**
**x86-64**](https://web.archive.org/web/20180908113552/https://stackoverflow.com/a/36760539).
...
So what’s going on here? The original ANSI C specification wasn’t clear about how intermediate floating point values get rounded, and implementations [all did it differently](https://news.ycombinator.com/item?id=16974770). The C99 specification cleaned this all up and introduced [`FLT_EVAL_METHOD`](https://en.wikipedia.org/wiki/C99#IEEE_754_floating_point_support). Implementations can still differ, but at least you can now determine at compile-time what the compiler would do by inspecting that macro.
...
The remnants of this high precision trend are still in JavaScript, where all arithmetic is double precision (even if [simulated using
integers](http://thibaultlaurens.github.io/javascript/2013/04/29/how-the-v8-engine-works/#more-example-on-how-v8-optimized-javascript-code)), and great pains have been made [to work around](https://blog.mozilla.org/javascript/2013/11/07/efficient-float32-arithmetic-in-javascript/) the performance consequences of this. [Until recently](http://tirania.org/blog/archive/2018/Apr-11.html), Mono had similar issues.

The trend reversed once SIMD hardware became widely available and there were huge performance gains to be had. Multiple values could be computed at once, side by side, at lower precision. So on x86-64, this became the default ( `FLT_EVAL_METHOD == 0`). The young Clang compiler wasn’t around until well after this trend reversed, so it behaves differently than the [backwards compatible](https://gcc.gnu.org/bugzilla/show_bug.cgi?id=323) GCC on the old x86.
...
I’ve saved this one for last since it’s my favorite. Suppose we have this little function, `new_image()`, that allocates a greyscale image for, say, [some multimedia library](/blog/2017/11/03/).
...
header library](https://github.com/nothings/stb) (and, secretly, because it’s necessary for illustrating the issue). Being a responsible citizen, the function even [checks for integer overflow](/blog/2017/07/19/) before allocating anything.
...
I soon realized I can take this further and trick Clang into performing an invalid optimization, [revealing a bug](https://bugs.llvm.org/show_bug.cgi?id=37304). Consider this slightly-optimized version that uses `calloc()` when the shade is zero (black). The `calloc()` function does its own overflow check, so `new_image()` doesn’t need to do it.
...
With this change, my overflow unit test is now also failing. The situation is even worse than before. The `calloc()` is being eliminated *despite the overflow*, and replaced with a simulated success. This time it’s actually a bug in Clang. While failing a unit test is mostly harmless, **this could introduce a vulnerability in a** **real program**. The OpenBSD folks are so worried about this sort of thing that [they’ve disabled this optimization](https://marc.info/?l=openbsd-cvs&m=150125592126437&w=2).
...
sophisticated](https://github.com/skeeto/growable-buf) than what was presented here, gave me artificial intelligence vibes. It’s that situation where a computer algorithm did something really clever and I felt it outsmarted me. It’s creepy to consider [how far that can go](https://wiki.lesswrong.com/wiki/Paperclip_maximizer). I’ve gotten that even from observing [AI I’ve written myself](/blog/2017/04/27/), and I know for sure no human taught it some particularly clever trick.

My favorite AI story along these lines is about [an AI that learned
how to play games on the Nintendo Entertainment System](https://www.youtube.com/watch?v=xOCurBYI_gY). It didn’t understand the games it was playing. It’s optimization task was simply to choose controller inputs that maximized memory values, because that’s generally associated with doing well — higher scores, more progress, etc. The most unexpected part came when playing Tetris. Eventually the screen would fill up with blocks, and the AI would face the inevitable situation of losing the game, with all that memory being reinitialized to low values. So what did it do?
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20When%20the%20Compiler%20Bites) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=When+the+Compiler+Bites).
