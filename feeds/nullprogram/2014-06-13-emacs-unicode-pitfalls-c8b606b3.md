...
url: https://nullprogram.com/blog/2014/06/13/
published: "2014-06-13T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2014/06/13/
---
...
nullprogram.com/blog/2014/06/13/
...
First, there are excellent materials online for learning Unicode. I recommend starting with [UTF-8 and Unicode FAQ for Unix/Linux](http://www.cl.cam.ac.uk/~mgk25/unicode.html). There’s no reason for me to repeat all this information here, but I’ll attempt to quickly summarize it.
...
UTF-16 encodes all the characters from the *Basic Multilingual Plane* (BMP) with two bytes. Even the original ASCII characters get two bytes ( *16* bits). The BMP covers virtually all modern languages and is generally all you’ll ever practically need. However, this doesn’t include the important [TROPICAL DRINK](http://www.fileformat.info/info/unicode/char/1f379/index.htm) or [PILE OF POO](http://www.fileformat.info/info/unicode/char/1F4A9/index.htm) characters from the supplemental (“astral”) plane. If you need to use these characters in UTF-16, you’re going to run into problems: characters outside the BMP don’t fit in two bytes. To accommodate these characters, UTF-16 uses *surrogate pairs*: these characters are encoded with two 16-bit units.

Because of this last point, **UTF-16 offers no practical advantages** **over UTF-8**. Its [existence was probably a big mistake](http://www.utf8everywhere.org/). You can’t do constant-time character lookup because you have to scan for surrogate pairs. It’s not backwards compatible and cannot be stored in null-terminated strings. In both Java and JavaScript, it leads to the awkward situation where the “length” of a string is not the number of characters, code points, or even bytes. Worst of all, [it has serious
security implications](https://speakerdeck.com/mathiasbynens/hacking-with-unicode?slide=114). New applications should avoid it whenever possible.
...
There are two modes for this in Emacs: unibyte and multibyte. Unibyte strings/buffers are just raw bytes. They have constant access O(1) time but can only hold single-byte values. The [byte-code compiler
outputs unibyte strings](/blog/2014/01/04/).
...
Pitfall #4: **[Reversing Unicode strings is non-trivial](https://github.com/mathiasbynens/esrever).** The [Rosetta Code](http://rosettacode.org/wiki/Reverse_a_string) page is full of incorrect examples, and [I’m personally guilty](/blog/2012/11/15/) of this, too. The other day I [submitted a patch to s.el](https://github.com/magnars/s.el/pull/58) to correct its `s-reverse` function for Unicode. If it’s accepted, you should never need to worry about this.
...
- [java.lang.Character Unicode Character Representations](http://docs.oracle.com/javase/7/docs/api/java/lang/Character.html#unicode)
- [GNU Emacs Lisp Reference Manual: Strings and Characters](http://www.gnu.org/software/emacs/manual/html_node/elisp/Strings-and-Characters.html)
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Emacs%20Unicode%20Pitfalls) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Emacs+Unicode+Pitfalls).
