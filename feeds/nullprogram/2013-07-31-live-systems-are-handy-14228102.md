...
url: https://nullprogram.com/blog/2013/07/31/
published: "2013-07-31T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2013/07/31/
---
...
nullprogram.com/blog/2013/07/31/

The [personal live system](/blog/2013/06/17/) I configured a couple months ago has turned out to be *quite* handy. I’ve used it almost every day, especially as I’ve learned more about the features of live-boot and live-config, the runtime components of [live-build](http://live.debian.net/).
...
For awhile these systems just had a base Ubuntu install from a couple of years ago. This works fine, of course, but it was missing [all of my dotfiles](/blog/2012/06/23/). It didn’t feel like home. I could put my dotfiles on these systems, but there are a number of these machines and I’d have to keep my dotfiles up to date on each, without any Internet access, as they changed.
...
These live systems use an interesting filesystem called [SquashFS](http://en.wikipedia.org/wiki/SquashFS). It’s like a compressed tar archive, but it has two significant advantages:
...
If the live system is installed on a thumb drive, the space *after* the image can be used for this persistence. After `dd` ing the ISO, use fdisk/parted to add a second partition, format a fresh partition labeled “persistence” in it, then drop in config file.
...
Furthermore, the persistence partition can be encrypted with LUKS, providing a portable, persistent, encrypted operating system. The live-config will automatically find it, prompt for the passphrase on boot, and handle all the rest on its own like normal. (Well, once [this bug fix is released](http://bugs.debian.org/cgi-bin/bugreport.cgi?bug=700902).)
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Live%20Systems%20are%20Handy) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Live+Systems+are+Handy).

« [Liquid Simulation in WebGL](/blog/2013/06/26/)

» [Coining Autoism](/blog/2013/08/16/)
