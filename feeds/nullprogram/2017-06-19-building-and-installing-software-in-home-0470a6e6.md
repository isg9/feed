...
url: https://nullprogram.com/blog/2017/06/19/
published: "2017-06-19T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2017/06/19/
---
...
nullprogram.com/blog/2017/06/19/
...
In the most desperate situation, all I need from the sysadmin is a decent C compiler and at least a minimal POSIX environment. I can [bootstrap anything I might need](/blog/2016/11/17/), both libraries and programs, including a better C compiler along the way. This is one major strength of open source software.
...
Another example is NetHack, which I prefer to play with a couple of custom patches ( [Menucolors](https://bilious.alt.org/?11), [wchar](https://gist.github.com/skeeto/11fed852dbfe9889a5fce80e9f6576ac)). The install to `$HOME/.local` [is also captured as a patch](https://gist.github.com/skeeto/5cb9d5e774ce62655aff3507cb806981).
...
[Mutt](/blog/2017/06/15/) is in a similar situation, since it wants to install a special setgid program ( `mutt_dotlock`) that synchronizes mailbox access. All MUAs need something like this.
...
1. The [crude, easy way](https://web.archive.org/web/20090312014334/http://blogs.sun.com/rie/entry/tt_ld_library_path_tt): `LD_LIBRARY_PATH`.
...
Personally, I always use the [Worse is Better](https://www.jwz.org/doc/worse-is-better.html) `LD_LIBRARY_PATH` shotgun. Occasionally it’s caused some annoying issues, but the vast majority of the time it gets the job done with little fuss. This is just my personal development environment, after all, not a production server.
...
While I’ve [poo-pooed autoconf](/blog/2017/03/30/) in the past, the standard `configure` script usually makes it trivial to build and install software in $HOME. The key ingredient is the `--prefix` option:
...
For [CMake](https://cmake.org/), there’s `CMAKE_INSTALL_PREFIX`:
...
Some software is just a single, static, standalone binary with [everything baked in](/blog/2016/11/15/). It doesn’t need to be given a prefix, and installation is as simple as copying the binary into place. For example, [Enchive](https://github.com/skeeto/enchive) works like this:
...
Some libraries aren’t so smart about `pkg-config` and need some handholding — for example, [ncurses](https://www.gnu.org/software/ncurses/). I mention it because it’s required for both Vim and Emacs, among many others, so I’m often building it myself. It ignores `--prefix` and needs to be told a second time where to install things:
...
I could go on and on like this, discussing the quirks for the various libraries and programs that I use. Over the years I’ve gotten used to many of these issues, committing the solutions to memory. Unfortunately, even within the same version of a piece of software, the quirks can change [between major operating system
releases](https://www.debian.org/News/2017/20170617.en.html), so I’m continuously learning my way around new issues. It’s really given me an appreciation for all the hard work that package maintainers put into customizing and maintaining software builds to [fit properly into a larger ecosystem](https://www.debian.org/doc/manuals/maint-guide/).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Building%20and%20Installing%20Software%20in%20$HOME) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Building+and+Installing+Software+in+%24HOME).

« [Switching to the Mutt Email Client](/blog/2017/06/15/)

» [Stack Clashing for Fun and Profit](/blog/2017/06/21/)
