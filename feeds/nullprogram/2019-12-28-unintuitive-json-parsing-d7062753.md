...
url: https://nullprogram.com/blog/2019/12/28/
published: "2019-12-28T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2019/12/28/
---
...
*This article was discussed [on Hacker News](https://news.ycombinator.com/item?id=21900715) and [on reddit](https://old.reddit.com/r/programming/comments/egvq11/unintuitive_json_parsing/).*

Despite the goal of JSON being a subset of JavaScript — which [it failed
to achieve](http://seriot.ch/parsing_json.php) (update: [this was fixed](https://github.com/tc39/proposal-json-superset)) — parsing JSON is quite unlike parsing a programming language. For invalid inputs, the specific cause of error is often counter-intuitive. Normally this doesn’t matter, but I recently [ran into a case where it does](https://github.com/skeeto/pdjson/pull/19/commits/1500ca73f2ed44ed8a6129fd1fa164bd7e326874#diff-eb030bc5ad128fc13160acab7d06f3a0R702).
...
However, JSON does not support leading zeros, neither ignoring them nor supporting octal notation. Here’s the railroad diagram for numbers [from
the JSON specficaiton](https://www.json.org/json-en.html):
...
[My JSON parser](https://github.com/skeeto/pdjson) comes with a testing tool that shows the token stream up until the parser rejects the input, useful for understanding these situations:
...
However, it *does* come up with concatenated JSON. Some parsers, including mine, will optionally consume multiple JSON values, one after another, from the same input. Here’s an example from [one of my
favorite](/blog/2016/09/15/) command line tools, [jq](https://stedolan.github.io/jq/):
...
- [Jackson](https://github.com/FasterXML/jackson): Reject as leading zero.
- [Noggit](https://github.com/yonik/noggit): Reject as leading zero.
- [yajl](https://lloyd.github.io/yajl/): Accept as two numbers.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Unintuitive%20JSON%20Parsing) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Unintuitive+JSON+Parsing).

« [Efficient Alias of a Built-In Emacs Lisp Function](/blog/2019/12/10/)
