...
url: https://nullprogram.com/blog/2014/03/06/
published: "2014-03-06T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2014/03/06/
---

# The Julia Programming Language

## [The Julia Programming Language](/blog/2014/03/06/)

March 06, 2014

nullprogram.com/blog/2014/03/06/
...
Speaking strictly in terms of the language, doing better than Matlab isn’t really a significant feat. Among major programming languages, [Matlab’s awfulness and bad design](/blog/2008/08/29/) is [second only to PHP](http://old.reddit.com/r/lolphp). Octave fixes a lot of the Matlab language, but it can only go so far.
...
If you’re curious about learning more, the [Julia manual](http://julia.readthedocs.org/en/latest/manual/) covers the entire language as it currently exists. Unfortunately anything outside the language proper and its standard library is under-documented at this time.
...
Julia has avoided what some call [The Billion Dollar Mistake](http://yinwang0.wordpress.com/2013/06/03/null/): null references. In languages such as Java, `null` is allowed in place of any object of any type. This allowance has lead to many run-time bugs that, if `null` didn’t exist, would have been caught at compile time.
...
(Note that to write this method correctly, either `Point` or the method should probably [promote](http://julia.readthedocs.org/en/latest/manual/conversion-and-promotion/) its arguments.)
...
This is absolutely essential to [interactive development](/blog/2012/10/31/). The lack of this makes Julia *far* less dynamic than it should be. Combined with the lack of a printer, **Julia is not currently suitable** **as an interactive interpreter subprocess** (Slime, Cider, Skewer, etc.).
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20The%20Julia%20Programming%20Language) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=The+Julia+Programming+Language).
