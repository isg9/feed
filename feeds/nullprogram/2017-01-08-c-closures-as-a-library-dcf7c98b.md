...
url: https://nullprogram.com/blog/2017/01/08/
published: "2017-01-08T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2017/01/08/
---
...
nullprogram.com/blog/2017/01/08/
...
A problem with these functions is that there’s no way to pass context to the callback. The callback may need information beyond the two element pointers when making its decision, or to [update a
result](/blog/2016/09/05/). For example, suppose I have a structure representing a two-dimensional coordinate, and a coordinate distance function.
...
Potential problems are that it’s neither thread-safe nor re-entrant. Two different threads cannot use this comparator [at the same
time](/blog/2014/10/12/). Also, on some platforms and configurations, repeatedly accessing a global variable in a comparator [may have a significant
cost](/blog/2016/12/23/). A common workaround for thread safety is to make the global variable thread-local by allocating it in thread-local storage (TLS):
...
In `closure_create()`, allocation is done with an anonymous `mmap()`, just like in [my JIT compiler](/blog/2015/03/19/). It’s initially mapped writable in order to copy the thunk, then the thunk page is set to executable.
...
- [closure-demo.c](/download/closure-demo.c)
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20C%20Closures%20as%20a%20Library) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=C+Closures+as+a+Library).
