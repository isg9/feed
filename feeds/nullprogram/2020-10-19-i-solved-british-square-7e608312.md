...
url: https://nullprogram.com/blog/2020/10/19/
published: "2020-10-19T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2020/10/19/
---
...
*Update*: I [solved another game](/blog/2022/10/12/) using essentially the same technique.

[British Square](https://boardgamegeek.com/boardgame/3719/british-square) is a 1978 abstract strategy board game which I recently discovered [from a YouTube video](https://www.youtube.com/watch?v=PChKZbut3lM&t=10m). It’s well-suited to play by pencil-and-paper, so my wife and I played a few rounds to try it out. Curious about strategies, I searched online for analysis and found nothing whatsoever, meaning I’d have to discover strategies for myself. This is *exactly* the sort of problem that [nerd snipes](https://xkcd.com/356/), and so I sunk a couple of evenings building an analysis engine in C — enough to fully solve the game and play *perfectly*.

**Repository**: [**British Square Analysis Engine**](https://github.com/skeeto/british-square) (and [prebuilt binaries](https://github.com/skeeto/british-square/releases))
...
As [I’ve done before](/blog/2017/04/27/), my engine represents the game using [bitboards](https://www.chessprogramming.org/Bitboards). Each player has a 25-bit bitboard representing their pieces. To make move validation more efficient, it also sometimes tracks a “mask” bitboard where invalid moves have been masked. Updating all bitboards is cheap ( `place()`, `mask()`), as is validating moves against the mask ( `valid()`).
...
When [I’ve used minimax before](/blog/2010/10/17/), I built an actual tree data structure in memory, linking states by pointer / reference. In this engine there is no such linkage, and instead the links are computed dynamically via the validation masks. Storing the pointers is more expensive than computing their equivalents on the fly, *so I don’t store* *them*. Therefore my game tree only requires 56 bits per node — or 64 bits in practice since I’m using a 64-bit integer. With only 8,659,987 nodes to store, that’s a mere 66MiB of memory! This analysis could have easily been done on commodity hardware two decades ago.
...
Once I’ve computed a result, I store it in a hash table so that I can find it later. Since the state is just a 64-bit integer, I use [an
integer hash function](/blog/2018/07/31/) to compute a starting index from which to linearly probe an open addressing hash table. The *entire* hash table implementation is literally a dozen lines of code:
...
I only need two operations to visit all 8 symmetries, and I chose transpose (flip around the diagonal) and vertical flip. Alternating between these operations visits each symmetry. Since they’re bitboards, transforms can be implemented using [fancy bit-twiddling hacks](https://www.chessprogramming.org/Flipping_Mirroring_and_Rotating). Chess boards, with their power-of-two dimensions, have useful properties which these British Square boards lack, so this is the best I could come up with:
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20I%20Solved%20British%20Square) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=I+Solved+British+Square).
