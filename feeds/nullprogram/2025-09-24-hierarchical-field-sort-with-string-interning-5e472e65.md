...
url: https://nullprogram.com/blog/2025/09/24/
published: "2025-09-24T00:00:00Z"
feed: nullprogram
guid: https://nullprogram.com/blog/2025/09/24/
---
...
Because [the standard library has little useful for us](/blog/2023/02/11/), I am building on [**previously-established definitions**](/blog/2025/01/19/), so refer to that article for basic definitions like `Str`. To start off, tokens will be a size-typed integer so we never need to worry about overflowing the token counter. We’d run out of memory first:
...
The map itself is a forever-useful [hash trie](/blog/2023/09/30/).
...
We’ll use this map to convert a string naming a field into a sequence of tokens, so we’ll [need a slice](/blog/2025/06/26/). Fields also have an offset within the record and a type, which we’ll track via its original ordering, which I’ll do with an `index` field (e.g. into the original header). Also track the original name.
...
Finally down to business: [cut up the list](/blog/2025/03/02/) and build the token sequences with the established `push` macro. The sort function isn’t interesting, and could be as simple as libc `qsort` with the above comparator (and adapter), so I’m only listing the prototype.
...
Seeing it written out, I notice more room for improvement. An optimization pass could coalesce instructions so that, for instance, `OPEN` then `KEY` [concatenate](/blog/2024/05/25/) to a single string at compile time so that it only needs one instruction. This program could be 15 instructions instead of 31. In my real case I didn’t need anything quite this sophisticated, but it was fun to explore.

- [c](/tags/c/)

Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Hierarchical%20field%20sort%20with%20string%20interning) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Hierarchical+field+sort+with+string+interning).
