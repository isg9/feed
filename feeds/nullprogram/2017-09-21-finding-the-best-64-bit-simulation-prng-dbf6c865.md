...
feed: nullprogram
guid: https://nullprogram.com/blog/2017/09/21/
---

# Finding the Best 64-bit Simulation PRNG

## [Finding the Best 64-bit Simulation PRNG](/blog/2017/09/21/)

September 21, 2017

nullprogram.com/blog/2017/09/21/

**August 2018 Update**: *xoroshiro128+ fails [PractRand](http://pracrand.sourceforge.net/) very* *badly. Since this article was published, its authors have supplanted it* *with **xoshiro256\*\***. It has essentially the same performance, but* *better statistical properties. xoshiro256\*\* is now my preferred PRNG.*
...
- **Monte Carlo simulations**, where PRNGs are used to [compute
  numeric estimates](https://possiblywrong.wordpress.com/2015/09/15/kanoodle-iq-fit-and-dancing-links/) for problems that are difficult or impossible to solve analytically.

- [**Monte Carlo tree search AI**](/blog/2017/04/27/), where massive numbers of games are played out randomly in search of an optimal move. This is a specific application of the last item.

- [**Genetic algorithms**](https://github.com/skeeto/carpet-fractal-genetics), where a PRNG creates the initial population, and then later guides in mutation and breeding of selected solutions.

- [**Cryptography**](https://blog.cr.yp.to/20140205-entropy.html), where a cryptographically-secure PRNGs (CSPRNGs) produce output that is predictable for recipients who know a particular secret, but not for anyone else. This article is only concerned with plain PRNGs.
...
2. PRNG output should have robust statistical qualities. Bits should appear to be independent and the output should closely follow the desired distribution. Poor quality output will negatively effect the algorithms using it. Also just as important is [how you use
   it](http://mumble.net/~campbell/2014/04/28/uniform-random-float), but this article will focus only on generating bits.
...
My preference is to BYOPRNG: *Bring Your Own Pseudo-random Number* *Generator*. You get reliable, identical output everywhere. Also, in the case of C and C++ — and if you do it right — by embedding the PRNG in your project, it will get inlined and unrolled, making it far more efficient than a [slow call into a dynamic library](/blog/2016/10/27/).
...
For the past few years my main go to for a drop-in PRNG has been [xorshift\*](https://en.wikipedia.org/wiki/Xorshift). The body of the function is 6 lines of C, and its entire state is a 64-bit integer, directly seeded. However, there are a number of choices here, including other variants of Xorshift. How do I know which one is best? The only way to know is to test it, hence my 64-bit PRNG shootout:

- [**64-bit PRNG Shootout**](https://github.com/skeeto/prng64-shootout)

Sure, there [are other such shootouts](http://xoroshiro.di.unimi.it/), but they’re all missing something I want to measure. I also want to test in an environment very close to how I’d use these PRNGs myself.
...
**The clear winner is [xoroshiro128+](http://xoroshiro.di.unimi.it/)**, with a function body of just 7 lines of C. It’s clearly the fastest, and the output had no observed statistical failures. However, that’s not the whole story. A couple of the other PRNGS have advantages that situationally makes them better suited than xoroshiro128+. I’ll go over these in the discussion below.
...
To measure the statistical quality of each PRNG — mostly as a sanity check — the raw binary output was run through [dieharder](http://webhome.phy.duke.edu/~rgb/General/dieharder.php) 3.31.1:
...
There’s not much information to glean from this part of the shootout. It mostly confirms that all of these PRNGs would work fine for simulation purposes. The WEAK results are not very significant and is only useful for breaking ties. Even a true RNG will get some WEAK results. For example, the [x86 RDRAND](https://en.wikipedia.org/wiki/RdRand) instruction (not included in actual shootout) got 7 WEAK results in my tests.
...
Admittedly the definition for “64-bit PRNG” is rather vague. My high performance targets are all 64-bit platforms, so the highest PRNG throughput will be built on 64-bit operations ( [if not wider](/blog/2015/07/10/)). The original plan was to focus on PRNGs built from 64-bit operations.
...
Chaining](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#CBC) (CBC) versus [Counter](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Counter_.28CTR.29) (CTR) mode. In CBC mode it encrypts zeros as plaintext. This just means it’s encrypting its last output. The ciphertext is the PRNG’s output.
...
In CBC mode, each output depends on the previous, but in CTR mode all blocks are independent. Work can begin on the next output before the previous output is complete. The x86 architecture uses out-of-order execution to achieve many of its performance gains: Instructions may be executed in a different order than they appear in the program, though their observable effects must [generally be ordered
correctly](http://preshing.com/20120515/memory-reordering-caught-in-the-act/). Breaking dependencies between instructions allows out-of-order execution to be fully exercised. It also gives the compiler more freedom in instruction scheduling, though the `volatile` accesses cannot be reordered with respect to each other (hence it helping to reground the benchmark).
...
On the other hand, CTR mode had a single failure in both the 4-round (dab\_filltree2) and 16-round (dab\_filltree) variants. At least for Blowfish, is there something that makes CTR mode less suitable than CBC mode as a PRNG?
...
Nobody ever got fired for choosing [Mersenne Twister](https://en.wikipedia.org/wiki/Mersenne_Twister). It’s the classical choice for simulations, and is still usually recommended to this day. However, Mersenne Twister’s best days are behind it. I tested the 64-bit variant, MT19937-64, and there are four problems:
...
The [Permuted Congruential Generator](http://www.pcg-random.org/) (PCG) has some really interesting history behind it, particularly with its somewhat [unusual
paper](http://www.pcg-random.org/paper.html), controversial for both its excessive length (58 pages) and informal style. It’s in close competition with Xorshift and xoroshiro128+. I was really interested in seeing how it stacked up.
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Finding%20the%20Best%2064-bit%20Simulation%20PRNG) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Finding+the+Best+64-bit+Simulation+PRNG).

« [Blowpipe: a Blowfish-encrypted, Authenticated Pipe](/blog/2017/09/15/)

» [A Branchless UTF-8 Decoder](/blog/2017/10/06/)
