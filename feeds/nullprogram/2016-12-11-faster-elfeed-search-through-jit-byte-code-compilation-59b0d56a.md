...
feed: nullprogram
guid: https://nullprogram.com/blog/2016/12/11/
---
...
nullprogram.com/blog/2016/12/11/

Today I pushed an update for [Elfeed](https://github.com/skeeto/elfeed) that doubles the speed of the search filter in the worse case. This is the user-entered expression that dynamically narrows the entry listing to a subset that meets certain criteria: published after a particular date, with/without particular tags, and matching/non-matching zero or more regular expressions. The filter is live, applied to the database as the expression is edited, so it’s important for usability that this search completes under a threshold that the user might notice.
...
My [real-world Elfeed database](/blog/2016/08/12/) currently has 46,772 entries with 36 distinct tags. For my benchmark I’ll round this up to a nice 100,000 entries, and use 26 distinct tags (A–Z), which has the nice alphabet property and more closely reflects the number of tags I still care about.
...
Here’s [my old time macro](/blog/2009/05/28/). An important change I’ve made since years ago is to call `garbage-collect` before starting the clock, eliminating bad samples from unlucky garbage collection events. Depending on what you want to measure, it may even be worth disabling garbage collection during the measurement by setting `gc-cons-threshold` to a high value.
...
built-ins](/blog/2013/01/22/), also has an opcode (157). It’s just a tiny bit more overhead.
...
When compiled under lexical scope ( `lexical-binding` is true), here’s the disassembly. To understand what this means, see [*Emacs Byte-code*
...
3. [Lexical scope has better performance](/blog/2016/12/22/).
...
I won’t write the byte-code directly, but instead generate Elisp code and use the byte-code compiler on it. This is safer, will work correctly in future versions of Emacs, and leverages the optimizations performed by the byte-compiler. This sort of thing recently [got a bad
rap on Emacs Horrors](http://emacshorrors.com/posts/when-data-becomes-code.html), but I was happy to see that this technique is already established.
...
It dynamically builds the code as an s-expression, runs that through the byte-code compiler, executes it, and throws it away. It’s “just-in-time,” though compiling to byte-code and not [native
code](/blog/2015/03/19/). For the benchmark tags of `(A B C D E F)`, this builds the following:
...
Have a comment on this article? Start a discussion in my [public inbox](https://lists.sr.ht/~skeeto/public-inbox) by sending an email to [~skeeto/public-inbox@lists.sr.ht](mailto:~skeeto/public-inbox@lists.sr.ht?Subject=Re%3A%20Faster%20Elfeed%20Search%20Through%20JIT%20Byte-code%20Compilation) \[ [mailing list etiquette](https://man.sr.ht/lists.sr.ht/etiquette.md)\] , or see [existing discussions](https://lists.sr.ht/~skeeto/public-inbox?search=Faster+Elfeed+Search+Through+JIT+Byte-code+Compilation).

« [A Showerthoughts Fortune File](/blog/2016/12/01/)

» [Some Performance Advantages of Lexical Scope](/blog/2016/12/22/)
