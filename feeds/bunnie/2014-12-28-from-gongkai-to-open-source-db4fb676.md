...
![](http://bunniefoo.com/fernvale/fernvale-phone-spread.jpg)
...
![](http://bunniefoo.com/fernvale/fernvale-bottom.png)

![](http://bunniefoo.com/fernvale/fernvale-top.png)
...
[![](http://bunniefoo.com/fernvale/fernvale-examples1-sm.jpg)](http://bunniefoo.com/fernvale/fernvale-examples1.jpg)

[![](http://bunniefoo.com/fernvale/fernvale-quarterangle_sm.jpg)](http://bunniefoo.com/fernvale/fernvale-quarterangle.jpg)
...
[![](http://bunniefoo.com/fernvale/fernvale-system-diagram_sm.jpg)](http://bunniefoo.com/fernvale/fernvale-system-diagram.jpg)

*Above: Fernvale system diagram, showing the features of each of the three boards*

[![](http://bunniefoo.com/fernvale/fernvale-frond-top_sm.jpg)](http://bunniefoo.com/fernvale/fernvale-frond-top.jpg)

*Fernvale Frond mainboard*

[![](http://bunniefoo.com/fernvale/fernvale-blade-top_sm.jpg)](http://bunniefoo.com/fernvale/fernvale-blade-top.jpg)

*Fernvale blade UI breakout*

[![](http://bunniefoo.com/fernvale/fernvale-afe-top_sm.jpg)](http://bunniefoo.com/fernvale/fernvale-afe-top.jpg)

*Fernvale spore AFE dev board*

All the hardware source documents can be [downloaded from our wiki](http://www.kosagi.com/w/index.php?title=Fernvale_Main_Page).
...
[![](http://bunniefoo.com/fernvale/fernvale-mtk6260DA_sm.jpg)](http://bunniefoo.com/fernvale/fernvale-mtk6260DA.jpg)
...
To our surprise, this $3 chip didn’t contain a single IC, but rather, it’s a set of at least 4 chips, possibly 5, integrated into a single multi-chip module (MCM) containing hundreds of wire bonds. I remember back when the Pentium Pro’s dual-die package came out. That sparked arguments over yielded costs of MCMs versus using a single bigger die; generally, multi-chip modules were considered exotic and expensive. I also remember at the time, [Krste Asanović](http://www.eecs.berkeley.edu/~krste/), then a professor at the MIT AI Lab now at Berkeley, told me that the future wouldn’t be system on a chip, but rather “system mostly on a chip”. The root of his claim is that the economics of adding in mask layers to merge DRAM, FLASH, Analog, RF, and Digital into a single process wasn’t favorable, and instead it would be cheaper and easier to bond multiple die together into a single package. It’s a race between the yield and cost impact (both per-unit and NRE) of adding more process steps in the semiconductor fab, vs. the yield impact (and relative reworkability and lower NRE cost) of assembling modules. Single-chip SoCs was the zeitgeist at the time (and still kind of is), so it’s interesting to see a significant datapoint validating Krste’s insight.
...
From here, we needed to speed up our measure-modify-test loop; desoldering the ROM, sticking it in a burner, and resoldering it onto the board was going to get old really fast. Given that we had previously implemented a [NAND FLASH ROMulator on Novena](http://www.bunniestudios.com/blog/?p=3265), it made sense to re-use that code base and implement a SPI ROMulator. We hacked up a GPBB board and its [corresponding FPGA code](https://github.com/bunnie/novena-spi-romulator), and implemented the ability to swap between the original boot SPI ROM and a dual-ported 64kiB emulator region that is also memory-mapped into the Novena Linux host’s address space.

![](http://bunniefoo.com/fernvale/fernvale-fpga.jpg)

*Block diagram of the SPI ROMulator FPGA*

[![](http://bunniefoo.com/fernvale/fernvale-mtk-phone-novena_sm.jpg)](http://bunniefoo.com/fernvale/fernvale-mtk-phone-novena.jpg)
...
The next step was to create a small interactive shell which we could use as a beachhead for running experiments on the target hardware. Xobs created a compact REPL environment called [Fernly](https://github.com/xobs/fernly) which supports commands like peeking and poking to memory, and dumping CPU registers.

Because we designed the ROMulator to make the emulated ROM appear as a 64k memory-mapped window on a Linux host, it enables the use a variety of POSIX abstractions, such as mmap(), open() (via /dev/mem), read() and write(), to access the emulated ROM. xobs used these abstractions to create an [I/O target for radare2](https://raw.githubusercontent.com/xobs/fernvale-qemu/master/hw/arm/fernvale.c). The I/O target automatically updates the SHA-1 hash every time we made changes in the 1bl code space, enabling us to do cute things like interactively patch and disassemble code within the emulated ROM space.

![](http://bunniefoo.com/fernvale/fernvale-radare2-2.png)
...
[![](http://bunniefoo.com/fernvale/fernvale-qemu.png)](http://bunniefoo.com/fernvale/fernvale-qemu-hires.png)
...
![](http://bunniefoo.com/fernvale/fernvale-early-memmap.png)
...
At this point, we had to open the Mediatek codebase and refer to the include file that contained the register offsets and bit definitions of the interrupt controller. We believe this is acceptable because facts are not copyrightable. Justice O’Connor wrote in [*Feist v. Rural*](http://en.wikipedia.org/wiki/Feist_v._Rural) (449 U.S. 340, 345, 349 (1991). See also Sony Computer Entm’t v. Connectix Corp., 203 F. 3d 596, 606 (9th Cir. 2000); Sega Enterprises Ltd. v. Accolade, Inc., 977 F.2d 1510, 1522-23 (9th Cir. 1992)) that
...
After about a year of on-and-off effort between work on the Novena and Chibitronics campaigns, we were able to boot a port of NuttX on the MT6260. A minimal set of hardware peripherals are currently supported; it’s enough for us to roughly reproduce the functionality of an AVR used in an Arduino-like context, but not much more. We’ve [presented our results this year at 31C3](http://events.ccc.de/congress/2014/Fahrplan/events/6156.html) ( [slides](http://www.bunniefoo.com/fernvale/fernvale-31c3.pdf)).

The story takes an unexpected twist right around the time we were writing our CFP proposal for 31C3. The week before submission, we became aware that Mediatek released the [LinkIT ONE](http://www.seeedstudio.com/wiki/LinkIt_ONE), based on the MT2502A, in conjunction with Seeed Studios. The LinkIT ONE is directly aimed at providing an Internet of Things platform to entrepreneurs and individuals. It’s integrated into the Arduino framework, featuring an open API that enables the full functionality of the chip, including GSM functions. However, the core OS that boots on the MT2502A in the LinkIT ONE is still the proprietary Nucleus OS and one cannot gain direct access to the hardware; they must go through the API calls provided by the Arduino shim.
...
Want to read more? [Check out xobs’ post on Fernvale](http://xobs.io/fernvale-the-path-not-taken/). Want to get involved? [Chime in at our forums](http://www.kosagi.com/forums/viewforum.php?id=13). Or, watch the recording of our talk below.
