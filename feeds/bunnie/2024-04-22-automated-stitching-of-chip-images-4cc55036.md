---
title: Automated Stitching of Chip Images
url: https://www.bunniestudios.com/blog/2024/automated-stitching-of-chip-images/
published: "2024-04-22T10:56:38Z"
feed: bunnie
guid: https://www.bunniestudios.com/blog/?p=7109
---
...
- [Machine control & focus software](https://www.bunniestudios.com/blog/2024/control-and-autofocus-software-for-chip-level-microscopy/)

This post will cover the software used to stitch together smaller images generated by the [control software](https://www.bunniestudios.com/blog/2024/control-and-autofocus-software-for-chip-level-microscopy/) into a single large image. My IRIS machine with a 10x objective generates single images that correspond to a patch of silicon that is only 0.8mm wide. Most chips are much larger than that, so I take a series of overlapping images that must be stitched together to generate a composite image corresponding to a full chip.

The un-aligned image tiles look like this:

[![](https://bunniefoo.com/iris/2024/before-stitch-mt6331_sm.jpg)](https://bunniefoo.com/iris/2024/before-stitch-mt6331.jpg)

And the stitching software assembles it into something like this:

[![](https://bunniefoo.com/iris/2024/after-stitch-mt6331_sm.jpg)](https://bunniefoo.com/iris/2024/after-stitch-mt6331.jpg)
...
x0.00\_y0.30\_z10.00\_p6869\_i3966\_t127\_j4096\_u127\_a0.0\_r1\_f8.5\_s13374\_v0.998\_d1.5\_o100.0.png

It’s basically an underscore separated [list of metadata](https://github.com/bunnie/jubiris/blob/2dd07f9435e24a0c80deb0c6cfccd5a3c0ccb5ef/midi-ctrl/miduet.py#L127-L148), where each element is tagged with a single ASCII character, followed by its value. It’s a little awkward, but functional and easy enough to migrate as I upgrade schemas.
...
![](https://bunniefoo.com/iris/2024/stitch-schema.png)
...
Again, my mind boggles, because I spend most of my time [writing Rust code](https://github.com/betrusted-io/xous-core/) for a device with [16MiB of RAM](https://precursor.dev).
...
![](https://bunniefoo.com/iris/2024/stitching-path.png)
...
![](https://bunniefoo.com/iris/2024/stitch-sample-ref.jpg)
...
[![](https://bunniefoo.com/iris/2024/stitch-convolution_sm.jpg)](https://bunniefoo.com/iris/2024/stitch-convolution.jpg)

Above is an example of the output of the template matching algorithm. Most of the region is gray, which indicates a poor match. Towards the right, you start to see “ripples” that correspond to the matching features starting to line up. As part of the algorithm, I extract contours to ring regions with a high match, and pick the center of the largest matching region, highlighted here with the pink arrow. The whole contour extraction and center picking thing is a native library in OpenCV with [pretty good documentation examples](https://docs.opencv.org/4.x/d9/d8b/tutorial_py_contours_hierarchy.html).
...
[![](https://bunniefoo.com/iris/2024/stitch-mse_sm.jpg)](https://bunniefoo.com/iris/2024/stitch-mse.jpg)
...
[![](https://bunniefoo.com/iris/2024/stitch-check-ui_sm.jpg)](https://bunniefoo.com/iris/2024/stitch-check-ui.jpg)
...
[![](https://bunniefoo.com/iris/2024/stitch-blended.jpg)](https://siliconpr0n.org/map/mediatek/mt6331/bunnie_bunnie_iris10xew/)

Click the image to browse a [full resolution version, hosted on siliconpr0n](https://siliconpr0n.org/map/mediatek/mt6331/bunnie_bunnie_iris10xew/).

There’s still four major stitch regions visible, and this is because OpenCV’s [MultiBandBlender](https://docs.opencv.org/4.x/d5/d4b/classcv_1_1detail_1_1MultiBandBlender.html) routine seems to be limited to handle 8GiB-ish of raw image data at once, so I can’t quite blend whole chips in a single go. I tried running the same code on a machine with a 24GiB graphics card, and got the same out of memory error, so the limit isn’t total GPU memory. When I dug in a bit, it seemed like there was some driver-level limitation related to the maximum number of pointers to image buffers that I was hitting, and I didn’t feel like shaving that yak.

The underlying algorithm used to do image blending is actually pretty neat, and based off a paper from 1983(!) by Burt and Adelson titled “ [A Multiresolution Spline with Application to Image Mosaics](https://persci.mit.edu/pub_pdfs/spline83.pdf)”. I actually tried implementing this directly using OpenCV’s [Image Pyramid](https://docs.opencv.org/4.x/dc/dff/tutorial_py_pyramids.html) feature, mainly because I couldn’t find any documentation on the MultiBandBlender routine. It was actually pretty fun and insightful to play around with [image pyramids](https://en.wikipedia.org/wiki/Pyramid_(image_processing)); it’s a useful idiom for extracting image features at vastly different scales, and for all its utility it’s pretty memory efficient (the full image pyramid consumes about 1.5x of the original image’s memory).
...
All of the code discussed in this blog post can be found in the [iris-stitcher](https://github.com/bunnie/iris-stitcher) repo on github. [stitch.py](https://github.com/bunnie/iris-stitcher/blob/main/stitch.py) contains the entry point for the code.
...
And as always, a big shout-out to [NLnet](https://nlnet.nl/entrust/) and to my [Github Sponsors](https://github.com/sponsors/bunnie/) for allowing me to do all this research while making it openly accessible for anyone to replicate and to use.
