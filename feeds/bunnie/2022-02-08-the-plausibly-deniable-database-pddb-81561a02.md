---
title: The Plausibly Deniable DataBase (PDDB)
url: https://www.bunniestudios.com/blog/2022/the-plausibly-deniable-database-pddb/
published: "2022-02-08T10:49:25Z"
feed: bunnie
guid: https://www.bunniestudios.com/blog/?p=6307
---
...
[![](https://imgs.xkcd.com/comics/security.png)](https://xkcd.com/538/)

*From [xkcd](https://xkcd.com/538/), CC-BY-NC 2.5*
...
For example, one can fill a disk with random data, and then use a [Veracrypt hidden volume](https://veracrypt.eu/en/docs/hidden-volume/), or LUKS with detached partition headers. So long as the entire disk is pre-filled with random data, it is difficult for an attacker to prove the existence of a hidden volume with these pre-existing technologies.
...
Secrets are overlaid or removed from the database transparently, through a mechanism called “Bases” (the plural of a single “Basis”, similar to the concept of a [vector basis](https://en.wikipedia.org/wiki/Basis_(linear_algebra)) from linear algebra). The application-facing view of data is computed as the union of the key/value pairs within the currently unlocked secret Bases. In the case that the same key exists within dictionaries with the same name in more than one Basis, by default the most recently unlocked key/value pairs take precedence over the oldest. By offering multiple views of the same dataset based on the currently unlocked set of secrets, application developers don’t have to do anything special to leverage the PD aspects of the PDDB.
...
If you want to learn more about the details of the PDDB, I’ve embedded extensive documentation [in the source code](https://github.com/betrusted-io/xous-core/tree/main/services/pddb) directly. The API is roughly modeled after Rust’s `std::io::File` model, where a `.get()` call is similar to the usual `.open()` call, and then a handle is returned which uses `Read` and `Write` traits for interaction with the key data itself. For a “live” example of its usage, users can have a look at WiFi connection manager, which saves a list of access points and their WPA keys in a dictionary: [code for saving an entry](https://github.com/betrusted-io/xous-core/blob/36bf400d46797df66a1064e5bfb84640d1935d89/services/shellchat/src/cmds/wlan.rs#L90-L115), [code for accessing entries](https://github.com/betrusted-io/xous-core/blob/36bf400d46797df66a1064e5bfb84640d1935d89/services/net/src/connection_manager.rs#L314-L334).

Our [CI tests](https://github.com/betrusted-io/xous-core/blob/main/services/pddb/src/tests.rs) contain examples of more interesting edge cases. We also have the ability to save out a PDDB image in “hosted mode” (that is, with Xous directly running on your local machine) and scrub through its entrails with a [Python script](https://github.com/betrusted-io/xous-core/blob/main/tools/pddbdbg.py) that greatly aids with debugging edge cases. The debugging script is only capable of dumping and analyzing a static image of the PDDB, and is coded to “just work well enough” for that job, but is nonetheless a useful resource if you’re looking to go deeper into the rabbit hole.

Please keep in mind that the PDDB is very much in its infancy, so there are likely to be bugs and API-breaking changes down the road. There’s also a lot of work that needs to be done on the UX for the PDDB. At the moment, we only have Rust API calls, so that applications can create and use secret Bases as well as dictionaries and keys. There currently are no user-facing command line tools, or a graphical “File Explorer” style tool available, but we hope to remedy this soon. However, I’m very happy that we were able to complete the core implementation of the PDDB. I’m hopeful that out-of-the-box, application developers for [Precursor](https://precursor.dev) will start using the PDDB for data storage, so that we can start a community dialogue on the overall design.

*This work was funded in part through the [NGI0 PET Fund](https://nlnet.nl/PET/), a fund established by NLNet with financial support from the European Commission’s [Next Generation Internet](https://www.ngi.eu/) Programme.*
