---
title: IRIS (Infra-Red, <em>in situ</em>) Project Updates
url: https://www.bunniestudios.com/blog/2024/iris-infra-red-in-situ-project-updates/
published: "2024-03-09T18:43:29Z"
feed: bunnie
guid: https://www.bunniestudios.com/blog/?p=6937
---

# IRIS (Infra-Red, <em>in situ</em>) Project Updates
...
Here’s an example of the [MPW7](https://www.zerotoasiccourse.com/post/mpw7_submitted/) run on [SKY130A](https://skywater-pdk.readthedocs.io/en/main/), courtesy of [Matt Venn](https://github.com/mattvenn/zero_to_asic_mpw7), imaged using my automated IRIS machine:
...
The traditional route of verification – observing the “fit and finish” of a product to detect counterfeits – is still the dominant method for most consumer products. However, in mobile phones and some laptops, manufacturers deploy electronic serial numbers and tamper detection techniques to deter would-be thieves of components or sub-assemblies. The effectiveness of these techniques depend intimately upon the implementation details; but in any case, they incur a cost in repairability and often times the end consumer can’t access the vendor’s databases to check that everything is in order. Instead, consumers are forced to [delegate their trust to the vendors](https://www.bunniestudios.com/blog/?page_id=1022); yet regular consumers have no way to audit the vendors. The deferred trust boogeyman haunts everything from Apple’s iPhone ecosystem, to Intel’s SGX remote attestation mechanisms.
...
![](https://bunniefoo.com/iris/2024/sc_component.png)
...
![](https://bunniefoo.com/iris/2024/sc_chip.png)

This is because prior to IRIS, your options for inspecting silicon are either destructive, or experiments conducted in high energy physics labs. If you’ve ever seen the brilliant teardowns done by companies like [TechInsights](https://www.techinsights.com/), the chips are generally imaged at the circuit level with a SEM or FIB, which requires the chip to be removed from its package and some of the metal layers to be permanently stripped off. Thus, one can obtain extremely high-quality imagery of a sample chip, but these techniques cannot be used to verify the very chip you want to use inside your computer, as the imaged chip must be destroyed.

An alternative, non-destructive technique known as X-ray ptychography can be thought of as a very high resolution 3D scanner for circuits. It’s very impressive, but to date it can only be done in a handful of high energy physics labs and it takes a long time ( [about three hours for 20 cubic microns](https://www.nature.com/articles/s41598-022-11292-8)) to image a full chip.
...
![](https://bunniefoo.com/iris/2024/sc_circuit.png)
...
As a result, scan chains are good for detecting flaws in known circuits introduced by mother nature, but ineffective at deterring a malicious adversary. This is true even if one could brute-force the entire state-space due to the [epistemic circularity](https://iep.utm.edu/ep-circ/) of trusting a circuit to test itself. More colloquially, one may have heard of the “ [Hawthorne Effect](https://en.wikipedia.org/wiki/Hawthorne_effect)” or the “observer effect”, which describes a subject under study temporarily altering their behavior because they know they are being studied, thus affecting the results of the study. In this case, a scan chain knows when it’s being queried, and thus, a malicious modification to a scan chain can add hidden states that alter its behavior for the duration of a scan, allowing it to pretend to be correctly constructed, but only when a check is actively running.
...
![](https://bunniefoo.com/iris/2024/verification-hiearchy.png)
...
![](https://bunniefoo.com/iris/2024/sc_scales.png)
...
[![](https://bunniefoo.com/iris/2024/iris-machine-overview_sm.jpg)](https://bunniefoo.com/iris/2024/iris-machine-overview.jpg)

Above is the IRIS machine that I’ve built. There’s a lot going on here – but basically, it’s an IR camera attached to a microscope, a nanometer-resolution focusing mechanism, and a pair of 1050nm light sources that have continuously adjustable azimuth and zenith. This microscope assembly is mounted in a [Jubilee](https://machineagency.github.io/science_jubilee/) motion platform. The Jubilee is [open source](https://github.com/machineagency/science_jubilee), and was designed by [Sonya Vasquez](https://www.doublejumpelectric.com/) of [Prof. Nadya Peek](https://www.hcde.washington.edu/peek)‘s [Machine Agency](http://depts.washington.edu/machines/) group. I got it as a kit from [Filastruder](https://www.filastruder.com/collections/jubilee). The base motion platform is capable of 10 micron steps, and features a kinematically coupled Z-bed with three independent actuators, allowing me to dynamically compensate for planarity issues in the sample undergoing imaging.
...
However, if you’re impatient and don’t want to wait for the documentation, you can already browse the source files for the [microscope](https://github.com/bunnie/iris-hw), [control software](https://github.com/bunnie/jubiris), [stitching software](https://github.com/bunnie/iris-stitcher), and [layout extraction software](https://github.com/bunnie/iris-layout).

![](https://bunniefoo.com/iris/iris-logo-gh.png)

Finally, a big shout-out to NLnet and the European Commission. [NLnet’s NGI0 Entrust](https://nlnet.nl/entrust/) fund, established with support from the [European Commission’s](https://commission.europa.eu/index_en) [Next Generation Internet](https://www.ngi.eu/) Program, are instrumental in facilitating my work on IRIS. Also a big shout-out to my [Github Sponsors](https://github.com/sponsors/bunnie/) for their incredible generosity and monthly support. Thanks to all these donors, I’m able to keep IRIS 100% open source and free of conflicts of interest with commercial investors.

[![](https://bunniefoo.com/iris/2024/nlnet-logos.png)](https://nlnet.nl/entrust/)

[❤️ Sponsor me on Github! ❤️](https://github.com/sponsors/bunnie/)
