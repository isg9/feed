---
title: Testing LLVM
url: https://blog.regehr.org/archives/1450
published: "2017-01-13T20:22:44Z"
feed: regehr
guid: http://blog.regehr.org/?p=1450
---

# Testing LLVM

*\[This piece is loosely a followup to [this one](https://blog.regehr.org/archives/1453).\]*
...
- It is often the case that a compiler is one of several independent implementations of a given specification, such as the C++ standard, enabling [differential testing](http://www.cs.dartmouth.edu/~mckeeman/references/DifferentialTestingForSoftware.pdf). Even when multiple implementations are unavailable, we can often test a compiler against itself by comparing the output of different backends or different optimization modes.
...
The first argument to the TEST\_F macro indicates the name of the test case (a collection of tests) and the second names the actual test shown here. The parseAssembly() and expectPattern() methods respectively call into an LLVM API and then check that this had the expected result. This example is from [ValueTrackingTest.cpp](https://github.com/llvm-mirror/llvm/blob/release_39/unittests/Analysis/ValueTrackingTest.cpp). Many tests can be put into a single file, keeping things fast by avoiding forks/execs.

The other infrastructure used by LLVM’s fast test suite is [lit, the LLVM Integrated Tester](http://llvm.org/docs/CommandGuide/lit.html). lit is shell-based: it executes commands found in a test case, and considers the test to have been successful if all of its sub-commands succeed.

Here’s a test case for lit (I grabbed the top of [this file](https://github.com/llvm-mirror/llvm/blob/release_39/test/Transforms/InstCombine/add2.ll), which contains additional tests that don’t matter to us right now):
...
To run this test case, the file is interpreted three times. First, lit scans it looking for lines containing RUN: and executes each associated command. Second, the file is interpreted by opt, the standalone optimizer for LLVM IR; this happens because lit replaces the %s variable with the name of the file being processed. Since comments in textual LLVM IR are preceded by a semicolon, the lit directives are ignored by opt. The output of opt is piped to the [FileCheck](http://llvm.org/docs/CommandGuide/FileCheck.html) utility which parses the file yet again, looking for commands such as CHECK and CHECK-NEXT; these tell it to look for strings in its stdin, and to return a non-zero status code if any of the specified strings isn't found. (CHECK-LABEL is used to divide up a file into a collection of logically separate tests.)

An important part of a long-term testing campaign is using coverage tools to find parts of the code base that aren't being tested. Here's a [recent LLVM coverage report](http://llvm.org/reports/coverage/) based on running the unit/regression tests. This data is pretty interesting to poke around in. Let's take a quick look at [coverage of InstCombine](http://llvm.org/reports/coverage/lib/Transforms/InstCombine/), which is generally very good. An interesting project for someone wanting to get started with LLVM would be to write and submit test cases that cover untested parts of InstCombine. For example, here's the first uncovered code (colored red) in [InstCombineAndOrXor.cpp](http://llvm.org/reports/coverage/lib/Transforms/InstCombine/InstCombineAndOrXor.cpp.gcov.html):

![](https://blog.regehr.org/extra_files/AndOrXor-coverage.png)
...
![](https://blog.regehr.org/extra_files/AndOrXor-dead.png)
...
[LNT](http://lnt.llvm.org/) (LLVM Nightly Test) doesn't contain any test cases; it is a tool for aggregating and analyzing test results, focusing on monitoring the quality of the compiler's generated code. It consists of local utilities for running tests and submitting results, and then there's a server side database and web frontend that makes it easy to look through results. The NTS (Nightly Test Suite) [results are here](http://llvm.org/perf/db_default/v4/nts/recent_activity).

## BuildBot

The [Linux/Windows BuiltBot](http://lab.llvm.org:8011/) and [the Darwin one](http://lab.llvm.org:8080/green/) (I don't know why there are two) are used to make sure LLVM configures, builds, and passes its unit/regression tests on a wide variety of platforms and in a variety of configurations. The BuildBot has some blame support to help find problematic commits and will send mail to their authors.

## Eclectic Testing Efforts

Some testing efforts originate outside of the core LLVM community and aren't as systematic in terms of which versions of LLVM get tested. These tests represent efforts by individuals who usually have some specific tool or technique to try out. For example, for a long time my group tested Clang+LLVM using [Csmith](https://github.com/csmith-project/csmith) and reported the resulting bugs. ( [See the high-level writeup.](http://www.cs.utah.edu/~regehr/papers/pldi11-preprint.pdf)) Sam Liedes [applied afl-fuzz](http://lists.llvm.org/pipermail/llvm-dev/2014-December/079390.html) to the Clang test suite. [Zhendong Su and his group](http://web.cs.ucdavis.edu/~su/emi-project/) have been finding a very impressive number of bugs. [Nuno Lopes](http://web.ist.utl.pt/nuno.lopes/) has done some awesome formal-methods-based testing of optimization passes that he'll hopefully write about soon.
