...
feed: matklad
guid: https://matklad.github.io/2024/03/22/basic-things.html
---
...
3. There’s only unstructured append-only pile of isolated documents. Things are *mostly* documented, often two or three times, but any new team member has to do the wheat from the chaff thing anew.
...
*First*, a build system is a bootstrap process: it is how you get from `git clone` to a working binary. The two aspects of this bootstrapping process are important:
...
But releasing software is also just code, which you can write in your primarly language. [The right tool for the job is often the tool you are already
using](https://twitter.com/id_aa_carmack/status/989951283900514304). It pays off to explicitly attack the problem of glue from the start, and to pick/write a library that makes writing subprocess wrangling logic easy.
