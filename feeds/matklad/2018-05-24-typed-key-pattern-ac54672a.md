...
feed: matklad
guid: https://matklad.github.io/2018/05/24/typed-key-pattern.html
---
...
[https://github.com/matklad/typed\_key](https://github.com/matklad/typed_key)

If you want to skip all the blah-blah-blah, you can dig right into the code & docs :)

## [The problem](\#The-problem)

You have an untyped `Map` and you need to get a typed `Foo` out of it by the `"foo"` key. The untyped map is often some kind of configuration, like a JSON file, but it can be a real map with type-erased `Any` objects as well.

In the common case of statically known configuration, the awesome solution that Rust offers is [serde](https://crates.io/crates/serde). You stick `derive(Deserialize)` in front of the `Config` struct and read it from JSON, YML, TOML or even just [environment variables](https://github.com/softprops/envy)!
...
However, the last pattern can become much more powerful and interesting if we associate types with string constants. The idea is to encode that the `"foo"` key can be used to extract an object of type `Foo`, and make it impossible to use it for, say, `Vec`. To do this, we’ll need a pinch of [`PhantomData`](https://doc.rust-lang.org/beta/std/marker/struct.PhantomData.html):
...
## [Pattern in the wild](\#Pattern-in-the-wild)

I’ve first encountered this pattern in [IntelliJ](https://github.com/JetBrains/intellij-community/blob/16bfca92199dca383b66c69c3427b4639ea8e213/platform/util/src/com/intellij/openapi/util/Key.java) code. It uses [`UserDataHolder`](https://github.com/JetBrains/intellij-community/blob/16bfca92199dca383b66c69c3427b4639ea8e213/platform/util/src/com/intellij/openapi/util/UserDataHolder.java), which is basically `Map`, everywhere. It helps plugin authors to extend built-in objects in crazy ways but is rather hard to reason about, and type-safety improves the situation a lot. I’ve also changed Exonum’s config to employ this pattern in this [PR](https://github.com/exonum/exonum/pull/417). It also was a case of plugin extensible, where an upfront definition of all configuration option is impossible.

Finally, I’ve written a small crate for this [`typed_key`](https://crates.io/crates/typed_key) :)
