...
feed: matklad
guid: https://matklad.github.io/2023/01/26/rusts-ugly-syntax.html
---
...
This function reads contents of a given binary file. This is lifted straight from the standard library, so it is very much not a straw man example. And, at least to me, it’s definitely not a pretty one!
...
```
public function readextends IncludingRef<Path>>(
    path: P,
): io.Result<Array> {
...
Having done this, we can actually get rid of `Vec` as well — we can no longer use generics to express efficient growable array of bytes in the language itself. We’d have to use some opaque `Bytes` type provided by the runtime:
