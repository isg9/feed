...
feed: matklad
guid: https://matklad.github.io/2023/08/17/typescript-is-surprisingly-ok-for-compilers.html
---
...
For more language-centric tasks, like a formal specification, or a toy hobby language, OCaml makes most sense. See, for example, [plzoo](https://plzoo.andrej.com) or [WebAssembly reference interpreter](https://github.com/WebAssembly/spec/tree/653938a88c6f40eb886d5980ca315136eb861d03/interpreter).
...
Even here, we already see high-level nature of TypeScript — string is just a `string`, there’s no thinking about `usize` vs `u32` as numbers are just `number` s.
...
```
export type ExprBool = ExprLiteralboolean, "bool">;
export type ExprInt = ExprLiteralnumber, "int">;

export interface ExprLiteralTag> {
  tag: Tag;
  value: V;
}
```
...
```
export type ExprControl = ExprIf;
...
There’s one problem though. What we really want to write here is something like `const inferred_type = switch(..)`, but in TypeScript `switch` is a statement, not an expression. So let’s define a generic visitor!

```
export type Visitor = {
  bool(kind: ExprBool): R;
  int(kind: ExprInt): R;
  binary(kind: ExprBinary): R;
  if(kind: ExprIf): R;
};
...
Before we go further, let’s generalize this visiting pattern a bit! Recall that our expressions are parametrized by the type of associated data, and type-checker-shaped transformations are essentially an `Expr -> Expr` transformation.

Let’s make this generic!

```
export function transform(expr: Expr, v: Visitor): Expr {
```

Transform maps an expression carrying `T` into an expression carrying `V` by applying an `f` visitor. Importantly, it’s `Visitor`, rather than a `Visitor`. This is counter-intuitive, but correct — we run transformation bottom up, transforming the leaves first. So, when the time comes to visit an interior node, all subexpression will have been transformed!
...
1. Note how here `expr.kind` is both `Expr` and `Expr` — literals don’t depend on this type parameter, and TypeScript is smart enough to figure this out without us manually re-assembling the same value with a different type.

This is where that magic with `Visitor` happens.
