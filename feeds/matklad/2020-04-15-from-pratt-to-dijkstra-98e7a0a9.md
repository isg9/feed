...
feed: matklad
guid: https://matklad.github.io/2020/04/15/from-pratt-to-dijkstra.html
---
...
This is a sequel to the [previous post](/2020/04/13/simple-but-powerful-pratt-parsing) about Pratt parsing. Here, we’ll study the relationship between top-down operator precedence (Pratt parsing) and the more famous shunting yard algorithm. Spoiler: they are the same algorithm, the difference is implementation style with recursion (Pratt) or a manual stack (Dijkstra).
...
```
fn expr_bp(lexer: &mut Lexer, min_bp: u8) -> Option {
    if min_bp == 100 {
        return None;
    }
    let mut lhs = match lexer.next() {
...
## [From Recursion to Stack](\#From-Recursion-to-Stack)
...
And now, let’s add a `stack: Vec`. This is the point where the magic happens. We’ll still keep the `top` local variable: representing a stack as `(T, Vec)` and not as just `Vec` gives us compile-time guarantee of non-emptiness. We replace the `expr_bp(lexer, r_bp)` recursive call with pushing to the stack. All operations after the call are moved after `return`. `return` itself is replaced with popping off the stack.

```
fn expr_bp(lexer: &mut Lexer) -> Option {
    let mut top = Frame {
        min_bp: 0,
        lhs: None,
        token: None,
...
The code from this and the previous article is available here: [https://github.com/matklad/minipratt](https://github.com/matklad/minipratt).
