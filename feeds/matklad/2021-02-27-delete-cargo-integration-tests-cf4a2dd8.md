...
feed: matklad
guid: https://matklad.github.io/2021/02/27/delete-cargo-integration-tests.html
---
...
When a refactoring along these lines was applied to Cargo itself, the effects were substantial ( [numbers](https://github.com/rust-lang/cargo/pull/5022#issuecomment-364691154)). The time to compile the test suite decreased 3x. The size of on-disk artifacts decreased 5x.
...
For a library with a public API which is published to crates.io, I avoid unit tests. Instead, I use a single integration tests, called **`it`** ( **i** ntegration **t** est):
...
## [Assorted Tricks](\#Assorted-Tricks)
...
This way, when you modify just the tests, Cargo is smart to not recompile the library crate. It knows that the contents of `tests.rs` only affects compilation when `--test` is passed to rustc. Learned this one from [@petrochenkov](https://github.com/petrochenkov), thanks!

*Third*, even if you stick to unit tests, the library is recompiled twice: once with, and once without `--test`. For this reason, folks from [pernosco](https://pernos.co) go even further. They add
