---
title: STD Doesn't Have to Abstract OS IO
url: https://matklad.github.io/2024/08/12/std-io.html
published: "2024-08-12T00:00:00Z"
feed: matklad
guid: https://matklad.github.io/2024/08/12/std-io.html
---
...
2. If this is a cross platform abstraction around an IO facility provided by an OS, and this IO facility has a reasonable common subset across most OSes, it should be in the stdlib.

3. If there’s one obvious way to implement it, it might go to stdlib.

So for example something like [`Vec`](https://doc.rust-lang.org/stable/std/vec/struct.Vec.html) goes into a standard library, because all *other* libraries are going to use vectors at the interfaces.

Something like [`lazy_static`](https://docs.rs/lazy_static/1.5.0/lazy_static/macro.lazy_static.html) doesn’t: while it is often needed, it is *not* a vocabulary interface type.

But it is acceptable for something like [`OnceCell`](https://docs.rs/once_cell/1.19.0/once_cell/sync/struct.OnceCell.html) to be in `std` — it is still not a vocabulary type, but, unlike `lazy_static`, it is clear that the API is more or less optimal, and that there aren’t that many good options to do this differently.
