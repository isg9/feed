...
url: https://matklad.github.io/2019/06/20/linux-desktop-tips.html
published: "2019-06-20T00:00:00Z"
feed: matklad
guid: https://matklad.github.io/2019/06/20/linux-desktop-tips.html
---
...
For some reason, this is not available out of the box in any of the Linux window managers I’ve tried. What is easy is binding launching an application to a shortcut, but I rarely use more than one instance of Firefox!

Luckily, [jumpapp](https://github.com/mkropat/jumpapp) is exactly what is needed to implement this properly.

I use [Xbindkeys](https://wiki.archlinux.org/index.php/Xbindkeys) for global shortcuts, with the following config:
...
I’ve always liked Quake-style terminals, which you can bring to front with a single keypress. For this reason, I was stuck with [yakuake](https://kde.org/applications/system/org.kde.yakuake) for a really long time.

`jumpapp` allows me to use any terminal in this fashion, so now I use full screen [kitty](https://sw.kovidgoyal.net/kitty/).
...
- To still have an ability to close/minimize the window with the mouse, I use [Active Window Menu](https://store.kde.org/p/998910/) Plasmoid. What it does is that it packs window title and close/maximize/minimize buttons into the desktop panel, without spending extra pixels:
...
A much better approach is [Home Row Computing](https://manybutfinite.com/post/home-row-computing/). I rebind, on the low level, `CapsLock + i/j/k/l` to arrow keys. This works in every app. It also works with `alt` and `shift` modifiers.

I use [xkbcomp](https://jlk.fjfi.cvut.cz/arch/manpages/man/xkbcomp.1) with [this config](https://github.com/matklad/config/blob/ed588057545276e05ea4979ea7086addc3724a4e/home-row.xkb) to set this up. I have no idea how this actually works :-)

## [File Organization](\#File-Organization)

I used to pile up **everything** on the desktop. But now my desktop is completely empty, and I enjoy uncluttered view of [The Hunters in the Snow](https://upload.wikimedia.org/wikipedia/commons/d/d8/Pieter_Bruegel_the_Elder_-_Hunters_in_the_Snow_%28Winter%29_-_Google_Art_Project.jpg) every time I boot my laptop.
...
I used to use `Zsh` with a bunch of plugins, hoping that I’ll learn `bash` this way. I still google “How to `if` in bash?” every single time though.

For this reason, I’ve switched to [fish](http://fishshell.com/) with mostly default config. The killer feature for me is autosuggestions: completion of the commands based on the history. `Zsh` has something similar, via a plugin, but this crucial feature works in `fish` out of the box.
...
## [Minimizing Firefox](\#Minimizing-Firefox)

Finally, the direct cause of this post!

I don’t use a lot of desktop apps, but I keep a browser with at least five tabs for different messaging apps. By the way, [Tree Style Tab](https://addons.mozilla.org/en-US/firefox/addon/tree-style-tab/) is the best tool for taming modern “ `apps`”!
...
I once tried to build it with `Xbindkeys` and `Xdotool`. The idea was to intercept `Alt+F4` and minize active window if it is Firefox. That didn’t work too well: to close all other applications, I tried to forward `Alt+F4`, but that recursed badly :-)
...
[This post](https://zren.github.io/2019/03/14/quick-tile-an-app-when-it-opens-using-a-kwin-script) was instrumental for me to figure this stuff out. Thanks Chris!
...
After than, I’ve ticked a box in front of `Smart Close Window` in System Settings › Window Management › KWin Scripts and added a shortcut in System Settings › Shortcuts › Global Shortcuts › System Settings. The last step took a while to figure out: although it looks like we set shortcut in the script itself, this doesn’t actually work for some reason.

## [Linux Distribution](\#Linux-Distribution)

Finally, my life has become significantly easier since I’ve settled on [NixOS](https://nixos.org/). I had mainly used [Arch](https://www.archlinux.org/) and a bit of [Ubuntu](https://ubuntu.com/) before, but NixOS is so much easier to control. I highly recommend to check it out!

## [My Dotfiles](\#My-Dotfiles)

Most of the stuff in this post is codified in my config repo: [https://github.com/matklad/config](https://github.com/matklad/config)
