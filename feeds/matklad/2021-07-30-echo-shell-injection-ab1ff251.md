...
feed: matklad
guid: https://matklad.github.io/2021/07/30/shell-injection.html
---
...
The first `execve` we see here is our original invocation of the `node` binary itself. The last one is what we want to do — spawn `curl` with a single argument, a url. And the middle one is what node’s `exec` actually does.
...
malice\_in\_the\_wonderland.txt
...
## [Those Who Study History Are Doomed to Repeat It](\#Those-Who-Study-History-Are-Doomed-to-Repeat-It)

There’s an equivalent safe API in node: [`spawn`](https://nodejs.org/api/child_process.html#child_process_child_process_spawn_command_args_options). unlike `exec`, it uses an array of arguments rather then a single string.
...
Why provide an exploitable API, while a safe version is possible and is more direct? I don’t know, but my guess is that it’s mostly just history. C has [`system`](https://en.cppreference.com/w/c/program/system), Perl’s backticks correspond directly to that, Ruby got backticks from Perl, Python just has `system`, node was probably influenced by all these scripting languages.

Note that security isn’t the only issue with `/bin/sh -c` based API. Read [this other post](https://julialang.org/blog/2012/03/shelling-out-sucks/) to learn about the rest of the problems.

## [Take Aways](\#Take-Aways)

If you are an *application developer*, be aware that this issue exists. Read the language documentation carefully — most likely, there are two flavors of process spawning functions. Note how shell injection is similar to [SQL injection](https://en.wikipedia.org/wiki/SQL_injection) and [XSS](https://en.wikipedia.org/wiki/Cross-site_scripting).
...
I was happily hacking on some Rust library. At some point I pressed the “run tests” button in [rust-analyzer](https://rust-analyzer.github.io). And, surprised, accidentally pwned myself!

```
Executing task: cargo test --doc --- Plotter::line_fill --nocapture

warning: An error occurred while redirecting file 'D'
open: No such file or directory

The terminal process
/bin/fish '-c', 'cargo test --doc --- Plotter::line_fill --nocapture'
