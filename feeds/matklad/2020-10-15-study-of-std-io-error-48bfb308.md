...
In this article we’ll dissect the implementation of `std::io::Error` type from the Rust’s standard library. The code in question is here: [library/std/src/io/error.rs](https://github.com/rust-lang/rust/blob/5565241f65cf402c3dbcb55dd492f172c473d4ce/library/std/src/io/error.rs).
...
When designing an `Error` type for use with `Result`, the main question to ask is “how the error will be used?”. Usually, one of the following is true.
...
An extreme version of this idea is [sans-io](https://sans-io.readthedocs.io/) programming. Most errors come from IO; if you push all IO to the caller, you can skip most of the error handing!

However bad the `enum` approach might be, it does achieve maximum inspectability of the first case.

The propagation-centered second case of error management is typically handled by using a boxed trait object. A type like `Box` can be constructed from any specific concrete error, can be printed via `Display`, and can still optionally expose the underlying error via dynamic downcasting. The [`anyhow`](https://lib.rs/crates/anyhow) crate is a great example of this style.
...
- `io::Error` is also a vocabulary type, and should be able to represent some not-quite-os errors. For example, Rust `Path` s can contain internal `0` bytes and `open` ing such path should return an `io::Error` *before* making a syscall.
...
The last point means that `io::Error` can be used for ad-hoc errors, as `&str` and `String` are convertible to `Box`:
...
I was surprised to learn that std in fact [does](https://github.com/rust-lang/rust/blob/e160e5cb80652bc2afe74cb3affbe35b74243ea9/library/std/src/sys/unix/fs.rs#L867-L869) an allocation for every path-related syscall.

It needs to be there in some form: OS API require that unfortunate zero byte at the end of strings. But I wonder if using a stack-allocated buffer for short paths would’ve made sense. Probably not — paths are not that short usually, and modern allocators handle transient allocations efficiently.
...
- Its size is pretty big:
  ...
  ```
  assert_eq!(size_of::(), 2 * size_of::<usize>());
  ```
...
*First*, we can get rid of double indirection by using a thin trait object, a-la [`failure`](https://github.com/rust-lang-nursery/failure/blob/135e2a3b9af422d9a9dc37ce7c69354c9b36e94b/src/error/error_impl_small.rs#L9-L18) or [`anyhow`](https://github.com/dtolnay/anyhow/blob/840afd84e9dd91ac5340c05afadeecbe45d0b810/src/error.rs#L671-L679). Now that [`GlobalAlloc`](https://doc.rust-lang.org/stable/std/alloc/trait.GlobalAlloc.html) exist, it’s a relatively straight-forward implementation.

*Second*, we can make use of the fact that pointers are aligned, and stash both `Os` and `Simple` variants into `usize` with the least significant bit set. I think we can even get creative and use the *second* least significant bit, leaving the first one as a niche. That way, even something like `io::Result` can be pointer-sized!

And this concludes the post. Next time you’ll be designing an error type for your library, take a moment to peer through [sources](https://github.com/rust-lang/rust/blob/5565241f65cf402c3dbcb55dd492f172c473d4ce/library/std/src/io/error.rs) of `std::io::Error`, you might find something to steal!

Discussion on [/r/rust](https://www.reddit.com/r/rust/comments/jbdk5x/blog_post_study_of_stdioerror/).

Bonus puzzler

Take a look at [this line](https://github.com/rust-lang/rust/blob/e160e5cb80652bc2afe74cb3affbe35b74243ea9/library/std/src/io/error.rs#L542) from the implementation:
