---
title: Min of Three
url: https://matklad.github.io/2017/03/12/min-of-three.html
published: "2017-03-12T00:00:00Z"
feed: matklad
guid: https://matklad.github.io/2017/03/12/min-of-three.html
---
...
The phenomenon described in this blog post was observed in [this thread](https://users.rust-lang.org/t/performance-issue-with-c-array-like-computation-2-times-worst-than-naive-java/9807) of the Rust forum. I am not the one who found out what is going on, I am just writing it down :)
...
We will be measuring [dynamic time warping](https://en.wikipedia.org/wiki/Dynamic_time_warping) algorithm. This algorithm calculates a distance between two real number sequences, `xs` and `ys`. It is very similar to [edit distance](https://en.wikipedia.org/wiki/Wagner%E2%80%93Fischer_algorithm) or [Needleman–Wunsch](https://en.wikipedia.org/wiki/Needleman%E2%80%93Wunsch_algorithm), because it uses the same dynamic programming structure.
...
What is the bottleneck? Let’s look at the instruction level profile of the main loop using [`perf annotate`](https://perf.wiki.kernel.org/index.php/Main_Page) command:
...
Can we do better? Let’s use `min2` function to calculate minimum of three elements recursively:
...
## [A possible explanation](\#A-possible-explanation)

A possible explanation is a synergy of CPU level parallelism and speculative execution. It was proposed by [@krdln and @vitalyd](https://users.rust-lang.org/t/performance-issue-with-c-array-like-computation-2-times-worst-than-naive-java/9807/30?u=matklad). I don’t know how to [falsify](https://en.wikipedia.org/wiki/Falsifiability) it, but it at least looks plausible to me!

Imagine for a second that instead of `vminsd %xmm0,%xmm1,%xmm0` instruction in the preceding assembly there is just `vmovsd %xmm1,%xmm0`. That is, we don’t use `xmm0` from the previous iteration at all! This corresponds to the following update rule:
...
So it looks like the CPU is able to speculatively execute `vminsd` and parallelise the following computation based on this speculation! Isn’t that awesome?
...
This is explored in [the second part](https://matklad.github.io/2017/03/18/min-of-three-part-2.html) of this post.
