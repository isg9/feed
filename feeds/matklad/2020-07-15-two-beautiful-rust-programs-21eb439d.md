...
feed: matklad
guid: https://matklad.github.io/2020/07/15/two-beautiful-programs.html
---
...
This is a short ad of a [Rust](https://www.rust-lang.org/) programming language targeting experienced C++ developers. Being an ad, it will only whet your appetite, consult other resources for fine print.
...
This program creates a vector of 32-bit integers ( `std::vector`), takes a reference to the first element, `x`, pushes one more number onto the vector and then uses `x`. The program is wrong: extending the vector may invalidate references to element, and `*x` might dereference a dangling pointer.
...
Rust compiler tracks the aliasing status of every piece of data and forbids mutations of potentially aliased data. In this example, `x` and `xs` alias the first integer in the vector’s storage in the heap.
...
If any of these constraints are broken, the compiler rejects the code. There’s no need for `std::shared_ptr` just to defensively make sure that the memory isn’t freed under your feet.
