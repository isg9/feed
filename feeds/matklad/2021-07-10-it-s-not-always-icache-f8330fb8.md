...
url: https://matklad.github.io/2021/07/10/its-not-always-icache.html
published: "2021-07-10T00:00:00Z"
feed: matklad
guid: https://matklad.github.io/2021/07/10/its-not-always-icache.html
---
...
This is a follow up to the [previous post](https://matklad.github.io/2021/07/09/inline-in-rust.html) about `#[inline]` in Rust specifically. This post is a bit more general, and a bit more ranty. Reader, beware!
...
The benchmark is based on my [once\_cell](https://github.com/matklad/once_cell) Rust library. The library provides a primitive, similar to [double-checked locking](https://en.wikipedia.org/wiki/Double-checked_locking). There’s a function that looks like this:

```
fn get_or_try_init(&self, f: F) -> Result<&T, E>
where
 F: FnOnce() -> Result,
{
...
You can see the full benchmark in this commit: [matklad/once\_cell@a741d5f](https://github.com/matklad/once_cell/commit/a741d5f2ca7cd89125ef1c70ee2e5fe660271050).
...
Note that we don’t use fancy statistics here. `/usr/bin/time` is enough to see the difference with a naked eye despite the fact that the effect we are looking for is very low-level. Hence, a general tip: if you are benchmarking relative difference (and not the absolute performance), don’t bother with measuring nanosecond-precision time. Instead, loop the benchmark enough to make the change human-perceptible.
...
Anyway, it seems there’s one thing that bears further investigation — why inlining changes the number of instructions executed? Inlining doesn’t actually change the code the CPU runs, so the number of instructions should stay the same. Let’s look at the asm then! The right tool here is [cargo-asm](https://github.com/gnzlbg/cargo-asm).
...
```
 ...
 .LBB14_1:
  ...
  je      .loop
  mov     rdi, r14
  call    once_cell::imp::OnceCell::initialize
  jmp     .loop
 .normal_exit:
  add     rsp, 8 ;
  pop     rbx    ; epilogue
...
Note that it’s highly unlikely that ICache affects the running code, as it’s a small bunch of instructions next to each other in memory. On the other hand, an extra `cmp` with a large immediate precisely accounts for the amount of extra instructions we observe (the loop is run 800\_000\_000 times).

## [Conclusions](\#Conclusions)

It’s hard enough to come up with a benchmark which demonstrate the difference between two alternatives. It’s even harder to explain the difference — there might be many [readily available](https://en.wikipedia.org/wiki/Availability_heuristic) explanations, but they are not necessary true. Nonetheless, today we have a wealth of helpful tooling. Two notable examples are [perf](https://perf.wiki.kernel.org/index.php/Tutorial) and [valgrind](https://valgrind.org/docs/manual/quick-start.html). Tools are not always correct — it’s a good idea to sanity check different tools against each other and against common-sense understanding of the problem.
...
If you are curious under which conditions ICache does become an issue, there’s [this excellent article](https://www.scylladb.com/2017/07/06/scyllas-approach-improve-performance-cpu-bound-workloads/) about one such case.
