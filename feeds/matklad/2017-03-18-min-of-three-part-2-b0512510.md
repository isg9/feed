...
url: https://matklad.github.io/2017/03/18/min-of-three-part-2.html
published: "2017-03-18T00:00:00Z"
feed: matklad
guid: https://matklad.github.io/2017/03/18/min-of-three-part-2.html
---
...
This is the continuation of the [previous post](https://matklad.github.io/2017/03/12/min-of-three.html) about optimizing 2D grid based dynamic programming algorithm for [CPU level parallelism](https://en.wikipedia.org/wiki/Superscalar_processor).

## [In The Previous Episode](\#In-The-Previous-Episode)
...
It calculates [dynamic time warping](https://en.wikipedia.org/wiki/Dynamic_time_warping) distance between two `double` vectors using an update rule which is structured like this:
...
Check the [previous post](https://matklad.github.io/2017/03/12/min-of-three.html) for more details!

## [The parallel plan](\#The-parallel-plan)
...
We’d need to remember **two** previous diagonals instead of one previous row, but all the cells on the next diagonal would be independent! In theory, compiler should be able to use [SIMD instructions](https://en.wikipedia.org/wiki/SIMD#Hardware) to make the computation truly parallel.
...
- `id` is the index of the diagonal. There are twice as many diagonals as rows.
...
Can we go further and add get SIMD instructions here? At the moment, Rust does not have a stable way to explicitly emit SIMD ( [it’s going to change some day](https://internals.rust-lang.org/t/getting-explicit-simd-on-stable-rust/4380)) (UPDATE: we have [SIMD on stable](https://doc.rust-lang.org/core/arch/index.html) now!), so the only choice we have is to tweak the source code until LLVM sees an opportunity for vectorization.

## [SIMD](\#SIMD)

Although bounds checks themselves don’t slow down the code that much, they can prevent LLVM from vectorizing. So let’s dip our toes into `unsafe`:
...
How can we get the same results with safe Rust? One possible way is to use iterators, but in this case the resulting code would be rather ugly, because you’ll need a lot of nested `.zip`’s. So let’s try a simple trick of hoisting the bounds checks of the loop. The idea is to transform this:
