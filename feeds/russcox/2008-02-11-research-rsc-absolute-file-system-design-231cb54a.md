---
title: 'research!rsc: Absolute File System Design'
url: https://research.swtch.com/abs-fs
published: "2008-02-11T00:00:00Z"
feed: russcox
guid: https://research.swtch.com/abs-fs
---

# research!rsc: Absolute File System Design
...
- [seregine](http://www.blogger.com/profile/00716572574695142998)(February 11, 2008 7:37 PM) Interesting. So if I insert a block at the beginning of a 1GB file, the FS has to write to every other block to update its offset?

- [Russ Cox](http://swtch.com/~rsc/)(February 12, 2008 3:19 AM) @seregine: Sure, but the traditional Unix file interface doesn't provide an "insert" operation anyway. "Inserting" at the beginning of the file is accomplished by writing the entire file out again at a different offset anyway.

This is why MP3 tags are usually placed at the end of the MP3 file, making them easier to edit.

- [fanf](http://www.blogger.com/profile/17320082374768944309)(February 15, 2008 2:51 PM) One reason against block labels is that they interleave data and metadata, so users can't mmap files and the OS can't have a unified buffer cache.

- [Russ Cox](http://swtch.com/~rsc/)(February 15, 2008 4:13 PM) @fanf: Placing a block full of N labels every N data blocks would address that problem.

- [mike.w.meyer](http://www.blogger.com/profile/14442121383593520632)(February 15, 2008 8:42 PM) Interleaving data and metadata also hoses the performance of
...
- [Russ Cox](http://swtch.com/~rsc/)(February 17, 2008 6:06 AM) @mike.w.meyer: I don't believe this is a concern in practice, since it's far more common to load the data first into the kernel's buffer cache. The buffer cache can certainly handle some of the buffers being label blocks, so it would still be a sequential transfer.

- [Marius Gedminas](http://www.blogger.com/profile/15155998626202067226)(March 15, 2008 5:52 AM) What are the performance implications of this kind of file system? Would seeking into the middle of a large file require you to read all the blocks from the beginning of the file to follow the metadata chain? How about appending to the end of a big file?

How much disk space would this kind of extensive metadata use, compared to something like ext3?

- [Russ Cox](http://swtch.com/~rsc/)(March 15, 2008 8:10 AM) @marius: Sure, if you used only the linked list to navigate, then the file system accesses would be slow. But there's nothing stopping you from building block trees like Unix file systems and using them as *hints* to find the blocks faster.
