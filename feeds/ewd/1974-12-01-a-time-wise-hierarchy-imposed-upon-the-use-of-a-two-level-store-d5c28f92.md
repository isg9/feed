---
title: "A time-wise hierarchy imposed upon the use of a two-level store (EWD462)"
url: https://www.cs.utexas.edu/~EWD/transcriptions/EWD04xx/EWD462.html
published: "1974-12-01T00:00:00Z"
feed: ewd
guid: https://www.cs.utexas.edu/~EWD/ewd04xx/EWD462.PDF
---
...
The
manuscript was published as pages 67–78 of
...
E.R.Categories: 4.32, 4.34. 6.21, 6.34, 6.39.

NUENEN, 6th December 1974prof.dr.Edsger W.Dijkstra

Burroughs Research Fellow

A time-wise hierarchy imposed upon the use of a two-leve1 store.
...
of what “reasonable performance” means: yet this is no licence to design,
...
addition give a clear indication what type of change —if any— of the
...
to feel having failed “to cope with the problem”; on the contrary: the
identification of seriously “complifying” hardware characteristics seems in the
light of the present state of the art a valuable discovery.
As primary store we assume a random access store as randomly accessible
...
1)     place of information in secondary store need not influence decisions
...
6)      such a modest amount of processor-status information (registers included!:
...
Remark 1. The above asumptions are —or at least: were— not unrealistic. We
...
“replacement algorithm” to choose that victim; its goal is to keep the
...
1)      With a (quasi-)random number generator an “arbitrary” page residing in
...
oldest is chosen and the algorithm becomes a FIFO (“First-In-First-Out”) rule.
...
1)      either we regard primary store as a homogeneous pool of page frames and
...
In this section we shall describe how we propose to exploit our first
three assumptions. After having observed that it is the function of the
replacement algorithm to try to reduce —with a given window size— the number
...
channel has been occupied with “transports” between storage levels as a result
...
processor and channel are resources doing something at a certain speed, but
...
In this section we assume that for each program the vagrancy characterist
...
We have introduced the notion of the “ideal” window size as the one
...
well.
Therefore, if a program is executed with a monotonic replacement
...
up to the maximum: it would only be a minor overhead on the actual page
...
“disastrous resonance” —see Note 1— is still desirable: most desastrous
...
—halfway, say— : either the suggestion is “serious” and the total change will
...
At the last level, but again at a lower frequency, change of window
...
(each with their own appropriate “grain of time”) can be meaningfully
...
The purpose of aiming at C/T-ratios close to 1 was to achieve for
...
Remark 3. when a producer and a consumer are coupled by a bounded buffer,
...
of consicously avoiding extreme of “skew” system states! (End of remark 3.)
...
luck” which no strategy should try to increase and which should never be
...
If the channel achieves a duty cycle close to 100 percent , but
...
If neither processor, nor channel achieves an acceptable duty cycle,
...
consider the attraction of more work, or reprogramming —so as to change vagrancy
...
Acknowledgements. Collective acknowledgements are due to the members of the
IFIP Working Group W.E.2.3 on “Programming Methodology” and to those of the
