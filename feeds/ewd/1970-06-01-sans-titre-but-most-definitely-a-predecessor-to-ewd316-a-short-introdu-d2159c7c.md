---
title: "Sans titre (but most definitely a predecessor to EWD316, “A short introduction into the art of programming”) (EWD287)"
url: https://www.cs.utexas.edu/~EWD/transcriptions/EWD02xx/EWD287.html
published: "1970-06-01T00:00:00Z"
feed: ewd
guid: https://www.cs.utexas.edu/~EWD/ewd02xx/EWD287.PDF
---
...
The market is already so heavily overloaded with introductory texts on computers
...
There is a simple, practical reason. At my University I teach, mainly for
...
There are purely local circumstances and as far as they ale concerned, a
...
The fact is that over the last years I have addressed myself to the question
...
The purpose of this little monograph is to assist the programming reader in
...
It will not be self�contained in the sense that I assume my readers somewhat
...
For those readers who identify the programmer's competence with a thorough
...
In yet another respect, this little monograph will be very incomplete. As
...
It will also be incomplete as a result of the choice of the examples and the
...
There in a further class of potential readers that will find this subject
...
Although I would love to do it, it is impossible to give a true acknowledgment
...
In this suction a number of notions will be introduced, because they are
...
The first notion is that of an action. An action is a happening, taking
place in a finite period of time and establishing a well-defined, intended net
effect. In this description, we have included the requirement that the action's
...
effect.
The requirement that the action should take place in a finite period of time
...
at moment T1".
An example of an action would be a housewife peeling the potatoes for the
...
When we dissect such a happening as a time sequence of (sub)actions, the
cumulative effect of which than equals the net effect of the total happening, then
we say that we regard the happening as a seguential process, or process for short.
Whenever much a dissection is permissible, we can regard the same happening
...
I must stress, that whether some happening is regarded as an action or as a
...
consideration.
The happening of the potato peeling housewife could for instance be described
...
Here the total happening has been described as a time-succession of four
...
cupboard".
We postulate that in each happening we can recognize a pattern of behaviour
or pattern for short; the happening occurs when this pattern is followed. The net
...
How we can recognize the same pattern in different happenings falls outside
...
How correct or incorrect this last statement is depends on what we mean by
...
The two actions of the housewife are as different from each other
...
differ etc.
Yet the two happenings are so similar that the same eye-witness account is
...
In writing down an algorithm, we start by considering the happening to
...
The notion of an algorithm, an executable precept for the establishing
...
In our description of an algorithm we have stressed that the primitive
...
Besides that we have stressed that the repertoire should be well-understood:
...
Comparing the two, we may well ask what we have gained, for it seems a
roundabout way of doing: describing a pattern of behaviour which, when followed,
will evoke the happening, while in the eye�witness account we had an excellent way
...
suppose that after the pan has been fetched, she puts on an apron if necessary,
i.e. when she wears a light-coloured skirt and that on one day she uses the apron
while on the other day she doesn't.
On a rather abstract level -i.e. without explicit mentioning of the apron
...
If, however, we want to go into more detail and want to mention the apron
...
It is here that the potential power of the algorithm becomes apparent, for
...
action begins.
We see two things: the inspection whether the skirt is light-coloured
...
peel the potatoes;
return the basket to the cellar" .
(For historical reasons the so-called conditional connective {if ... do" is
split into two symbols "if" and "do", enclosing the inspection.)
The conditional connective connects two actions, the first of which must be
...
We need a further connective before we can see the full superiority of the
algorithm over the eye�witness account, viz. a repetitive connective.
Suppose that we want to express that "peeling the potatoes" is in itself a
...
If we know a priori that in the worst case (i.e. many guests and vary small
...
Several objections can be made against this solution. There is the practical
...
convince herself!
In order to meet these objections, we introduce a repetitive connective
...
We can also describe the semantics of the repetitive connective in terms of
the conditional one recursively:
"while inspection do action"
is semantically equivalent with
"if inspection do
begin action; while inspection do action end" .
Here the symbols "begin" and "end" are used as opening and closing bracket respectively;
...
[Illegible]
As a final exercise I shall give the algorithm describing the pattern of
...
The notion of an algorithm is a very powerful one, for a single algorithm
...
Note. In the above I have approached the idea of an algorithm starting my
...
connective "if...do" and the repetitive connective "while...do". It is
...
The part of the text denoted by "if condition do" is then described as
...
I see fit to do so.
On the one hand we have the algorithm, a finite text, a static, timeless
...
We have stated that we restrict ourselves to happenings taking place in a finite
...
our attention to happenings that are taking place as a time-succession of a finite
number of primitive actions.
And now we are beginning to see the price: it is very easy to write down a
...
is light�coloured and will remain so and what the pattern suggests to describe is
the peeling of an infinite number of next potatoes. This is usually called "an
improper algorithm".
The question, whether a text that looks like an algorithm, is indeed a proper
...
The assumption of the existence of such an algorithm leeds to a contradiction
...
Consider now the following text, named L
...
Our next fundamental notion is a machine (or "a computer"). A machine is
...
Above, we have given two algorithms for peeling potatoes, one for a natural
...
A mechanism that can only do one thing (such as one of the most widely-spread
...
Machines are mechanical algorithm followers. The fact that in the last
...
Note. It is a trivial matter to compose an algorithm for the fastest machine
...
In other words, we reserve the name program for those algorithms that are intended
...
for a composition in three�four time, he may do so, because, remarkably enough,
...
The fact that machines are fully obedient slaves has caused complaints from
...
In the preceding paragraphs I have introduced programming as an important
...
The computer as piece of equipment presents us with an entirely new combination
...
grip.
But its basic simplicity is only one side of the coin: the other side presents
...
It is the possibility to consider realistically effective solutions of
...
Proqramminq Languages and their Implementation.
...
either "A; B" or "B; A"
viz. action A in time followed by action B or the other way round. It will not
...
For various reasons I decided to restrict my attention to purely sequential
...
With the advent of what is called "large scale integration" (being a term from
...
To get some feeling for the demands made upon the modern automatic computer,
...
The construction of machines able to cope (reliably!) with these two very
...
a) the store (called "memory" in American); this is the component able to
...
It is not the function of the arithmetic unit to be large in the sense that
...
R:= t * R;
x = R
The first instruction fetches the value of a from store into the register R,
...
It also demonstrates that, just as at any moment of time, only a tiny fraction of
...
It also demonstrates that it is no good just to divide the machine into two
...
We have said that the store should be able to strore information, it must, for
...
Storage cells can store information by virtue of the fact that they can be in
...
different states (say "North" and "South"), two such components can be together
in 4 different total states ("North-North", "North-South", "South-North" and "South-
South"), N such components together can be in 2N mutually different states. The
...
is 232, i.e. slightly over 4*1O9; the arithmetic unit will associate with each
state a numerical value, in terms of these numerical values a storage cell can then
hold, for instance, any integer value ranging from (roughly) -2*1O9 to +2*1O9.
The finite capacity of the storage cell is something the user must be aware of:
...
In the mean time we have explained enough about the general machine structure
...
So much for the finite capacity of each storage cell and the fact that a store
...
This natural oredring of storage cells is heavily exploited. If a vector,
i.e. a sequence of numbers a0, al, ... , an has to be stored, its elements
can be stored in successive storage cells. If the address of the element a0 is
known, the address of element ai can than be computed, viz. by adding (the value
of) i to the address of the element a0.
The natural order of the storage cells is also exploited in the program
...
To sketch briefly how this can be done, we return to our previous example
where
"x:= (3 + b)*(c + d)"
...
The first convention is to chose for each instruction a unique number
...
(Note: This is by no means the only way to represent programs inside the
...
possibility.)
The dual role of the store -storage of instructions and storage of
...
The above concludes our bird's eye view of the so-called hardware machine,
...
Because the programmer expressed his program in terms of the instruction
...
But even without the problem of transferring programs from one hardware
...
It was not only the preponderance of coding tricks that made programming
"in machine code" as it is called nowadays, too difficult and too risky.
Firstly, a program in machine code contains very little redundance and
as a result it is very sensitive to even small writing errors -errors of
the level of "spelling mistakes" or "printing errors".
Secondly, the programmer who thinks in terms of variables has to denote
...
Thirdly -and this is the major reason- machine code is an improper
...
The above shortcomings led to the design of so-called "(higher level)
...
The problem now is that on the one hand we have the hardware machine A,
...
The gap is bridged by "software": given machine A, we can make, once and
...
Usually this combination of hard- and software processes
...
The standard software that goes with the machine shields the user off
...
We have mentioned the concept of the virtual store because it is related
...
When introducing the basic notions we have said that different happenings
could take place following the same pattern of behaviour. And as the happening.
...
we shall show how so-called variables are used for the description of the
...
We begin with a very simple program: given two positive integer values
...
(Remark. We have restricted ourselves to positive numbers in order to
...
GCD(0, B) = B .
But admitting zero as argument is asking for trouble, because GCD(0, 0) is
clearly undefined! In order to avoid these complications, we restrict our-
selves to positive arguments.)
For the sake of argument we request an algorithm in which no arithmetic
operations other than addition and subtraction will be used. How do we find
such an algorithm?
Well, there seem in this case two ways to attack the problem. The first
...
We could do it along the lines just sketched (or at least use it as a
...
meaning that a number of its properties are known. If we can think of
so may of its properties that they define the GCD -i.e. that the GCD is the
...
The first property states that GCD is a symmetric function. The second
...
a b separately. The case a = b, however, is very special: it is
the only case in which the value of the GCD is given directly!
Relations 2, 3.1 and 3.} tell us that the GCD of a pair of numbers is
...
applied.
With the second requirement in mind, rule 2 does not look to promising:
...
3.3' if a Now the moment has come to consider our first version of the program:
program 1:
...
(In this program we have used the well-known alternative connective
"if...then...else". The construction
...
When we try to understand this program we should bear the following in
mind:
| While the typical use of variables manifests itself with the
| program loop, the way to understand such a program implies
| looking for the relations between their values which remain
| invariant during the repetition.
In this example the invariant relation P is
P: a > 0 and b > 0 and GCD(a, b) = GCD(A, B) .
The relation P holds after initialization (for the a = A and b = B; from
A > 0 and B > 0, relation P then follows).
The repeatable statement will only be executed under the additional
condition a ≠ b; i.e. either a b. If a > b, then the new value of
a, viz. a - b, will again be positive and GCD(a, b) will remain unchanged on
account of 3.1'; if a When the loop terminates, a = b is guaranteed to hold, GCD(A, B) = GCD(a, b) =
GCD(a, a) and on account of 3.2 the assignment "gcd:= a" will establish the net
effect "gcd:= GCD(A, B)".
To complete the proof we must demonstrate that the repetition will indeed
...
completes the proof.
Once we have this program, it is not difficult to think of others.
...
of a > b already implies the truth of a ≠ b. What happens in time is that a
...
while b > a do b:= b - a
end; gcd:= a; Print(A); print(B); print(gcd)
end
Exercise Prove the correctness of program 2.
Exercise Rewrite program 2 such that the outer repeatable statement contains
only one loop instead of two.Prove its correctness.
Before going on, it is desirable to give a more formal description of
the kind of theorems which we use. (In the following I shall make use of a
formalism introduced by C.A.R.Hoare.)
Let P, P1, P2, .... stand for predicates stating a relation between
...
predicate, i.a, delivering one of the values true of false without further
affecting values of variables, i.e. without changing the current state.
Then P1{S}P2
means: "the truth of P1 immediately prior to the execution of S implies the
...
(Here "true" is the condition which is satisfied per definition, i.e. the
...
Theorem 5 is one of the most useful theorems when dealing with loops. The
...
B: a ≠ b .
We draw attention to the fact that we could not show "P {S} P" but only
"(P and B){S} P": for a and b to remain positive it was necessary to know that
...
a = GCD(A, B) we did not only use the mild conclusion "P {while B do 5} P"
but the strong conclusion "P {while B do S}(P and non B)": we needed the
...
Theorem 6:
Given: (P and B){S} P
Conclusion: in P {while B do S} will (P and B) hold immediately after
each execution of the repeatable statement which is not the
the last one.
This theorem usually plays a role when deriving a contradiction from the
assumption that the loop will not terminate.
* * *
There is an alternative form of repetition control which might be
represented by
"repeat S until B"
...
Sometimes the repeat-clause comes in really handy and gains the text in
...
Remark: This theorem only applies to the case that the repetition terminates,
otherwise it is void.
The greater complexity of the assumptions about B and S which have to be
verfied, reflect the additional care required in the use of the repeat-clause.
Exercise. We know give three tentative alternative programs (3, 4 and 5) for
...
a:= A; b:= B;
while a ≠ b do
begin if a do begin x := a; a := b; b := x end
...
Exercise. Prove that the following program will print in addition to the
...
Hint. Follow the value of the expression a * c + b * d. (Courtesy Software
Sciences Holdings Limited, London)
* * *
For a meaningful loop controlled by a while-clause we can make the
following further observations. Let us consider the loop "while B do S".
Our first observation is that the execution of the repeatable statement
must change the state. Suppose that it does not. If then the repeatable statement
...
So, execution of statement S changes the state: let us call "s" that part
...
"while B(s) do s:= S(s)" .
This is the basic form of every repetition which is controlled by a
...
Let the initial state be denoted by s0 and let the state immediately
after the i�th execution of the repeatable statement be denoted by si ; let the
loop terminate after the k-th execution of the repeatable statement. Then the
following theorems are true.
Theorem 8:
Given s0, the remaining successive values of s are given by the recurrence
relation
si = S(s i-1) for 0 Theorem 9:
In the sequence s0, s1, ... , sk, no two values (with different subscripts)
are equal.
Theorem 10:
B(si) for all i satisfying 0 ≤ i non B(sk)
Theorem 8 is fairly obvious. It is mentioned, however, because it shows
that a repetition is an adequate tool for the evaluation of a recurrence relation
Theorem 9 is mentioned because it is so obviously correct (although I see
no use for it).
Theorem 10 -also called "The Linear Search Theorem"-which is also fairly
obvious, is a very important one, comparable in importance to Theorem 5. It
does not only assert that after termination the state sk will be such that non B
is valid, it asserts that in all previous states si with i smallest number for which a predicate B is false: by inspecting the numbers
in the order of increasing magnitude, the smallest becomes the first found.
...
Remark. We have called the last three theorems "obvious". This is not meant to
...
Finally. in Theorems 2, 3 and 5 we always have assumptions including
...
Theorem 8 mentions successive states connected by a
...
for i = 0 a0 = 1 (1)
c0 = 1 - b , with 0 0) 0 ai =(1 + ci-1)* ai-1 (2)
ci = ci-1 .
Then lim ai = 1/b
i → ∞
Exercise. Prove the last formula. (This has nothing to do with programming,
it is secondary school algebra. The clue of the proof is to be found in the
relation
1 1 + ci-1
--------- = ---------- .)
1 - ci-1 1 - ci
It is requested to use this recurrence relation to approximate the value
of 1/b; obviously we cannot compute infinitely many elements of the sequence
a0, a1, a2, ... but we can accept ak as e sufficiently close (how close?)
approximation of 1/b when ck is less than a given, small, positive tolerance
...
while B(s) do s:= S(s)
and asserts that after the initial state s0 the states si after the i-th
execution of the repeatable statement will satisfy
si = S(si-1) (3)
Our recurrence relations (2) are exactly of the form (3) if we identify
the state si with the value pair ai, ci. That is, we have to introduce, to
...
execution of the repeatable statement by Ai and Ci respectively. We associate
the state si (as given by the values Ai and Ci) with the value pair ai, ci
by the relations Ai = ai (4)
Ci = ci .
(Mind you: in the left hand sides the subscript "i" means "the value of
the variable after the i�th execution of the repeatable statement", in the
...
between them.)
Within the scope of a declaration "real A, C" it is now a straightforward
task to write the piece of program:
"A:= 1; C:= 1 - b;
while abs(C) > eps do +-------------------+
begin A:=(1 + C)* A; | A / (1-C) = 1 / b |
C := C * C +-------------------+
end" .
The first line has to create the proper state s0 and does so in accordance
...
A:= (1 + C)* A;
C:= C * C; .
With the initial condition A = ai-1, C = ci-1 the First assignment is
equivalent to
A:= (1 + ci-1)*ai-1
...
A = ai, C = ci .
Thanks to the explicit occurrence of the subscripts the order of the two
...
vital!
Exercise. In the same EDSAC subroutine library the next scheme is used. Consider
the sequence of pairs ai, ci , given by
for i = 0 a0 = b
c0 =1- b with 0 0 ai = (1 + .5 * ci-1)* ai-1
ci = c2i-1 *(.75 + .25 * ci-1)
then lim ai= b.5
i → ∞
Prove the last Formula and make a program using it for the approximation of
...
Exercise. In the same EDSAC subroutine library the next schema is used.
Consider the sequence of triples inci, si, xi given by
for i = 0 inc0 = log 2
s0 = 0
xi = arg (with 1 ≤ arg O
for x2i-1 i = .5 * inci-1
si = si-1
xi = x2i-1
for x2i-1 ≥ 2 inci = .5 * inci-1
si = si-1 + .5 * inci-1
xi = .5 * x2i-1 .
then lim si = log(arg)
i → ∞
Prove this relation and make a program using it to approximate the logarithm
...
The clue of the proof can be found in the invariance of the relation
...
a[1], a[2], a[3], , a[N] (with ≥ 1)
and a variable called "max". Make a piece of program assigning to the variable
named "max" the maximum value occurring in the sequence. (As N ≥ 1, the sequence
...
on.
How do we define the maximum value occurring in a sequence of length N
for general N ≥ 1? If we call "maximumk" the maximum value occurring among the
first k elements a[1], ... , a[k]
1) the ansewer sought is maximumN
2) the values maximumk are given
for k = 1 by the base: maximum1 = a[1] (5)
-appealing to the knowledge that the maximum element in a
sequence of length 1 must be the only element occurring in the
sequence
for k > 1 by the recurrence relation: maximumk = MAX(maximumk-1, a[k]) (6)
�assuming the knowledge of the function MAX of two arguments.
The recurrence relation (6) presents us with an additional difficulty
...
a method. If we call nk the k�th natural number, then nk = k; the numbers nk
the obvious recurrence relation nk = 1 + nk-1 We can now rewrite the definition
for the sequence of values maximumk in the form of a definition for the pairs
(nk, maximumk):
for k = 1 n1 = 1 (7)
maximum1 = a[1]
for k > 1 nk = 1 + nk-1 (8)
maximumk = MAX(maximumk-1, a[1 + nk-1])
and now the recurrence relations are of the form s, = S(si-1), the
...
(integer) variable; call it "m". Our state si will associate (with k = i + 1)
maxi = maximumk
mi = nk .
...
We have given the above piece of reasoning and the explicit reference
...
It does not consider �"statically" so to speak- the sequence of values
s0, s1,... in the sense that it bothers about the values of the subscript i
in si. It appeals directly to Theorems 5 and 6 and works in terms of assertions
...
P: 1 and maximum = maximumk .
(Here any use of "maximum" and "k" stands for the current value of the variable
thus named, while "maximumk" stands for the value as given by the recurrence
relation. This double use of the same names is tricky but programmers do it.
I too.)
The program then consists oF two parts: establishing relation P in
...
"B and P" i.e.
k and 1 ≤ k ≤ N and maximum = maximumk
which reduces to
1 ≤ k and maximum = maximumk (9)
In order to show that the execution of the repeatable statement under
...
execution by primes.
Initially we have relation (9); after the assignment k:= k + 1 we have
the relation k' = k + 1 and from the first part of (9), i.e. 1 ≤ k k, a[k']),
resulting in the relation
maximum� = maximumk, .
Relations (10) and (11) combine to a replica of P, but now for the primed
quantities.
Termination follows from the fact that each execution of the repeatable
statement involves an effective increase of the integer values variable k.
After termination we have, according to Theorem 5, P and non B, i.e.
1 ≤ k ≤ N and maximum = maximumk and non k k
which concludes the proof.
Exercise. Make a program effectively assigning "prod:= X * Y" with integer X and
Y, satisfying X ≥ O, Y ≥ O
a) using only addition and subtraction
b) using in addition the boolean function "odd(x)", doubling and halving
of a number. (The so�called Egyptian multiplication.)
Exercise. Make a program effectively assigning "rem:= REM(X,Y)" with integer X
and Y, X ≥ O, Y > O, where the function REM(X,Y) is the remainder after division
of X by Y
a) using only addition and subtraction
...
We conclude this section by an example of the (standard) circumstance in
...
x[i] = y[i] for all i satisfying 1 ≤ i ≤ N and the value false if in that range
a value of i exists such that x[i] ≠ y[i]. (The sequences may be empty, in that
case "eq" should get the value true.)
How do we define equality of sequences of length N for general N? Again
by means of a recurrence relation.
Let eqi mean "no difference has been observed among the first i pairs";
then the sequence of values eqi is given by
for i = 0 eq0 = true
for i > O eqi = eqi-1 and x[i] = y[i]
...
to
eq:= true; i:= 0;
while i do begin i:= i + 1; eq:= (eq and x[i] = y[i]) end .
Although the above program is correct, the following program, besides being
equally correct, is on the average much more efficient:
eq:= true; i:= O;
while i and eq do begin i:= i + 1; eq:= (x[i] = y[i]) end
...
Here we go. Consider sequences composed of 1's, 2's and 3's which contain
only pairs of different adjoining non�empty subsequences. Examples of good
sequences are
1
12
12312
...
12523131
32132132132 .
In all probability the list of good sequences is infinite. I tell you that
...
I have used this example extensively in oral examinations. After some initial
...
final digit (now ≠ 3) should be increased by 1, giving the next trial sequence.
...
To a program along the lines sketched, a number of objections can be made.
One objection is that at the beginning of the execution of the repeatable statement
its length will be ≤ 100 and furthermore we know that the operation "ADJUST" will
...
much harder.
A much nicer program is based upon the observation that we can regard the
...
while not GOOD do ADJUST;
PRINT
until length = 100 .
The objections raised are no longer valid. The true reason, however, why
...
PRINT
until length = 100.
(Note. In programs 1, 2 and 3 the (outer) repetition could also have been
controlled by a while-clause.)
This is much more preferable. Observe that here, in program 3,
...
at a time.
To show that all this is not just idle playing with words we shall proceed
...
reasoning!)
To find this refinement we do a step backwards, asking ourselves what
...
SOLUTION".
When refining this operator the trial sequences have to be generated and
...
Here "GOOD" stands for a rather complicated function; an alternative form
...
It has a property "length", now satisfying the inequalities O ≤ length ≤ 1OO
...
declare an integer array d[1:100] and that at any moment the sequence will be
represented by
d[1], d[2], ... , d[length] .
We draw attention to the fact that this is a well-motivated decision. An
...
SET SEQUENCE EMPTY AND LENGTH TD ZERO:
length:= O
EXTEND WITH ZERO:
...
d[length]:= d[length] + 1
PRINT:
i:= O; repeat i:= i + 1; printdigit(d[i]) until i = length; newline
where we have assumed the availability of the primitives "printdigit" and "newline"
...
To investigate an arbitrary sequence is indeed a gruesome task, but it
...
it has to be rejectd as bad if there exists an m (satisfying O SET GOOD:
integer m;
good:= true; m:=1;
while 2 * m ≤ length and good do
begin GIVE GOOD THE MEANING THAT THE HSEQUENEES DIFFER;
m:= m + 1
end
or (probably) better
SET GOOD:
...
begin GIVE GOOD THE MEANING THAT THE MSEQUENCE5 DIFFER;
m:=m+1
end
Here the operator div is the integer divide, rounding the quotient to the
...
repeat good:= (d[firstend - k] ,≠ d[length - k]);
k:= k + 1
until k = m or good
again (see above) expressing that the comparison of two msequences can be
terminated as soon as their difference somewhere has been established.
Collecting the declarations and inserting the refinements -keeping their
...
The program to be made could be regarded as a derivation from the ridiculous
one sketched above, viz. by the introduction of some significant short�cuts. At
present we shall not stress this relation any further.

begin integer array d[1:100]; boolean good; integer length, i, m, unbound, k, firstend;
SET SEQUENCE EMPTY AND LENGTH TO ZERO:
length:=0;
repeat TRANSFORM SEQUENCE INTO NEXT SOLUTION:
EXTEND WITH ZERO:
...
begin GIVE GOOD THE MEANING THAT THE MSEQUENCES DIFFER:
firstend:=length - m; k:=0;
repeat good:= (d[firstend - k] ≠ d[1ength - k]);
k := k + 1
until k = m or good;
m:= m + 1
end
...
We do not know whether there are any solutions at all but we do know that
...
It is this latter property which makes this problem mathematicaaly speaking
very much similar to the previous one.
Hint. The test for acceptability can be speeded up considerably by tabulating
which quintuples are to be found in the sequence.
Remark. This problem is difficult and it will take you hours to produce a
beautiful program. But these hours will be extremely well-spent.
The shortest spanning subtree of a graph.
I have chosen the following example for a variety of reasons. Firstly,
...
Two points can be connected by one point-to-point connection, three points
...
N points equals NN_2.
We now assume that for each possible branch the length has been given.
...
Note. The points don't need to lie in a Euclidean plane, the given distances
don't need to satisfy the triangle inequality.
An apparently straightforward solution would generate all trees between
the N points, compute their lengths and select the shortest one, but Cayley�s
...
length(V) The above theorem tells us that a red subtree of the shortest tree T can
...
COLOUR ONE POINT RED AND THE REMEINING ONES BLUE;
while NUMBER OF RED POINTS do
begin SELECT SHORTEST VIOLET BRANCH;
COLOUR IT AND ITS BLUE ENDPOINT RED
end .
As it stands, the main task will be "SELECT SHORTEST VIOLET BRANCH",
...
Can we find such a definition of the concept "ultraviolet"? Well, for lack of
further knowledge I can only suggest that we try.
Considering that the set of violet branches leading from the k red points
...
Our aim is to keep the ultraviolet subset small, but from their size we
...
Without trying the various adjustments of the ultraviolet sets, there is
...
Consider now that we have a red subtree R and that from the corresponding
...
Exercise. Convince yourself that the rejected choice of the concept "ultraviolet"
is not so helpful.
Let us try to represent our algorithm in its current stage of refinement:
COLOUR ONE POINT RED AND THE REMAININE ONES BLUE;
CONSTRUCT THE SET OF ULTRAVIOLET BRANCHES;
while NUMBER OF RED POINTS do
...
By now the time has come to bother about the representation of the information
...
The answer asked is a tree of N�1 branches, each branch being identified
...
integer array from, to[I:N-1]
where for each h satisfying 1 ≤ h ≤ N-1 the pair "from[h], to[h]" gives
...
from[h], to[h] will be red for 1 ≤ h Point N is chosen as the initial point to be coloured red. SELECT SHORTEST
ULTRAVIOLET BRANCH is a straightforward search for a minimum value (see above)
...
k:=1;
CONSTRUCT THE SET OF ULTRAVIOLET BRANCHES:
h:= 1; while h do begin from[h]:= N; to[h]:= h; 1ength[h]:= distance[N,h]; h:= h + 1 end;
while k do
begin SELECT SHORTEST ULTRAVIOLET BRANCH AND CALL ITS BLUE ENDPOINT P:
minh:= k; minlen:= length[k]; h:= k + 1;
while h do begin len:= length[h]; if len do begin minlen:= len; minh:= h end; h:=h+1
p:= to[minh];
COLOUR IT AND POINT P RED:
...
h:= k;
while h do
begin len:= distance[p, to[h]]; if len do begin length[h]:= len; from[h]:= p end; h:= h +1;
end;
h:= 1; while h do begin print(from[h]); print(to[h]); newline; h := h + 1 end
end
Exercise.
Let distance[i,j] be the distance from point i to point j in that direction.
...
is worth trying!
The towers of Hanoi.
Given three pegs, numbered 1, 2 and 3 and a set of N disks (N ≥ 1]
...
Now, if we can solve this game for any two pegs for N = N0, we can also
...
movedisk(A, B) .
The set of moves
movetower(N0+ 1, A, H, C)
is the same as that of the successive moves of
movetower(N0, A, C , H)
movedisk(A, C)
movetower(N0, B, A, E) .
In words: first a tower of N0 disks is moved from A to B, using C as
auxiliary peg, then the N0 + 1st disk is moved directly from A to C and
finally the tower of N0, that has been placed temporarily on peg B is moved
to its final destination C, using A as auxiliary peg. As the tower consists
of the N0 smallest disks, the presence of larger disks will not cause violation
of the condition of decreasing disk diameter. Moving a one-disk tower (N0 = 1)
...
Note. It is not realistic to require the execution of this program for large
values of N because the total number of moves required = 2N - 1.
The fact that a move with N > 1 is decomposed into three "smaller"
...
introduce
movek, movek-1, ....., move2, move 1
to be done in the order of decreasing subscript, in the order from left
to right. If movek is simple, it is done, leaving
movek'=k-1,......, move2, move0
(indicating with k' the new value of k, the length of the list of obligations)
otherwise movek is replaced by three others, leaving
movek'=k+2 movek'-1=k+1 , movek'-2=k , movek'-3=k-1 ,.....move2 move1 .
In both transformations the lower (i.a. later) k-1 moves have been
unaffected.
...
We can store these moves in four arrays integer array n, from, via, to
[1; 2*N�1]. (Verify that the list of obligations is never longer than 2*N-1
moves.)
...
The non-simple move (with N > 1), given in tabular form by
n= from= via = to =
k: N A B C
...
unaffected.)
Remark. In the program we have not described the details ov the operation
...
The reader is strongly invited to follow the above program himself
...
and for 0 ≤ i The array x and the scalar n are together sufficient to fix any configuration
...
Exercise. For ) 0
2) c[i]>c[i-1]for 1 Again, write two programs, one without and one with recursion and establish
their correctness.
...
for 1≤k As a result of this rearrangement A[f] equals the f-th value in order of
non�decreasing magnitude. We call the array rearranged satisfying (1) "split
...
values to the right. The difficulty is that for given f the final value of A[f],
i.e. our criterion "small/large", is unknown to start with.
Hoare's invention is the following. Select some� rather arbitrary criterion
...
A[k] ≤ V for m ≤ k Having chosen the splitting value, the algorithm will have the duty of building
...
while i do perform the appropriate step" .
...
In its applications we shall only call the procedure "split" with m Exercise. Show that in a version of split that only needs to cater for m repeat until clause as
well.
...
for 1 ≤ i The idea is to apply the operator "split" first to the original array
...
If the split is found to the right of f (i.e. f s, p has to be reset to s + 1 and q can be left
...
element and both halves will be empty. The relation p integer p, q, s; p:= 1 q := N;
while p do
begin s:= split(A, p, q);
if f = s then begin p:= f; q:= f end
else if f then q:= s-1 else p := s+1
end
...
begin integer s;
s:= split(a, p, q)
if p do sort(a, p, s-1);
if s + 1 do sort(a,s + 1, q)
end
such that the call
sort (A, 1, N)
...
if qu - s then
begin if s + 1 do sort(a, s-1, qu); qu;= s - 1 end
else
begin if pu do sort(a,pu, s-1); pu:= s + 1 end
end
end
